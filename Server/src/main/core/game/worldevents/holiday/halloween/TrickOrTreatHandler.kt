...
class TrickOrTreatHandler : InteractionListener {
    override fun defineListeners() {
        on(IntType.NPC, "trick-or-treat"){ player, node ->
            ...
            if(hasDone5){
                sendNPCDialogue(player, node.id, "My informants tell me you've already collected candy from 5 people today.", core.game.dialogue.FacialExpression.FRIENDLY)
                return@on true
            }

            if(hasDoneMe){
                sendNPCDialogue(player, node.id, "You've already asked me today! Don't get greedy, now.", core.game.dialogue.FacialExpression.ANNOYED)
                return@on true
            }
            ...
            player.dialogueInterpreter.open(object : DialogueFile(){
                override fun handle(componentID: Int, buttonID: Int) {
                    when(stage){
                        ...
                        10 -> npcl(FacialExpression.EVIL_LAUGH, "I CHOOSE TRICK!").also { player.lock(); Pulser.submit(
                            object : Pulse() {
                                var counter = 0
                                override fun pulse(): Boolean {
                                    //gfx 1898
                                    when(counter++){
                                        0 -> npc!!.visualize(Animation(1979),
                                            Graphic(1898)
