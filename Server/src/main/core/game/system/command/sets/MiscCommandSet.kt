...
class MiscCommandSet : CommandSet(Privilege.ADMIN) {
    override fun defineCommands() {
        ...
        define("balloon", Privilege.ADMIN, "", "Toggle balloon travel.") { player, _ ->
        ...
        /*
            Calculates and shows you your current max hit.
         */
        ...
        define("players", Privilege.MODERATOR, "", "Lists the online players.") { player, _ ->
            ...
            for (p in Repository.players) {
                if (!p.isArtificial) player.packetDispatch.sendString(
        ...
        define("commands", Privilege.STANDARD, "::commands <lt>page<gt>", "Lists all the commands.") { player, args ->
            ...
            for (i in pages[page] until end) {
                ...
                if (rights > 0) title = "(<img=$icon>) $title"

                player.packetDispatch.sendString(title, Components.QUESTJOURNAL_SCROLL_275, lineid++)

                if (command.usage.isNotEmpty()) player.packetDispatch.sendString(
                    "Usage: ${command.usage}", Components.QUESTJOURNAL_SCROLL_275, lineid++
                )

                if (command.description.isNotEmpty()) player.packetDispatch.sendString(
                    command.description, Components.QUESTJOURNAL_SCROLL_275, lineid++
                )

                player.packetDispatch.sendString(
                    "<str>-------------------------------</str>", Components.QUESTJOURNAL_SCROLL_275, lineid++
                )

                if (lineid > 306) {
                    player.packetDispatch.sendString(
                        "To view the next page, use ::commands ${page + 2}", Components.QUESTJOURNAL_SCROLL_275, lineid
                    )
                    break
                }
        ...
        /*
         * Cleaning all barbarian training related attributes.
         */
        ...
        /*
         * Cleaning barbarian firemaking attributes.
         */
        ...
        /*
         * Cleaning barbarian herblore attributes.
         */

        define("clearbarbherb", Privilege.ADMIN, "::clearbarbherb", "Clean barbarian herblore training attributes.") { player, _ ->
            removeAttributes(player, BarbarianTraining.ATTR_BARB_TRAIN_HERBLORE_BEGIN, BarbarianTraining.ATTR_BARB_TRAIN_HERBLORE)
            notify(player, "%RRemoved barbarian herblore attributes.")
        }
    ...
    fun showGeBotsearch(player: Player, searchTerm: String) {
        ...
        for (i in 0..299) {
            ...
            if (offer != null) setInterfaceText(
                player,
                "${getItemName(offer.key)} (<col=6bff89>x${offer.value}</col>) -> Price: <col=e8d151>${offerPrice[offer.key]}</col>gp",
                Components.QUESTJOURNAL_SCROLL_275,
                lineId++
            )
    ...
    fun showGeSell(player: Player) {
        ...
        for (i in 0..299) {
            ...
            if (offer != null) setInterfaceText(
                player,
                "${getItemName(offer.key)} (<col=6bff89>x${offer.value}</col>) -> Lowest: <col=e8d151>${lowestPrice[offer.key]}</col>gp",
                Components.QUESTJOURNAL_SCROLL_275,
                lineId++
            )
            else setInterfaceText(player, "", Components.QUESTJOURNAL_SCROLL_275, lineId++)
    ...
    fun showGeBuy(player: Player) {
        ...
        for (i in 0..299) {
            ...
            if (offer != null) setInterfaceText(
                player,
                "${getItemName(offer.key)} (<col=6bff89>x${offer.value}</col>) -> Highest: <col=e8d151>${highestPrice[offer.key]}</col>gp",
                Components.QUESTJOURNAL_SCROLL_275,
                lineId++
            )
            else setInterfaceText(player, "", Components.QUESTJOURNAL_SCROLL_275, lineId++)
        ...
        for (i in 0..299) {
            if (i > buyingAmount.keys.size) {
                ...
                if (offer != null) {
                    setInterfaceText(
                        player,
                        "[SELLING] ${getItemName(offer.key)} (<col=6bff89>x${offer.value}</col>) -> Lowest: <col=e8d151>${sellingLowest[offer.key]}</col>gp",
                        Components.QUESTJOURNAL_SCROLL_275,
                        lineId++
                    )
                    continue
                ...
                if (offer != null) {
                    setInterfaceText(
                        player,
                        "[BUYING] ${getItemName(offer.key)} (<col=6bff89>x${offer.value}</col>) -> Highest: <col=e8d151>${buyingHighest[offer.key]}</col>gp",
                        Components.QUESTJOURNAL_SCROLL_275,
                        lineId++
                    )
                    continue
