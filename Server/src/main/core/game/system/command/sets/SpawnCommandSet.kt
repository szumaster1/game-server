...
class SpawnCommandSet : CommandSet(Privilege.ADMIN){
    ...
    override fun defineCommands() {
        ...
        define("object"){player,args ->
            ...
            val `object` = if (args.size > 3) Scenery(CommandPlugin.toInteger(args[1]), player.location, CommandPlugin.toInteger(
                args[2]
            ), CommandPlugin.toInteger(
                args[3]
            )) else if (args.size == 3) Scenery(CommandPlugin.toInteger(
                args[1]
            ), player.location, CommandPlugin.toInteger(
                args[2]
            )) else Scenery(CommandPlugin.toInteger(
                args[1]
            ), player.location)
