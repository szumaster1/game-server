...
enum class Decoders530(val opcode: Int) {
    ...
    USEWITH_ITEM(27) {
        override fun decode(player: Player, buffer: IoBuffer): Packet {
            ...
            return Packet.UseWithItem(
                player, usedId, usedWithId, usedSlot, usedWithSlot, usedIface, usedWithIface, usedChild, usedWithChild
            )
        }
    },
    USEWITH_SCENERY(134) {
    ...
    CHAT_SETTINGS(157) {
        override fun decode(player: Player, buffer: IoBuffer): Packet {
            return Packet.ChatSetting(
                player, buffer.get(), buffer.get(), buffer.get()
            )
        }
    },
    CHAT_MESSAGE(237) {
    ...
    QUICKCHAT(167) {
        override fun decode(player: Player, buffer: IoBuffer): Packet {
            ...
            val packetType = when (x.array().size) {
                ...
                else -> QCPacketType.UNHANDLED.also {
                    log(
                        this::class.java, Log.WARN, "UNHANDLED QC PACKET TYPE Size ${x.array().size}"
                    )
                }
            }
            ...
            when (packetType) {
                ...
                QCPacketType.UNHANDLED -> log(
                    this::class.java, Log.WARN, "Unhandled packet type, skipping remaining buffer contents."
                )

                else -> {}
            }
            return Packet.QuickChat(
                player, selection_a_index, selection_b_index, forClan, multiplier, offset, packetType
            )
        }
    },
    ...
    companion object {
        ...
        @JvmStatic
        fun process(player: Player, opcode: Int, buffer: IoBuffer): Packet {
