...
class UpdateClanChat : OutgoingPacket<ClanContext> {
    ...
    override fun send(context: ClanContext) {
        ...
        if (!context.isLeave) {
            buffer.putLong(StringUtils.stringToLong(clan.owner))
            buffer.putLong(StringUtils.stringToLong(clan.name))
            ...
            for (i in 0 until size) {
                ...
                val name = entry.name
                buffer.putLong(StringUtils.stringToLong(name)).putShort(entry.worldId)
