...
public final class AnimationDefinition {
    ...
    private void readValues(ByteBuffer buffer, int opcode) {
        if (opcode == 1) {
            ...
            if (opcode != 3) {
                ...
                else if (opcode == 5) anInt2142 = buffer.get() & 0xFF;
                else if (opcode != 6) {
                    if (opcode == 7) emoteItem = buffer.getShort() & 0xFFFF;
                    else if ((~opcode) != -9) {
                        if (opcode != 9) {
                            if (opcode != 10) {
                                if (opcode == 11) anInt2155 = buffer.get() & 0xFF;
                                else if (opcode == 12) {
                            ...
                            } else anInt2162 = buffer.get() & 0xFF;
                        } else anInt2140 = buffer.get() & 0xFF;
                    } else anInt2136 = buffer.get() & 0xFF;
                } else anInt2144 = buffer.getShort() & 0xFFFF;
            } else {
    ...
    public void method2394() {
        ...
        if (anInt2162 == -1) {
            if (aBooleanArray2149 == null) anInt2162 = 0;
            else anInt2162 = 2;
        }
    }
