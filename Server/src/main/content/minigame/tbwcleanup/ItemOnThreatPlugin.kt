...
class ItemOnThreatPlugin : InteractionListener {
    ...
    override fun defineListeners() {
        ...
        onUseAnyWith(IntType.NPC, BROODOO_VICTIM_GREEN) { player, used, with ->
            val maxDamage = if (ANTIPOISONS.contains(used.id)) { 20 }
            else if (SUPER_ANTIPOISONS.contains(used.id)) { 30 }
            else if (SANFEW_SYRUM.contains(used.id)) { 30 }
            else if (ANTIPOISON_PLUS.contains(used.id)) { 40 }
            else if (ANTIPOISON_PLUS_PLUS.contains(used.id)) { 50 }
            else {
        ...
        onUseWith(IntType.NPC, Items.INSECT_REPELLENT_28, *MOSQUITOS) { _, _, with ->
            ...
            for (skillNumber in arrayOf(0,1,2,4,6)) {
                with.asNpc().skills.setStaticLevel(skillNumber, (with.asNpc().skills.getStaticLevel(skillNumber)+1)/2)
            }
