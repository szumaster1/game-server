...
class BountyHunterActivity @JvmOverloads constructor(val type: CraterType = CraterType.LOW_LEVEL) : ActivityPlugin("BH " + type.name.lowercase(Locale.getDefault()), false, false, false, ZoneRestriction.FOLLOWERS) {
    ...
    private val waitRoomPulse: Pulse = object : Pulse(1) {
    ...
    private val gamePulse: Pulse = object : Pulse(10) {
        override fun pulse(): Boolean {
    ...
    override fun register() {
        ...
        if (type == CraterType.LOW_LEVEL) {
            ...
            definePlugin(object : ComponentPlugin() {
                ...
                override fun handle(
                    ...
                    if (button == 18) {
                        ...
                        player.lock(1)
                        val activity = player.getExtension<BountyHunterActivity>(
    ...
    override fun ignoreMultiBoundaries(attacker: Entity, victim: Entity): Boolean {
    ...
    override fun actionButton(
        ...
        if (interfaceId == 192 && buttonId == 19) {
            ...
            if (entry != null && player.getAttribute("pickup_penalty", 0) > ticks) {
                player.packetDispatch.sendMessage("You should not be picking up items. Now you must wait before you can leave.")
            ...
            if (player.getAttribute("exit_penalty", 0) > ticks) {
                player.packetDispatch.sendMessage("You can't use the protect item prayer until your penalty has passed.")
    ...
    override fun continueAttack(e: Entity, target: Node, style: CombatStyle, message: Boolean): Boolean {
        if (e is Player && target is Player) {
            if (target.getAttribute("bh_joined", -1) > ticks) {
                e.packetDispatch.sendMessage("This player has only just entered and is temporarily invulnerable to attacks.")
                return false
            }
    ...
    fun leaveCrater(player: Player, logout: Boolean, entry: BountyEntry) {
        if (entry.hunter != null) {
            ...
            if (other != null) {
                entry.hunter!!.packetDispatch.sendMessage("Your target has " + (if (logout) "logged out" else "left") + ". You shall be found a new target.")
                other.target = null
    ...
    override fun death(e: Entity, killer: Entity): Boolean {
        if (e is Player) {
            ...
            if (entry != null) {
                ...
                } else if (entry.hunter === killer) { // "They" is not a
                    // typo, this was
                    // the actual
                    // message on RS.
                    entry.hunter!!.packetDispatch.sendMessage("You killed " + player.username + ". They were your target, so your Hunter PvP rating increases!")
                    entry.hunter!!.getSavedData().activityData.updateBountyHunterRate(1)
                    BHScoreBoard.hunters.check(entry.hunter!!)
                } else if (entry.hunter != null) {
                    entry.hunter!!.packetDispatch.sendMessage("Your target has died. You shall be found a new target.")
                }
                if (entry.hunter != null) {
                    val other = players[entry.hunter]
                    if (other != null) {
    ...
    private fun handleRogueKill(player: Player, victim: Player, entry: BountyEntry) {
        ...
        player.packetDispatch.sendMessage("This means you get the pick-up penalty: pick anything up and you can't leave!")
