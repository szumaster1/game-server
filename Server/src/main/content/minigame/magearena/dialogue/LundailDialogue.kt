...
/**
 * Represents the Lundail dialogue.
 */
@Initializable
class LundailDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            3 -> npc("I sell rune stones. I've got some good stuff, some really", "powerful little rocks. Take a look.").also { stage++ }
            4 -> end().also { openNpcShop(player, NPCs.LUNDAIL_903) }
            5 -> npc("That, my friend is the mage battle arena. Top mages", "come from all over " + settings!!.name + " to compete in the arena.").also { stage++ }
