...
class KolodionDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        ...
        when (stage) {
            0 -> if (getStatLevel(player, Skills.MAGIC) < 60) {
                npc("Do not waste my time with trivial questions. I am the", "Great Kolodion, master of battle magic. I have an arena", "to run.").also { stage++ }.also { stage++ }
            } else {
    ...
    private fun startFight(player: Player) {
        ...
        player.lock()
        animate(npc, Animation.create(811))
        player.teleport(Location.create(3105, 3934, 0), 3)
        visualize(player, Animation.create(1816), Graphic.create(301, 50))
