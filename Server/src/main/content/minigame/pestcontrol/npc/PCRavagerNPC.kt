...
class PCRavagerNPC : AbstractNPC {
        ...
        if (session != null && !properties.combatPulse.isAttacking) {
            if (target == null) {
                ...
                if (!target!!.isActive || !session!!.barricades.contains(target)) {
                    ...
                    Pulser.submit(object : Pulse(1, this, o) {
    ...
    private fun walk(destination: Node?) {
        pulseManager.run(object : MovementPulse(this, destination, Pathfinder.SMART) {
            override fun pulse(): Boolean {
                return true
            }
        }, PulseType.STANDARD)
    ...
    override fun getIds(): IntArray {
        return intArrayOf(3742, 3743, 3744, 3745, 3746)
    }
