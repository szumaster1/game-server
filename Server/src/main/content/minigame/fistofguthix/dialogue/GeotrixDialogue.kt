    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            17 -> when (buttonId) {
                ...
                3 -> npc("This is what Alran thinks, for what it's worth...which isn't", "very much, if you ask me. Guthix has apparently left us", "a test. He deliberately created this site of power", "as a test of individual strength and balance.").also { stage = 32 }
                4 -> player("I'd better get going.").also { stage = END_DIALOGUE }
            }
            19 -> npc("At the end of the God Wars, Guthix returned to Gielinor", "and was angered by the destruction caused by the other", "gods in their vanity and quest for power.").also { stage++ }
            20 -> npc("So, maybe he punched the ground in anger, leaving an imprint of", "power on the ground that has", "lasted ever since.").also { stage++ }
            21 -> npc("Because he was angry at the time, the power would have", "been tainted with aggression, which is why he", "left Fiara as a guardian, to make sure the power", "did not escape unfettered.").also { stage++ }
            22 -> npc("Do you want to hear another theory?").also { stage = 16 }
            24 -> player("What did he find?").also { stage++ }
            25 -> npc("I don't know for certain, but it must be under", "that strange impression in the big cave. I think", "that mark is some sort of ward that Guthix has", "left to protect the site.").also { stage++ }
            ...
            42 -> npc("Errr, ages. Since the end of the God Wars I think.", "She doesn't talk about that much.").also { stage = END_DIALOGUE }
        }
        return true
    }
