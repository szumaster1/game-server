...
class BlastFurnace : MapArea, PersistPlayer, TickListener {
    ...
    private fun updateVisuals() {
        ...
        sceneryController.updateBreakable(
            state.potPipeBroken, state.pumpPipeBroken, state.beltBroken, state.cogBroken
        )
    ...
    companion object {
        ...
        fun placeAllOre(p: Player, id: Int = -1, accountForSkill: Boolean = false) {
            ...
            for ((oreId, amount) in oreCounts) {
                ...
                if (oreId == Items.COPPER_ORE_436 || oreId == Items.TIN_ORE_438) maxAmt += (BlastUtils.ORE_LIMIT - getAmountOnBelt(
                    p,
                    oreId
                ))
