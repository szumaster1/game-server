        ...
        when (stage) {
            ...
            320 -> {
                ...
                if (player!!.inventory.containsItem(price) && player!!.inventory.remove(price)) {
                    npcl(FacialExpression.OLD_NORMAL, "Here you are, then.")
            ...
            330 -> npcl(FacialExpression.OLD_NORMAL, "Okay then.").also { stage = END_DIALOGUE }
            40 -> npcl(FacialExpression.OLD_NORMAL, "Of course.").also { stage++ }
            41 -> {
                end()
                player!!.interfaceManager.open(Component(686))
            }

            51 -> npcl(FacialExpression.OLD_NORMAL, "Okay, dear.").also { stage = END_DIALOGUE }
        }
    }
}
