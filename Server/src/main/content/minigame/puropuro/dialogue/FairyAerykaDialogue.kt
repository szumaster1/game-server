...
class FairyAerykaDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            1 -> npc(FacialExpression.OLD_DEFAULT, "It's still here. The crop circle is still here.").also { stage++ }
            2 -> player("Oh yes, thanks Aery. It didn't go anywhere in the", "meantime, then?").also { stage++ }
            3 -> npc(FacialExpression.OLD_DEFAULT, "Nope. It just sat there.").also { stage++ }
            4 -> player("Jolly good. I can come back and visit Puro-Puro", "whenever I want then. Brilliant!").also { stage++ }
            5 -> options("What's in Puro-Puro?", "So what are these implings then?", "I've heard I may find dragon equipment in Puro-Puro?", "No, bye!").also { stage++ }
            6 -> when (buttonId) {
