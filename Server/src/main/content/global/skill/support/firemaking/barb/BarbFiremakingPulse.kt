...
class BarbFiremakingPulse(player: Player, node: Item, groundItem: GroundItem?) : SkillPulse<Item?>(player, node) {
    ...
    private val bows = SkillingTool.getBow(player)
    ...
    override fun checkRequirements(): Boolean {
        ...
        if (!inInventory(player,bows!!.id)) {
            sendMessage(player, "You do not have the required items to light this.")
            return false
        }

        if (getStatLevel(player, Skills.FIREMAKING) < fire.level) {
            sendMessage(player, "You need a firemaking level of " + fire.level + " to light this log.")
            return false
        }
