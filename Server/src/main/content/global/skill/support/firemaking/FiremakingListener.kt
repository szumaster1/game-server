...
class FiremakingListener : InteractionListener {
    ...
    private var barbarianTools = getToolForSkill(player!!, Skills.FIREMAKING)
    ...
    override fun defineListeners() {
        ...
        onUseWith(IntType.ITEM, barbarianTools!!.id, *logs) { player, _, with ->
            player.pulseManager.run(FireMakingPulse(player, with.asItem(), null))
            return@onUseWith true
        }

        onUseWith(IntType.GROUNDITEM, barbarianTools!!.id, *logs) { player, _, with ->
