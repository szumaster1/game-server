...
class KaramjaGrapple : OptionHandler() {
    ...
    fun handleObjects(add: Boolean, player: Player) {
        ...
        if (add) {
            if (current.y > 3134) {
                ROPES.add(Scenery(1998, Location.create(2874, 3141, 0), 10, 0))
                ROPES.add(Scenery(1998, Location.create(2874, 3140, 0), 10, 0))
                ROPES.add(Scenery(1998, Location.create(2874, 3139, 0), 10, 0))
                ROPES.add(Scenery(1998, Location.create(2874, 3138, 0), 10, 0))
                ROPES.add(Scenery(1998, Location.create(2874, 3137, 0), 10, 0))
            } else {
                ROPES.add(Scenery(1998, Location.create(2874, 3128, 0), 10, 0))
                ROPES.add(Scenery(1998, Location.create(2874, 3129, 0), 10, 0))
                ROPES.add(Scenery(1998, Location.create(2874, 3130, 0), 10, 0))
                ROPES.add(Scenery(1998, Location.create(2874, 3131, 0), 10, 0))
                ROPES.add(Scenery(1998, Location.create(2874, 3132, 0), 10, 0))
                ROPES.add(Scenery(1998, Location.create(2874, 3133, 0), 10, 0))
            }
            for (rope in ROPES) {
                ...
                Pulser.submit(object : Pulse(0, player) {
                    ...
                    override fun pulse(): Boolean {
                        when (counter++) {
                            ...
                            12 -> {
                                ForceMovement.run(player, player.location, if (current.y > 3134) Location.create(2875, 3136) else Location.create(2875, 3133), ForceMovement.WALK_ANIMATION, ForceMovement.WALK_ANIMATION)
                                handleObjects(false, player)
                            }
                            15 -> ForceMovement.run(player, player.location, if (current.y > 3134) Location.create(2875, 3133) else Location.create(2875, 3136), RUN_ANIMATION, ForceMovement.RUNNING_SPEED)
                            ...
                            25 -> visualize(player, -1, Graphic.create(68))
                            26 -> {
                                ForceMovement.run(player, player.location, if (current.y > 3134) Location.create(2874, 3127) else Location.create(2874, 3142), Animation.create(4466), ForceMovement.RUNNING_SPEED)
                                handleObjects(true, player)
                            }
                            34 -> {
                                player.unlock()
