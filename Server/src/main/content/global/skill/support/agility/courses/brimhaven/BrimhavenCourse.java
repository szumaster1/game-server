package content.global.skill.support.agility.courses.brimhaven;
...
public final class BrimhavenCourse extends OptionHandler {
    ...
    private static void handlePlankObstacle(final Player player, Scenery object) {
        ...
        if (object.getCharge() == 500) { // Plank is broken
            player.lock(7);
            AgilityHandler.walk(player, -1, start, start.transform(dir.getStepX() * 2, dir.getStepY() * 2, 0), Animation.create(1426), 0.0, null);
