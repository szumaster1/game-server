...
class KittenInteractDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            4 -> when (buttonId) {
                1 -> {
                    if (player.familiarManager.hasFamiliar()) { //in case the cat had already run away from hunger by the time the player clicked 'yes'
                        player.sendChat("Shoo cat!")
                        val currentPet = player.familiarManager.familiar as Pet
