...
public final class SpinningPulse extends SkillPulse<Item> {
    ...
    private final SpinningData type;

    private int ammount;

    private int ticks;

    public SpinningPulse(Player player, Item node, int ammount, SpinningData def) {
        super(player, node);
        this.type = def;
    ...
    public boolean reward() {
        ...
        if (player.getInventory().remove(new Item(type.getNeed(), 1))) {
             ...
             */
            if (player.getViewport().getRegion().getId() == 10806 && !player.getAchievementDiaryManager().getDiary(DiaryType.SEERS_VILLAGE).isComplete(0, 4)) {
