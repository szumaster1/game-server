...
class StewRecipe : Recipe() {
    ...
    override fun getMixMessage(event: NodeUsageEvent): String {
        return "You cut up the " + (if (event.usedItem.name.lowercase().contains("incomplete")) event.baseItem.name.lowercase() else event.usedItem.name.lowercase().replace("cooked", "").trim { it <= ' ' }) + " and put it into the stew."
    }
