...
import content.global.skill.gathering.farming.Patch;
...
import static core.game.system.command.sets.StatsAttributeSetKt.STATS_LOGS;

/**
 * The Woodcutting skill pulse.
 */
public class WoodcuttingPulse extends Pulse {
    private final int[] woodcuttingSounds = {
    ...
    /**
     * Instantiates a new Woodcutting skill pulse.
     *
     * @param player the player
     * @param node   the node
     */
    public WoodcuttingPulse(Player player, Scenery node) {
        super(1, player, node);
        this.player = player;
        this.node = node;
        super.stop();
    ...
    public boolean reward() {
        ...
        if (SkillingTool.getHatchet(player).id == 13661 && RandomFunction.random(100) < 25) {
        ...
        if (reward > 0) {
            ...
            if (RandomFunction.random(chance) == chance / 2) {
                if (SkillcapePerks.isActive(SkillcapePerks.NEST_HUNTER, player)) {
                    if (!player.getInventory().add(BirdNestDropTable.getRandomNest(false).getNest())) {
