...
class FalconryCatchPulse(player: Player?, node: NPC, private val falconCatch: FalconCatch) :
    ...
    override fun checkRequirements(): Boolean {
        if (!checked) {
            ...
            if (player.getSkills().getLevel(Skills.HUNTER) < falconCatch.level) {
                sendMessage(
                    player,
    ...
    override fun reward(): Boolean {
        ...
        if (success) {
            ...
            setAttribute(falcon, "falcon:catch", falconCatch)
            falcon.init()
            registerHintIcon(player, falcon)
            playAudio(player, Sounds.HUNTING_FALCON_SWOOP_2634, 10, 1, node!!.asNpc().location, 12)
            Pulser.submit(object : Pulse(100, falcon) {
                override fun pulse(): Boolean {
                    if (!falcon.isActive) {
                        return true
                    }
                    val projectile = Projectile.create(node!!.asNpc(), findNPC(5093), 918) // 916-924
                    projectile.speed = 80
                    projectile.send()
                    sendMessage(player, "Your falcon has left its prey. You see it heading back toward the falconer.")
                    falcon.clear()
                    return true
                }
            })
        } else {
            removeItem(player, GLOVE)
