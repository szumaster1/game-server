package content.global.transportation
 ...
 */
class FairyRing : InterfaceListener, InteractionListener {
 ...
 */
enum class Ring(val tile: Location?, val tip: String = "", val childId: Int = -1) {
    /**
     * Aiq.
     */
    ...
    companion object {
        ...
        fun drawLog(player: Player) {
            for (i in Ring.values().indices) {
                if (!player.savedData.globalData.hasTravelLog(i)) {
                    continue
                }
                val ring = Ring.values()[i]
                if (ring.childId == -1) {
                    continue
                }
