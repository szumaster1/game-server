        ...
        when (stage) {
            START_DIALOGUE -> when {
                hasIronmanRestriction(player, IronmanMode.ULTIMATE) -> {
                    npcl(FacialExpression.ANNOYED, "My apologies, dear ${if (player.isMale) "sir" else "madam"}, " + "our services are not available for Ultimate ${if (player.isMale) "Ironmen" else "Ironwomen"}").also { stage = END_DIALOGUE }
                }
            ...
            2 -> showTopics(
                ...
                IfTopic(FacialExpression.FRIENDLY, "I'd like to switch to my ${getBankAccountName(player, true)} bank account.", 13, hasActivatedSecondaryBankAccount(player)),
                IfTopic(FacialExpression.FRIENDLY, "I'd like to open a secondary bank account.", 20, !hasActivatedSecondaryBankAccount(player)),
