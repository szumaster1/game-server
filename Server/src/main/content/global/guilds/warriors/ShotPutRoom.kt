...
class ShotPutRoom(player: Player? = null) : Dialogue(player) {
    ...
    companion object {
        ...
        private fun throwShotPut(player: Player, delay: Int, lowWeight: Boolean, message: String) {
            ...
            Pulser.submit(object : Pulse(delay, player) {
                ...
                override fun pulse(): Boolean {
                    if (!thrown) {
                        ...
                        val projectile = Projectile.create(loc, loc.transform(tiles, 0, 0).also { loc = it }, 690, 40, 0, if (getDelay() == 5) 5 else 12, speed, 20, 11)
                        projectile.send()
