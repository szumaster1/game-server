...
/**
 * The Grand exchange interface plugin.
 */
@Initializable
public class GrandExchangeInterfacePlugin extends ComponentPlugin {
    ...
    /**
     * Handle search interface boolean.
     *
    ...
    public boolean handleCollectionBox(final Player player, int opcode, int button, int slot, int itemId) {
        ...
        if (index > -1 && (offer = records.getOffer(records.getOfferRecords()[index])) != null) {
            StockMarketInterfaceListener.withdraw(player, offer, slot >> 1);
        }
        return true;
    }
