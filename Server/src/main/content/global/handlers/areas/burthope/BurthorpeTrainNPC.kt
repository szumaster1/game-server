...
class BurthorpeTrainNPC : AbstractNPC {
    ...
    override fun construct(id: Int, location: Location, vararg objects: Any): AbstractNPC {
        return content.global.handlers.areas.burthope.BurthorpeTrainNPC(id, location)
    }
    ...
    override fun getIds(): IntArray {
        return content.global.handlers.areas.burthope.BurthorpeTrainNPC.Companion.IDS
    }
    ...
    private fun action() {
        when (id) {
            1061 -> {
                sendChat(
                    content.global.handlers.areas.burthope.BurthorpeTrainNPC.Companion.MESSAGES[RandomFunction.random(
                        content.global.handlers.areas.burthope.BurthorpeTrainNPC.Companion.MESSAGES.size)])
            ...
            1063 -> {
                faceLocation(getLocation().transform(2, 0, 0))
                animate(if (RandomFunction.random(2) == 1) content.global.handlers.areas.burthope.BurthorpeTrainNPC.Companion.PUNCH else content.global.handlers.areas.burthope.BurthorpeTrainNPC.Companion.KICK)
                delay = System.currentTimeMillis() + 3000 + RandomFunction.random(1000, 3000)
            }

            1066, 1067, 1068 -> {
                animate(content.global.handlers.areas.burthope.BurthorpeTrainNPC.Companion.EAT)
                delay = System.currentTimeMillis() + 3000 + RandomFunction.random(1000, 2000)
            }

            1062 -> {
                val rand = RandomFunction.random(5)
                val animation = if (rand == 1) content.global.handlers.areas.burthope.BurthorpeTrainNPC.Companion.PUNCH else if (rand == 2) content.global.handlers.areas.burthope.BurthorpeTrainNPC.Companion.KICK else content.global.handlers.areas.burthope.BurthorpeTrainNPC.Companion.DEFEND
                animate(animation)
