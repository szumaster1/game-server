package content.global.handlers.areas.falador.partyroom
...
class PartyRoomOptions : OptionHandler() {
    ...
    override fun newInstance(arg: Any?): Plugin<Any> {
        ...
        definePlugin(content.global.handlers.areas.falador.partyroom.BalloonManager())
        return this
    }
    ...
    override fun handle(player: Player, node: Node, option: String): Boolean {
        when (node.id) {
            ...
            LEVER -> PartyRoom.handleLever(player, node.asScenery())
        }
        return true
    }
    ...
    private fun deposit(player: Player) {
        if (!viewers.containsKey(player.name)) {
            viewers[player.name] = content.global.handlers.areas.falador.partyroom.ChestViewer(player).view()
        } else {
    ...
    class DepositInterfaceHandler : ComponentPlugin() {
        ...
        override fun handle(player: Player, component: Component, opcode: Int, button: Int, slot: Int, itemId: Int): Boolean {
            var itemId = itemId
            val viewer = player.getExtension<content.global.handlers.areas.falador.partyroom.ChestViewer>(content.global.handlers.areas.falador.partyroom.ChestViewer::class.java)
    ...
    companion object {
        ...
        @JvmStatic
        val viewers: MutableMap<String, content.global.handlers.areas.falador.partyroom.ChestViewer> = HashMap()
        val balloonManager = content.global.handlers.areas.falador.partyroom.BalloonManager()
        var isDancing = false
