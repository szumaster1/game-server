...
/**
 * The Skull sceptre plugin.
 */
@Initializable
public class SkullSceptrePlugin extends UseWithHandler {
    /**
     * Instantiates a new Skull sceptre plugin.
     */
    ...
    public boolean handle(NodeUsageEvent event) {
        if (event.getUsedItem().getId() == 9008 && event.getUsedWith().getId() == 9007) {
        ...
        if (event.getUsedItem().getId() == 9011 && event.getUsedWith().getId() == 9010) {
        ...
        if (event.getUsedItem().getId() == 9012 && event.getUsedWith().getId() == 9009) {
