...
class OysterPlugin : OptionHandler() {
    ...
    override fun handle(player: Player, node: Node, option: String): Boolean {
        player.inventory.replace(if (RandomFunction.random(5) < 3) OYSTERS[RandomFunction.random(OYSTERS.size - 1)] else OYSTERS[RandomFunction.random(OYSTERS.size)], (node as Item).slot)
        return true
    }
