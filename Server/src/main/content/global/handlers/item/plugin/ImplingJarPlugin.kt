...
class ImplingJarPlugin : UseWithHandler(
    ...
    override fun newInstance(arg: Any?): Plugin<Any> {
        ...
        for (i in getValidChildren(5908)) {
    ...
    private fun fillOilStill(player: Player, used: Item) {
        ...
        if (used.id == 1939) {
            ...
            if (configValue == 0) {
                ...
                if (inInventory(player, 4525, 1) || inInventory(player,4535, 1) || inInventory(player,4546, 1) || inInventory(player,4700, 1)) {
                    val replace = Item(if (used.id == 4525) 4522 else if (used.id == 4535) 4537 else if (used.id == 4546) 4548 else 4701)
