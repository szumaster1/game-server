...
@Initializable
class EquipmentInterfacePlugin : ComponentPlugin() {
        ...
        if (component.id == Components.EQUIP_SCREEN2_667) {
            ...
            when (opcode) {
                155 -> {
                    ...
                    Pulser.submit(object : Pulse(1, p) {
        ...
        when (opcode) {
            206 -> {
                ...
                52 -> {
                    ...
                    // (Highlight white items are auto destroyed on death Enum#616
                    // (Items kept on death interface)
                    // TODO: Parse server sided
                    // SCRIPT 118 - Items kept on death interface CS
                    // ARG 0: Safe location check Takes:
                    // 0 Safe Area/2 in POH/3 in Castle Wars/4 in Trouble Brewing/5 in Barbass
                ...
                55 -> {
                    ...
                    p.interfaceManager.openComponent(Components.EQUIP_SCREEN2_667).setCloseEvent { player: Player, c: Component? ->
                    ...
                    InterfaceContainer.generateItems(p, p.inventory.toArray(), arrayOf("Wear"), Components.INVENTORY_WEAR2_670, 0, 7, 4, 93)
