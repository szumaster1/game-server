...
object LetterDeliveryContent {
    ...
    private val RELDO_LETTER_CONTENT = arrayOf(
    ...
    private val ABBOT_LANGLEY_LETTER_CONTENT = arrayOf(
    ...
    private val THURGO_LETTER_CONTENT = arrayOf(
    ...
    private val HIGH_PRIEST_LETTER_CONTENT = arrayOf(
    ...
    private val FR_LAWRENCE_LETTER_CONTENT = arrayOf(
    ...
    private val FR_AERECK_LETTER_CONTENT = arrayOf(
    ...
    val LETTER_DELIVERY_ATTRIBUTE = "/save:${WomtUtils.LETTER_DELIVERY}"

    fun getLetterContent(player: Player, npc: Int) {
        //val npcId = getAttribute(player, OldManUtils.LETTER_DELIVERY, npc)
        val test = intArrayOf(NPCs.ORACLE_746, NPCs.RELDO_2660, NPCs.ABBOT_LANGLEY_801, NPCs.THURGO_604, NPCs.HIGH_PRIEST_216, NPCs.FATHER_LAWRENCE_640, NPCs.FATHER_AERECK_456)
        when (test.random()) {
            NPCs.ORACLE_746 -> ScrollInterface.scrollSetup(player, Components.MESSAGESCROLL_220, THE_ORACLE_LETTER_CONTENT)
            NPCs.RELDO_2660 -> ScrollInterface.scrollSetup(player, Components.MESSAGESCROLL_220, RELDO_LETTER_CONTENT)
            NPCs.ABBOT_LANGLEY_801 -> ScrollInterface.scrollSetup(player, Components.MESSAGESCROLL_220, ABBOT_LANGLEY_LETTER_CONTENT)
            NPCs.THURGO_604 -> ScrollInterface.scrollSetup(player, Components.MESSAGESCROLL_220, THURGO_LETTER_CONTENT)
            NPCs.HIGH_PRIEST_216 -> ScrollInterface.scrollSetup(player, Components.MESSAGESCROLL_220, HIGH_PRIEST_LETTER_CONTENT)
            NPCs.FATHER_LAWRENCE_640 -> ScrollInterface.scrollSetup(player, Components.MESSAGESCROLL_220, FR_LAWRENCE_LETTER_CONTENT)
            NPCs.FATHER_AERECK_456 -> ScrollInterface.scrollSetup(player, Components.MESSAGESCROLL_220, FR_AERECK_LETTER_CONTENT)
        }
