...
class WiseOldManTaskDialogue : DialogueFile() {
    ...
    override fun handle(componentID: Int, buttonID: Int) {
        val item = ItemDelivery.values().map { it.item }.toIntArray().random()
        val testAmount = RandomFunction.random(1,15)
        npc = NPC(NPCs.WISE_OLD_MAN_2253)
        when (stage) {
            0 -> if(getAttribute(player!!, WomtUtils.TASK_START, false)) {
                player("What did you ask me to do?").also { stage++ }
                setAttribute(player!!, "/save:${WomtUtils.TASK_START}", true)
            } else {
                player("Is there anything I can do for you?").also { stage++ }
            }
            1 -> npcl(FacialExpression.HALF_GUILTY, "I need " + " $testAmount " + ItemDelivery.forId(item).toString().lowercase().replace("_", " ") + ".").also { setAttribute(player!!, WomtUtils.TASK_START, true); stage++ }
