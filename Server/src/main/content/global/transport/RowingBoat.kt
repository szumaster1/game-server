package content.global.transport
                ...
                when (counter++) {
                    0 -> {
                        if (npc.id == NPCs.KATHY_CORKAT_3831) {
                            ...
                            sendPlainDialogue(player, true, "", "", "Kathy Corkat rows you down the river to the sea...", "")
                        }
                        openInterface(player, Components.FADE_TO_BLACK_120)
                    }

                    4 -> {
                        teleport(player, if (npc.id == NPCs.KATHY_CORKAT_3831) Location(2369, 3484, 0) else Location(2357, 3641, 0), TeleportManager.TeleportType.INSTANT)
