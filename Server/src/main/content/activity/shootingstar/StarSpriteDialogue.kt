package content.activity.shootingstar
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            50 -> {
                ...
                val dust = if (amountInInventory(player, ShootingStarPlugin.STAR_DUST) > 200) 200 else amountInInventory(player, ShootingStarPlugin.STAR_DUST)
                if (removeItem(player, Item(ShootingStarPlugin.STAR_DUST, dust))) {
                    ...
                    val coins =
                        (Math.ceil(250.0 * dust) * AMPLIFIER).toInt() // limits the amount of gp per turn-in to 90k, same as 2009 rs, and minimum from all stardust from a level 1 star works out to 50k, same as rs in 2009
