package content.dialogue.sophanem
...
class SphinxDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            1 -> options("Yes, but I don't bring it to harsh places like this.", "No, you are mistaken.", "Yes, but I have left mine in the bank.").also { stage++ }
            2 -> when (buttonId) {
            ...
            3 -> npc(FacialExpression.SAD, "A pity, they can be of great help in", "some adventures. I would like to talk to", "your cat. Would you bring it to me?").also { stage++ }
            4 -> player("I might, but I have a few things to sort out first.").also { stage = END_DIALOGUE }
            5 -> npc(FacialExpression.SUSPICIOUS, "Really? I'm generally quite good at knowing", "these things.").also { stage = END_DIALOGUE }
            6 -> npc(FacialExpression.AFRAID, "What?  That's no place for a cat!").also { stage = END_DIALOGUE }
