...
class CapeMerchantDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            2 -> npc("Ahh well they make it less likely that you'll accidently", "attack anyone wearing the same cape as you and easier", "to attack everyone else. They also make it easier to", "distinguish people who're wearing the same cape as you").also { stage++ }
            3 -> npc("from everyone else. They're very useful when out in", "the wilderness with friends or anyone else you don't", "want to harm.").also { stage = END_DIALOGUE }
            4 -> {
                end()
