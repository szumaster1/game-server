...
class YohnusDialogue(player: Player? = null): Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            0 -> npc("Sorry but the blacksmiths is closed. But I can let you", "use the furnace at the cost of 20 gold pieces.").also { stage++ }
            ...
            2 -> when (buttonId) {
                1 -> {
                    if (!removeItem(player, Item(Items.COINS_995, 20))) {
                        ...
                        npc(FacialExpression.FRIENDLY, "Thanks Bwana! Enjoy the facilities!").also { stage = END_DIALOGUE }
