...
class FaladorSquireDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        if ((stage == -1 && buttonId == 2) || AchievementDiary.canReplaceReward(player, DiaryType.FALADOR, 2)) {
            openDialogue(player, FaladorSquireDiaryDialogue(), npc)
        } else {
            when (getQuestStage(player, "The Knight's Sword")) {
                100 -> when (stage) {
                ...
                60 -> when (stage) {
                   -1 -> npc("So how are you doing getting a sword?").also { stage = 0 }
                    ...
                    2 -> npc("Thank you, thank you, thank you! I was seriously", "worried I would have to own up to Sir Vyvin!").also { stage++ }
                    3 -> sendDialogue(player!!, "You give the sword to the squire.").also { stage++ }
                    4 -> {
                        if (removeItem(player, Item(Items.BLURITE_SWORD_667))) {
                            finishQuest(player, "The Knight's Sword")
                            end()
                        }
                    }
                }
                50 -> when (stage) {
                   -1 -> npc("So how are you doing getting a sword?").also { stage = 0 }
                    ...
                    1 -> npc("Please try and get it quickly... I am scared Sir Vyvin", "will find out!").also { stage = END_DIALOGUE }
                ...
                40 -> when (stage) {
                   -1 -> npc("So how are you doing getting a sword?").also { stage = 0 }
                    ...
                    4 -> {
                        setQuestStage(player, "The Knight's Sword", 50)
                        end()
                    }
                }
                30 -> when (stage) {
                ...
                0 -> when (stage) {
                   -1 -> npc("Hello. I am the squire to Sir Vyvin.").also { stage = 0 }
                    ...
                    121 -> npc("The thing is, this sword is a family heirloom. It has been", "passed down through Vyvin's family for five", "generations! It was originally made by the Imcando", "dwarves, who were").also { stage++ }
                    ...
                    161 -> npc("I don't suppose you could try and track down the", "Imcando dwarves for me? I've got so much work to", "do...").also { stage++ }
                    162 -> options("Ok, I'll give it a go.", "No, I've got lots of mining work to do.").also { stage++ }
                    163 -> when (buttonId) {
                    ...
                    165 -> {
                        ...
                        setQuestStage(player, "The Knight's Sword", 10)
                        player.getQuestRepository().syncronizeTab(player)
                        end()
                    }
                    170 -> npc("Yes, me too. I'm not looking forward to telling Vyvin", "I've lost it. He's going to want it for the parade next", "week as well.").also { stage = END_DIALOGUE }
                    130 -> npc("He doesn't know yet. I was hoping I could think of", "something to do before he does find out, But I find", "myself at a loss.").also { stage = END_DIALOGUE }
                }
            }
        }
        return true
