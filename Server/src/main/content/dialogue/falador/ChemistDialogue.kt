...
import content.dialogue.ChemistDiaryDialogue
import core.api.*
...
class ChemistDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun open(vararg args: Any): Boolean {
        setTitle(player, 4)
        if (!isQuestComplete(player, "Biohazard") && isQuestInProgress(player, "Biohazard", 10, 99)) {
            ...
            sendDialogueOptions(player, "Do you want to talk about lamps?", "Yes.", "No.", "Impling jars.", "Falador Achievement Diary.").also { stage = 0 }
        }
        return true
    }
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            12 -> npc("It's really quite simple. You use the small still in here.", "It's all set up, so there's no fiddling around with dials...").also { stage++ }
            13 -> npc("Just put ordinary swamp tar in, and then use a lantern", "or lamp to get the oil out.").also { stage++ }
            ...
            33 -> player("Apparently, if I can make a jar like this it will be useful", "for storing implings in.").also { stage++ }
            34 -> npc("My lamp oil still may be able to do what you want. Use the", "oil and flower mix on the still.").also { stage++ }
            35 -> npc("Once that's done. Use one of those butterfly jars to", "collect the distillate.").also { stage++ }
            36 -> player("Thanks!").also { stage++ }
            37 -> options("So how do you make anchovy oil?", "Do you have a sieve I can use?", "I'd better go and get the repellent.").also { stage++ }
            38 -> when (buttonId) {
