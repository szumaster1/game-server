...
class BlurberryDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun open(vararg args: Any?): Boolean {
        ...
        if (tutorialProgress == 18) {
            npc(FacialExpression.OLD_NORMAL, "Yes, you, Aluft said you would be coming.").also { stage = 0 }
            return true
        }

        if (tutorialComplete) {
            npc(FacialExpression.OLD_NORMAL, "I do hope you're enjoying your work!").also { stage = END_DIALOGUE }
            return true
        }

        npc(FacialExpression.OLD_NORMAL, "Hello, have you made that drink for me?")
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            1 -> npc(FacialExpression.OLD_NORMAL, "Well, I'd like to show you how to make", "a drink.").also { stage++ }
            2 -> player("Woah, sounds like fun!").also { stage++ }
            3 -> npc(FacialExpression.OLD_NORMAL, "Alright, then, let's get to it!").also { stage++ }
            4 -> npc(FacialExpression.OLD_NORMAL, "I want you to make me a fruit blast. Simple drink!").also { stage++ }
            5 -> npc(FacialExpression.OLD_NORMAL, "Here's everything you need.").also { stage++ }
            6 -> {
                end()
            ...
            20 -> if (inInventory(player, Items.FRUIT_BLAST_9514)) {
            ...
            22 -> npc(FacialExpression.OLD_NORMAL, "Excellent, I think you're ready to go on the job!").also { stage++ }
            23 -> {
                end()
                npc(FacialExpression.OLD_NORMAL, "Go back and speak with Aluft now.")
