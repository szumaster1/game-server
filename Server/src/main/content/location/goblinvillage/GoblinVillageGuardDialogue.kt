package content.location.goblinvillage
...
class GoblinVillageGuardDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            0 -> npc("I know. We guards usually stay by banks and shops,", "but I got sent all the way out here to keep an eye on", "the brigands loitering just south of here.").also { stage++ }
            1 -> player("Sounds more exciting than standing", "around guarding banks and shops.").also { stage++ }
            2 -> npc("It's not too bad. At least I don't get attacked so often", "out here. Guards in the cities get killed all the time.").also { stage++ }
