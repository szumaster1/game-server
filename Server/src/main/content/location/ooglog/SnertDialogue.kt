...
class SnertDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        if (isQuestComplete(player, "As a First Resort")) {
            when (stage) {
                0 -> playerl(FacialExpression.FRIENDLY, "Hi, ogre.").also { stage++ }
                ...
                2 -> when (buttonId) {
                    ...
                    2 -> playerl(FacialExpression.HALF_ASKING, "Can you tell me about this copper-coloured pool?").also { stage = 7 }
                }
                3 -> npcl(FacialExpression.CHILD_NORMAL, "Me like pie!").also { stage++ }
                4 -> playerl(FacialExpression.FRIENDLY, "Doesn't everyone?").also { stage++ }
                5 -> npcl(FacialExpression.CHILD_NORMAL, "Pie! Pie! Pie! Pie! Pie!").also { stage++ }
                6 -> playerl(FacialExpression.FRIENDLY, "Yes, dear, pie.").also { stage = END_DIALOGUE }
                7 -> npcl(FacialExpression.CHILD_NORMAL, "Yeah, it red!").also { stage++ }
                8 -> playerl(FacialExpression.FRIENDLY, "Yes, I can see that.").also { stage++ }
                9 -> npcl(FacialExpression.CHILD_NORMAL, "Then why you ask, silly human?").also { stage++ }
                10 -> playerl(FacialExpression.FRIENDLY, "Well, it's hard to explain. I just sense a strange energy coming from this pool. Do you know anything about it?").also { stage++ }
                11 -> npcl(FacialExpression.CHILD_NORMAL, "Oh, well, it called de 'Bandos pool' round here. It s'pposed to be special-special. It hard to explain to human creature like you. Maybe you need talk to auntie Seegud. She explain good about these things").also { stage++ }
                12 -> end()
            }
        } else {
