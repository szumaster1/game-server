...
class TarikDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            37 -> sendDialogue(player, "The first room in the pyramid requires a thieving level of 21. Each subsequent room requires an extra 10 levels to enter.").also { stage++ }
            ...
            47 -> playerl(FacialExpression.HALF_ASKING, "Tumeken?").also {
                setAttribute(player, "/save:tarik-spoken-to", true); stage++
            }
