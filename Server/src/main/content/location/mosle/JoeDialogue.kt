package content.location.mosle
...
class JoeDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            4 -> options("Whadda ya got?", "Nothing! Honest!", "I wanted to know where you got that harpoon hand.").also { stage++ }
            5 -> when (buttonId) {
            ...
            14 -> npcl(FacialExpression.FRIENDLY, "If ye don't wants to hear the story then I ain't gonna tell ye!").also { stage = END_DIALOGUE }
            16 -> {
                end()
