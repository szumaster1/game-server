        ...
        when (stage) {
            0 -> {
                if(getAttribute(player!!, "draynor:feed-tree-guard", false)) {
                    ...
                    npcl(FacialExpression.AFRAID,"Ssshhh! What do you want?").also { stage++ }
                } else {
            ...
            9 -> npcl("I'd be doing a lot better if nits like you didn't come crowding around me all day.").also { stage++ }
            10 -> playerl(FacialExpression.HALF_GUILTY, "But your legs are hanging down!").also { stage++ }
            11 -> npcl(FacialExpression.ANNOYED, "Go away!").also { stage++ }
            12 -> playerl("Please yourself!").also { stage = END_DIALOGUE }

            13 -> npcl("That's very kind of you. I'd rather like a nice bowl of stew, if you could fetch me one. I don't get many meal breaks.").also { stage++ }
            14 -> playerl(FacialExpression.HALF_ASKING, "You want a bowl of stew?").also { stage++ }
            15 -> npcl(FacialExpression.ROLLING_EYES,"If you wouldn't mind...").also { stage++ }
            16 -> playerl(FacialExpression.FRIENDLY, "I'll think about it!").also { stage = END_DIALOGUE }
            ...
            19 -> {
                sendNPCDialogue(player!!, NPCs.GUARD_345, "Gosh, that's very kind of you! Here, have a few coins for your trouble.", FacialExpression.HAPPY)
                end()
                removeItem(player!!, Items.STEW_2003)
                addItemOrDrop(player!!, Items.COINS_995, 30)
                setAttribute(player!!, "draynor:feed-tree-guard", true)
                submitWorldPulse(object : Pulse(80) {
                    var counter = 0
                    override fun pulse(): Boolean {
                        when (counter++) {
                            0 -> {
                                produceGroundItem(player!!, Items.BOWL_1923, 1, Location.create(3086, 3245, 0))
                                removeAttribute(player!!, "draynor:feed-tree-guard")
                                return true
                            }
                        }
                        return false
            ...
            20 -> npcl("Fair enough. But if you change your mind, I'll probably still be here.").also { stage = END_DIALOGUE }
        }
    }
}
