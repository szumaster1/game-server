...
class ProfessorOddensteinDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun open(vararg args: Any): Boolean {
        ...
        when (quest.getStage(player)) {
            ...
            20 -> {
                npc(FacialExpression.HALF_GUILTY, "Have you found everything yet?")
                stage = 0
            }
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        ...
        when (quest.getStage(player)) {
            0, 100 -> when (stage) {
                ...
                1 -> when (buttonId) {
                    1 -> {
                        player(FacialExpression.HALF_GUILTY, "What does this machine do?")
                        stage = 11
                    }

                    2 -> {
                        player(FacialExpression.HALF_GUILTY, "Is this your house?")
                        stage = 20
                    }
                }
            ...
            10 -> when (stage) {
                ...
                3 -> {
                    player(FacialExpression.HALF_GUILTY, "So you know where he is then?")
                    stage = 4
                }

                4 -> {
                    npc(FacialExpression.HALF_GUILTY, "He's that chicken over there.")
                    stage = 5
                }
                ...
                9 -> {
                    npc(FacialExpression.HALF_GUILTY, "Who's Veronica?")
                    stage = 10
                }
                ...
                13 -> {
                    npc(FacialExpression.HALF_GUILTY, "Umm... It's not so easy...")
                    stage = 14
                }
                ...
                15 -> {
                    player(FacialExpression.HALF_GUILTY, "Well I can look for them.")
                    stage = 16
                }
            ...
            20 -> when (stage) {
                0 -> stage = if (player.inventory.containsItem(OIL_CAN) && player.inventory.containsItem(
                        PRESSURE_GAUGE
                ...
                2 -> end()
                3 -> {
                    npc(FacialExpression.HALF_GUILTY, "Give 'em here then.")
                    stage = 4
                }
                    ...
                    Pulser.submit(object : Pulse(1, player) {
                        ...
                        override fun pulse(): Boolean {
                            when (counter++) {
                                ...
                                4 -> {
                                    val ernest = NPC.create(287, chicken!!.location)
                                    ...
                                    if (player.inventory.remove(OIL_CAN, PRESSURE_GAUGE, RUBBER_TUBE)) {
                                        player.dialogueInterpreter.open(287, ernest)
                                    }
                                }
