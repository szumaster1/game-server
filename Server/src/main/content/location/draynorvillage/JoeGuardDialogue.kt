...
class JoeGuardDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun open(vararg args: Any): Boolean {
        ...
        when (quest!!.getStage(player)) {
            ...
            else -> npc(FacialExpression.HALF_GUILTY, "Hi, I'm Joe, door guard for Lady Keli.")
        }
        return true
    }
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        if (quest!!.getStage(player) > 50) {
            when (stage) {
                ...
                1 -> npc("The Prince got away, I am in trouble. I better not talk", "to you, they are not sure I was drunk.").also { stage = 2 }
                2 -> end()
            }
            return true
        }
        if (stage >= 10 && quest!!.getStage(player) == 40) {
            when (stage) {
                ...
                13 -> if (!removeItem(player, Item(Items.BEER_1917))) {
                    end()
                } else {
        ...
        when (stage) {
            0 -> player(FacialExpression.HALF_GUILTY, "Hi, who are you guarding here?").also { stage++ }
            1 -> npc(FacialExpression.HALF_GUILTY, "Can't say, all very secret. You should get out of here.", "I am not suposed to talk while I guard.").also { stage++ }
            2 -> end()
        }
        return true
    }
