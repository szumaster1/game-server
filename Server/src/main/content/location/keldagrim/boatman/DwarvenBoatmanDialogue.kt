package content.location.keldagrim.boatman
...
/**
 * Represents the Dwarven boatman forth dialogue.
 */
@Initializable
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        if (!getAttribute(player, "/save:keldagrim-visited", false)) {
            when (stage) {
                ...
                7 -> showTopics(Topic(FacialExpression.FRIENDLY, "That's a deal!", 8), Topic(FacialExpression.FRIENDLY, "I prefer the mines to the city.", 15))
                ...
                14 -> npcl(FacialExpression.OLD_HAPPY, "Sure, sure! Come back when you need to!").also { stage = END_DIALOGUE }
                15 -> npcl(FacialExpression.OLD_HAPPY, "Well, suit yourself then!").also { stage = END_DIALOGUE }
            }
        } else {
            when (stage) {
                START_DIALOGUE -> npcl(FacialExpression.OLD_HAPPY, "Hello again, ${player.name}! Want to go back to Keldagrim?").also { stage++ }
                1 -> showTopics(Topic(FacialExpression.FRIENDLY, "Yes, please take me.", 2), Topic(FacialExpression.FRIENDLY, "What, on your ship? No way.", 3))
                2 -> {
                    end()
                    submitWorldPulse(TravelForthPulse(player))
                }
                3 -> npcl(FacialExpression.OLD_NORMAL, "Hey, it was only a little accident! Calm down!").also { stage = END_DIALOGUE }
            }
        }
        return true
    }
...
/**
 * Represents the Travel forth pulse.
 */
