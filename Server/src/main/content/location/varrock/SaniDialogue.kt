...
class SaniDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            8 -> player("I see...", "So this means that you only sell common armours", "to help adventurers like myself get started in the world", "of ${GameWorld.settings!!.name}.").also { stage++ }
            9 -> npc("Indeed, that is true. If you have any more questions", "The ${GameWorld.settings!!.name} guide near the green portal should be able", "to assist you further.").also { stage = 0 }
            10 -> npc("Take the box set to the Grand Exchange clerk. Right", "click her and select 'sets'. Next, right-click the box", "item that is in your inventory to exchange the set for", "the items that are inside the box.").also { stage++ }
            11 -> npc("You may always convert the individual items back into", "a box set at any time in order to save banking", "space.").also { stage = 0 }
