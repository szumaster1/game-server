...
class BaraekDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun open(vararg args: Any): Boolean {
        npc = args[0] as NPC
        showTopics(
            IfTopic("Can you tell me where I can find the Phoenix Gang?", 0, getQuestStage(player, "Shield of Arrav") in 30..40),
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            4 -> npc("Of course, if I was, say 20 gold coins richer I may", "happen to be more inclined to take that sort of risk...").also { stage++ }
            5 -> options("Okay. Have 20 gold coins.", "No, I don't like things like bribery.", "Yes. I'd like to be 20 gold coins richer too.").also { stage++ }
            6 -> when (buttonId) {
            ...
            7 -> if (!removeItem(player, Item(Items.COINS_995, 20))) {
                player("I don't have enough coins.").also { stage = END_DIALOGUE }
            } else {
                setQuestStage(player, "Shield of Arrav", 40)
                npc("Ok, to get to the gang hideout, enter Varrock through", "the south gate. Then, if you take the first turning east", "somewhere along there is an alleway to the south. The", "door at the end of there is an entrance to the Phoenix").also { stage++ }
            }
            8 -> npc("Gang. they're operating there under the name of", "VTAM Corporation. Be careful. The Phoenixes ain't", "the type to be messed about.").also { stage++ }
            9 -> player("Thanks!").also { stage = END_DIALOGUE }
            10 -> npc("Heh. If you wanna deal with the Phoenix Gand they're", "involved in much worse than a bit of briber.").also { stage = END_DIALOGUE }
            ...
            14 -> npc("Well, I can't go any cheaper than that mate. I have a", "family to feed.").also { stage = END_DIALOGUE }
            15 -> {
            ...
            26 -> if (!inEquipment(player, Items.RING_OF_CHAROSA_6465)) {
                npc("It's not in the best condition. I guess I could give you", "120 coins for it.").also { stage++ }
            } else {
                npc("It's not in the best condition. I guess I could give you", "150 coins for it.").also { stage++ }
            }
