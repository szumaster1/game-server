...
class HeadChefDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            2 -> when (buttonId) {
                1 -> {
                    player(FacialExpression.HAPPY, "Nice cape, you're wearing!")
                    stage = 10
                }

                2 -> {
                    player(FacialExpression.FRIENDLY, "Thanks, bye.")
                    stage = 3
                }
            }
            ...
            152 -> when (buttonId) {
                1 -> {
                    player(FacialExpression.EXTREMELY_SHOCKED, "That's much too expensive.")
                    stage = 160
                }

                2 -> {
                    player(FacialExpression.HAPPY, "Sure.")
                    stage = 200
                }
            }
