...
class RedbeardFrankDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            102 -> {
                player(FacialExpression.AMAZED, "Plundering booty?")
                stage = 103
            }
            ...
            200 -> if (AchievementDiary.canReplaceReward(player, DiaryType.FALADOR, level)) {
            ...
            208 -> {
                player(FacialExpression.AMAZED, "Wow, thanks!")
                stage = 209
            }
