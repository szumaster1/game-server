...
class MonkOfEntranaDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun open(vararg args: Any): Boolean {
        ...
        if (npc.id in intArrayOf(NPCs.MONK_OF_ENTRANA_2730,NPCs.MONK_OF_ENTRANA_658,NPCs.MONK_OF_ENTRANA_2731)) {
            npc(FacialExpression.HALF_ASKING,"Do you wish to leave holy Entrana?").also { stage = 25 }
            return true
        }
        npc(FacialExpression.HALF_GUILTY, "Do you seek passage to holy Entrana? If so, you must", "leave your weaponry and armour behind. This is", "Saradomin's will.").also { stage = 0 }
        return true
    }
        ...
        when (stage) {
            ...
            22 -> {
                if (!ItemDefinition.canEnterEntrana(player)) {
                    npc(FacialExpression.ANNOYED, "NO WEAPONS OR ARMOUR are permitted on holy", "Entrana AT ALL. We will not allow you to travel there", "in breach of mighty Saradomin's edict.").also { stage = 23 }
                    return true
                }
