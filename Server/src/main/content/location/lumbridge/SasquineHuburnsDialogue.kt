...
class SasquineHuburnsDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            9 -> player("Don't touch what? What's going to happen to me if I", "touch it? Are these things safe?").also { stage++ }
            10 -> npc(FacialExpression.OLD_LAUGH1, "Hee hee! People fall for that every time.").also { stage++ }
            11 -> player("I bet I could pick you and hammer your chin into", "the ground if I tried.").also { stage++ }
            12 -> npc(FacialExpression.OLD_HAPPY, "Oh you humans have no sense of humour.", "Anyway, where would you like to go first?").also { stage++ }
            ...
            14 -> when (buttonId) {
                ...
                2 -> npc(FacialExpression.OLD_HAPPY, "Take this ticket. It'll tell the gnomecopter to take you to", "Castle Wars. Just sit on a gnomecopter and off you", "go!").also { stage = 16 }
            ...
            15 -> npc(FacialExpression.OLD_NORMAL, "That's the spirit!", "Where would you like to go this time?").also { stage = 13 }

            16 -> {
                end()
                sendItemDialogue(player, Items.GNOMECOPTER_TICKET_12843, "Sasquine Huburns gives you a gnomecopter ticket.")
