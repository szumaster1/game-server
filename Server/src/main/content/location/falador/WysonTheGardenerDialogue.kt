...
class WysonTheGardenerDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun open(vararg args: Any): Boolean {
        ...
        if (birdNest) {
            npc("If I'm not mistaken, you've got some mole bits there!", "I'll trade 'em for bird nest if ye likes.").also { stage = 100 }
            return true
        } else {
            npc("I'm the head gardener around here.", "If you're looking for woad leaves, or if you need help", "with owt, I'm yer man.").also { stage = 0 }
        }
        return true
    }
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            11 -> options("How about 5 coins?", "How about 10 coins?", "How about 15 coins?", "How about 20 coins?").also { stage++ }
            ...
            110 -> npc("No no, that's far too little. Woad leaves are hard to get. I", "used to have plenty but someone kept stealing them off", "me.").also { stage = END_DIALOGUE }
            ...
            100 -> options("Yes, I will trade the mole claws.", "Okay, I will trade the mole skin.", "I'd like to trade both.", "No, thanks.").also { stage = 900 }
    ...
    private fun addRewards() {
        ...
        for (i in 0 until nestAmount) {
            if (!player.inventory.add(Item(BirdNestDropTable.getRandomNest(true)!!.nest.id, 1), player)) {
                GroundItemManager.create(Item(BirdNestDropTable.getRandomNest(true)!!.nest.id, 1), player.location, player)
            }
        }
    }
