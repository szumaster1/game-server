...
class ElunedDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        ...
        when (stage) {
            500 -> end()
            1000 -> {
                if (quest.getStage(player) == 15 && !player.inventory.contains(
                        RovingElves.CONSECRATION_SEED_CHARGED.id,
                        1
                    )
                ) {
            ...
            1001 -> stage = if (quest.getStage(player) >= 100 && player.inventory.contains(
                    Items.TINY_ELF_CRYSTAL_6103,
                    1
                )
            ) {
                player(FacialExpression.HALF_GUILTY,
                    "I am looking to recharge teleportation crystals."
                )
                1200
            } else {
            ...
            1203 -> when (buttonId) {
                1 -> {
                    ...
                    if (!player.inventory.contains(995, price)) {
                        ...
                        interpreter.sendDialogue(
                            String.format(
                                "Eluned recharges your elven teleportation crystal for %d gold.",
                                price
                            )
                        )
                        if (player.inventory.remove(Item(Items.TINY_ELF_CRYSTAL_6103)) && player.inventory.remove(
                                Item(
                                    995,
                                    price
                                )
                ...
                2 -> {
                    player(FacialExpression.HALF_GUILTY,
                        "Nevermind, I really must be going."
                    )
                    stage = 1204
                }
            }
            ...
            4 -> {
                player(FacialExpression.HALF_GUILTY,
                    "OK... I'll be back as soon as I have it."
                )
                stage = 5
            }
            ...
            8 -> {
                player(FacialExpression.HALF_GUILTY,
                    "How odd. I can see her lips moving... But there's no",
                    "sound."
                )
                stage = 9
            }
            ...
            13 -> {
                interpreter.sendDialogues(
                    1679,
                    FacialExpression.HALF_GUILTY,
                    "Well done... You should go see Islwyn, but I'd guess he",
                    "already knows."
                )
                stage = 500
            }
