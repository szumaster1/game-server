...
public final class TzhaarFightCavesPlugin extends ActivityPlugin {
    ...
    public boolean start(final Player player, boolean login, Object... args) {
        ...
        if (!login && !practice) {
            ...
            pulse = new Pulse(1, player) {
                ...
                public boolean pulse() {
                    if (count++ == 0) {
                        ...
                        player.getDialogueInterpreter().sendDialogues(2617, FacialExpression.CHILD_GUILTY, "You're on your own now, JalYt.", "Prepare to fight for your life!");
                    } else if (count == 16) {
                        spawnWave(wave);
                        return true;
                    }
            ...
            if (player.getAttribute("fc_wave", 0) == 62) {
                player.getDialogueInterpreter().sendDialogues(2617, FacialExpression.CHILD_GUILTY, "Look out, here comes TzTok-Jad!");
            }
    ...
    public void leave(Player player, int wave) {
        ...
        if (wave == 63) {
            ...
            if (!practice) {
                ...
                player.getDialogueInterpreter().sendDialogues(2617, FacialExpression.CHILD_GUILTY, "You even defeated TzTok-Jad, I am most impressed!", "Please accept this gift as a reward.");
                sendNews(player.getUsername() + " has been victorious in defeating TzTok-Jad for a firecape!");
            } else {
                player.getDialogueInterpreter().sendDialogues(2617, FacialExpression.CHILD_GUILTY, "You defeated TzTok-Jad. I am most impressed!", "Here is you TokKul back.", "Maybe next time you do all training, and get real reward...");
            }
        } else if (wave <= 1) {
            player.getDialogueInterpreter().sendDialogues(2617, FacialExpression.CHILD_GUILTY, "Well I suppose you tried... better luck next time.");
        } else {
            if (!practice) {
                player.getDialogueInterpreter().sendDialogues(2617, FacialExpression.CHILD_GUILTY, "Well done in the cave, here, take TokKul as reward.");
            } else {
                player.getDialogueInterpreter().sendDialogues(2617, FacialExpression.CHILD_GUILTY, "You both impatient and also failure.", "Better luck next time.");
            }
        }
    ...
    public boolean interact(Entity e, Node target, Option option) {
        if (target instanceof Scenery && target.getId() == 9357) {
            leave((Player) e, e.getAttribute("fc_wave", 0));
            removeAttribute(player, "fc_wave");
            removeAttribute(player, "fc_offset");
            removeAttribute(player, "fc_practice_jad");
