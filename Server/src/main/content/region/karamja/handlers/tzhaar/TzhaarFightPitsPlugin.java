...
public final class TzhaarFightPitsPlugin extends ActivityPlugin {
    ...
    /**
     * Instantiates a new Tzhaar fight pits plugin.
     */
        ...
        if (WAR_PLAYERS.remove(e)) {
            if (WAR_PLAYERS.size() < 2) {
                if (!WAR_PLAYERS.isEmpty()) {
                    ...
                    RegionManager.forId(9552).getPlanes()[0].getNpcs().get(0).setAttribute("fp_champion", getChampionName());
                }
                minutes = 0;
            }
