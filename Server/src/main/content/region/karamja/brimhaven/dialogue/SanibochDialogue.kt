...
class SanibochDialogue(player: Player? = null): Dialogue(player) {
    ...
    /*
     * Saniboch is a tribesman responsible for guarding the entrance to Brimhaven Dungeon.
     * Location: 2744,3152
     */
    ...
    override fun open(vararg args: Any): Boolean {
        ...
        if (stage == 0) {
            sendNPCDialogue(player, NPCs.SANIBOCH_1595, "Good day to you bwana.", FacialExpression.HALF_GUILTY)
        } else {
            handle(0, 0)
        }
        return true
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            20 -> npc("To a huge fearsome dungeon, populated by giants and", "strange dogs. Adventurers come from all around to", "explore its depths.").also { stage = 21 }
            21 -> npc("I know not what lies deeper in myself, for my skills in", "agility and woodcutting are inadequate.").also { stage = END_DIALOGUE }
            30 -> npc("It leads to a huge fearsome dungeon, populated by", "giants and strange dogs. Adventurers come from all", "around to explore its depths.").also { stage++ }
            31 -> npc("I know not what lies deeper in myself, for my skills in", "agility and woodcutting are inadequate, but I hear tell", "of even greater dangers deeper in.").also { stage = END_DIALOGUE }
            32 -> if (removeItem(player, Item(Items.COINS_995, 875))) {
            ...
            33 -> npc("Many thanks. You may now pass the door. May your", "death be a glorious one!").also { stage = END_DIALOGUE }
            35 -> {
                if (getAttribute(player, "saniboch:paid", false)) {
                    end()
            ...
            36 -> npc("Well this is a dungeon for the more wealthy discerning", "adventurer, be gone with your riff raff.").also { stage++ }
            37 -> player("But you don't even have clothes, how can you seriously", "call anyone riff raff.").also { stage++ }
            38 -> npc("Hummph.").also { stage = END_DIALOGUE }
            40 -> npc("My employer tells me it is an uncommon sort of tree", "called the Fyburglars tree.").also { stage = END_DIALOGUE }
        }
        return true
    }
