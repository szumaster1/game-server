...
class RevenantCombatHandler(meleeAnimation: Animation?, magicAnimation: Animation?, rangeAnimation: Animation?) :
    MultiSwingHandler(
        ...
        SwitchAttack(CombatStyle.RANGE.swingHandler, rangeAnimation, createProjectile(RANGE_GRAPHIC)),
        SwitchAttack(CombatStyle.MAGIC.swingHandler, magicAnimation, createProjectile(MAGIC_GRAPHIC))
    ) {
    ...
    override fun visualize(entity: Entity, victim: Entity?, state: BattleState?) {
        ...
        if (victim!!.isPlayer) {
            ...
            if (attack.style == CombatStyle.MAGIC) {
                playGlobalAudio(entity.location, Sounds.TELEPORTBLOCK_CAST_202, 0, 1, 10)
