...
class BorkNPC : AbstractNPC {
    ...
    override fun commenceDeath(killer: Entity) {
        ...
        Pulser.submit(object : Pulse(10, player) {
            override fun pulse(): Boolean {
                ...
                PacketRepository.send(
                    ...
                    CameraContext(player!!, CameraType.SHAKE, 3, 2, 2, 2, 2)
                )
    ...
    override fun handleDrops(p: Player, killer: Entity) {
        ...
        val drops = if (Item(player!!.equipment.getId(12)).name.lowercase(Locale.getDefault())
    ...
    private fun spawnLegion() {
        ...
        Pulser.submit(object : Pulse(1, player, this) {
            override fun pulse(): Boolean {
                animator.forceAnimation(Animation.create(8757))
                ...
                Pulser.submit(object : Pulse(13, player) {
                    override fun pulse(): Boolean {
                        player!!.interfaceManager.close()
                        for (i in 0..2) {
                            val legion = create(
                                7135,
                                getLocation().transform(RandomFunction.random(1, 3), RandomFunction.random(1, 3), 0),
                                player
                            )
                            legion.init()
    ...
    inner class DagonElite : AbstractNPC {
        ...
        override fun isAttackable(e: Entity, style: CombatStyle, message: Boolean): Boolean {
            return false
        }
    ...
    inner class BorkCutscene() : CutscenePlugin("Bork cutscene") {
        ...
        override fun leave(entity: Entity, logout: Boolean): Boolean {
            if (entity is Player) {
                PacketRepository.send(CameraViewPacket::class.java, CameraContext(entity.asPlayer(), CameraType.RESET, 3, 2, 2, 2, 2))
            }
        ...
        override fun configure() {
            region = DynamicRegion.create(Regions.BORK_REGION_12374)
