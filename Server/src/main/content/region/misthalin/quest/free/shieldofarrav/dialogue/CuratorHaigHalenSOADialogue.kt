...
class CuratorHaigHalenSOADialogue : DialogueFile() {
    ...
    override fun handle(componentID: Int, buttonID: Int) {
        ...
        if (player.inventory.containsItem(ShieldofArrav.PHOENIX_SHIELD) || player.inventory.containsItem(ShieldofArrav.BLACKARM_SHIELD)) {
            when (stage) {
                ...
                4 -> sendItemDialogue(player, ShieldofArrav.getShield(player), "The curator peers at the shield.").also { stage++ }
                5 -> npc("This is incredible!").also { stage++ }
                6 -> npc("That shield has been missing for over twenty-five years!").also { stage++ }
                7 -> npc("Leave the shield here with me and I'll write you out a", "certificate saying that you have returned the shield, so", "that you can claim your reward from the King.").also { stage++ }
        ...
        when (stage) {
            0, 12 -> npc("Of course you won't actually be able to claim the", "reward with only half the reward certificate...").also { stage++ }
            13 -> player("What? I went through a lot of trouble to get that shield", "piece and now you tell me it was for nothing? That's", "not very fair!").also { stage++ }
            14 -> npc("Well, if you were to get me the other half of the shield,", "I could give you the other half of the reward certificate.", "It's rumoured to be in the possession of the infamous", "Blackarm Gang, beyond that I can't help you.").also { stage++ }
