...
@Initializable
class LumbridgeCookCADialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun open(vararg args: Any?): Boolean {
        ...
        if (player?.questRepository?.getQuest("Cook's Assistant")!!
                .getStage(player) <= 0
        ) { //If the player has ot started cook's assistant
            npc(FacialExpression.SAD, "What am I to do?")
            stage = 0
            return true
        } else if (player?.questRepository?.getQuest("Cook's Assistant")!!
                .getStage(player) in 10..99
        ) { //During the Cook's Assistant Quest
            ...
            ) { //If the player has handed all the ingredients to the chef but did not continue the dialogue
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        if (player.questRepository.getQuest("Lost Tribe").getStage(player) == 10) {
            when (stage) {
                ...
                4 -> npc(
                    ...
                    "this mystery."
                ).also { stage = 1000; player.questRepository.getQuest("Lost Tribe").setStage(player, 20) }
        ...
        when (stage) {
            ...
            40 -> npc(
                FacialExpression.SAD,
                "Er, thank you. It's a pretty ordinary cook's hat, really."
            ).also { stage++ }

            41 -> player(
                FacialExpression.HAPPY,
                "Still, it suits you. The trousers are pretty special too."
            ).also { stage++ }
            ...
            ).also { player?.questRepository?.getQuest("Cook's Assistant")?.start(player!!); stage++ }
            ...
            61 -> when (buttonId) {
                ...
                4 -> player(FacialExpression.NEUTRAL, "Actually, I know where to find this stuff.").also {
                    stage = 1000
                }
            }
            ...
            70 -> npc(
                ...
                "though, " + if (inInventory(
                        player,
                        EMPTY_POT,
                        1
                    )
                ) "you've got one on you already." else "there should be one on the table in here."
            ).also { stage = 80 }
            ...
            81 -> when (buttonId) {
                ...
                4 -> player(FacialExpression.NEUTRAL, "I've got all the information I need. Thanks.").also {
                    stage = 1000
                }
            }

            100 ->
                if (!player.getAttribute("cooks_assistant:milk_submitted", false) && inInventory(player, MILK, 1)) {
                ...
                } else if (!player.getAttribute("cooks_assistant:flour_submitted", false) && inInventory(
                        player,
                        FLOUR,
                        1
                    )
                ...
                } else if (!player.getAttribute("cooks_assistant:egg_submitted", false) && inInventory(
                        player,
                        EGG,
                        1
                    )
                    ...
                    if (gave) {
                        ...
                        if (player.getAttribute(
                            ...
                            npc(
                                ...
                                "Thank you!"
                            ).also { setAttribute(player, "/save:cooks_assistant:all_submitted", true); stage = 200 }
                        } else {
                            npc(
                        ...
                        if (!player.getAttribute("cooks_assistant:submitted_some_items", false)) {
                            player(
                                FacialExpression.NEUTRAL,
                                "I haven't gotten any of them yet, I'm still looking."
                            ).also { stage = 155 }
                        } else {
                            options(
            ...
            156 -> sendDialogue("You still need to get:", "A bucket of milk. A pot of flour. An egg.").also {
                stage = 160
            }
            ...
            //I keep on dying
            320 -> npc(FacialExpression.HAPPY, "Ah, well, at least you keep coming back to life too!").also {
                stage = 1000
            }
