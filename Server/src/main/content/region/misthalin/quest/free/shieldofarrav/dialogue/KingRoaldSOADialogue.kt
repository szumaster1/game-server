...
class KingRoaldSOADialogue : DialogueFile() {
    ...
    override fun handle(componentID: Int, buttonID: Int) {
        ...
        if (player!!.inventory.containsItem(ShieldofArrav.PHOENIX_SHIELD) || player!!.inventory.containsItem(ShieldofArrav.BLACKARM_SHIELD)) {
            ...
            when (stage) {
                ...
                2 -> npc("I'm  afraid that's only half the reward certificate. You'll", "have to get the other half and join them together if you", "want to claim the reward.").also { stage = END_DIALOGUE }
                END_DIALOGUE -> end()
            }
            ...
            when (stage) {
                ...
                5 -> {
                    interpreter!!.sendItemMessage(
                        CERTIFICATE.id,
                        "You hand over a certificate. The king gives you 600 gp."
                    )
