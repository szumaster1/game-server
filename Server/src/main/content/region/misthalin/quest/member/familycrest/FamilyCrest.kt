...
class FamilyCrest : Quest("Family Crest", 59, 58, 1, Vars.VARP_QUEST_FAMILY_CREST_PROGRESS, 0, 1, 11) {
    ...
    override fun drawJournal(player: Player, stage: Int) {
        ...
        if (stage == 0) {
            ...
            line(player, "!!40 Crafting??", line++, getStatLevel(player, Skills.CRAFTING) >= 40)
            line(player, "!!40 Smithing??", line++, getStatLevel(player, Skills.SMITHING) >= 40)
            line(player, "!!40 Mining??", line++, getStatLevel(player, Skills.MINING) >= 40)
            line(player, "!!59 Magic??", line++, getStatLevel(player, Skills.MAGIC) >= 59)
        }
        if (stage >= 10) {
        ...
        if (stage == 100) {
            ...
            line(player, "to give them bonuses in specific skills", line++)
