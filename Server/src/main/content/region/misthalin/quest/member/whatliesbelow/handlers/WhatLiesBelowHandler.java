...
public class WhatLiesBelowHandler extends OptionHandler {
    ...
    public Plugin<Object> newInstance(Object arg) throws Throwable {
        ...
        ActivityManager.register(new WhatLiesBelowCutscene());
        ...
        SceneryDefinition.forId(24281).getHandlers().put("option:search", this);
    ...
    public boolean handle(final Player player, Node node, String option) {
        ...
        switch (option) {
            ...
            case "operate":
                if (quest.getStage(player) > 70) {
                    ...
                    if (quest.getStage(player) == 70 && player.getZoneMonitor().isInZone("What Lies below")) {
                        WhatLiesBelowCutscene cutscene = player.getAttribute("cutscene", null);
                        cutscene.summonZaff();
                        return true;
                    }
            ...
            case "read":
                ...
                break;
            case "search":
                if (node.getId() == 24281 && !inInventory(player, Items.DAGONHAI_HISTORY_11001, 1)) {
                    sendMessage(player, "You search the " + node.getName().toLowerCase() + "...");
                    sendMessage(player, "and find a book named 'Dagon'hai history'");
                    addItemOrDrop(player, Items.DAGONHAI_HISTORY_11001, 1);
                    break;
                } else {
                    sendMessage(player, "You search the " + node.getName().toLowerCase() + " and you find nothing of interest to you.");
                    break;
                }
        }
        return true;
