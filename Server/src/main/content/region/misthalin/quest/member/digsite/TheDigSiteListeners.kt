package content.region.misthalin.quest.member.digsite

import content.global.skill.support.thieving.PickpocketsListeners
import content.region.misthalin.quest.member.digsite.dialogue.PanningGuideCannotPanDialogueFile
import core.api.*
    ...
    override fun defineListeners() {
        ...
        on(intArrayOf(NPCs.DIGSITE_WORKMAN_613, NPCs.DIGSITE_WORKMAN_4564, NPCs.DIGSITE_WORKMAN_4565), NPC, "steal-from") { player, node ->
            ...
            if (getQuestStage(player, "The Dig Site") == 3) {
                player.animator.animate(PickpocketsListeners.PICKPOCKET_ANIM)
                val rollOutcome = PickpocketsListeners.pickpocketRoll(player, 84.0, 240.0, workmanPickpocketingTable)
                if (rollOutcome != null) {
                    queueScript(player, PickpocketsListeners.PICKPOCKET_ANIM.duration, QueueStrength.NORMAL) { stage: Int ->
                        when (stage) {
                            0 -> {
                    ...
                    node.asNpc().animator.animate(PickpocketsListeners.NPC_ANIM)
                ...
                player.animator.animate(PickpocketsListeners.PICKPOCKET_ANIM)
                val rollOutcome =
                    PickpocketsListeners.pickpocketRoll(player, 84.0, 240.0, workmanPostQuestPickpocketingTable)
                if (rollOutcome != null) {
                    queueScript(player, PickpocketsListeners.PICKPOCKET_ANIM.duration, QueueStrength.NORMAL) { stage: Int ->
                        when (stage) {
                            0 -> {
        ...
        onUseWith(SCENERY, Items.TROWEL_676, Scenery.SOIL_2376, Scenery.SOIL_2377, Scenery.SOIL_2378) { player, _, _ ->
            ...
            if (level3DigRight.insideBorder(player.location) || level3DigLeft.insideBorder(player.location)) {
                if (getQuestStage(player, "The Dig Site") >= 6) {
                    queueScript(player, 0, QueueStrength.NORMAL) { stage: Int ->
                        when (stage) {
                            0 -> {
        ...
        on(Scenery.WINCH_2350, SCENERY, "operate") { player, _ ->
            ...
            } else if (getQuestStage(player, "The Dig Site") >= 8) {
        ...
        on(Scenery.BRICK_2362, SCENERY, "search") { player, _ ->
            if (getQuestStage(player, "The Dig Site") == 8) {
                ...
                setQuestStage(player, "The Dig Site", 9)
            }
        ...
        onUseWith(SCENERY, Items.CHEMICAL_COMPOUND_707, Scenery.BRICK_2362) { player, used, _ ->
            if (getQuestStage(player, "The Dig Site") == 9) {
                if (removeItem(player, used)) {
                    ...
                    setQuestStage(player, "The Dig Site", 10)
                }
            }
            return@onUseWith true
        }
