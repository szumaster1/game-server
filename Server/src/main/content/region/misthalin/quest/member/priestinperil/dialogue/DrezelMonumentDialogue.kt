...
class DrezelMonumentDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun open(vararg args: Any): Boolean {
        ...
        if (quest.getStage(player) == 18) {
            stage = if (player.inventory.contains(1436, 1) || player.inventory.contains(7936, 1)) {
                player(FacialExpression.HALF_GUILTY, "I brought you some Rune Essence.")
                100
            } else {
        ...
        if (quest.getStage(player) == 100 && !player.getSavedData().questData.isTalkedDrezel) { // guy.
            player(FacialExpression.HALF_GUILTY, "So can I pass through that barrier now?")
            stage = 400
            return true
        }
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        ...
        when (stage) {
            ...
            121 -> end()
            100 -> {
                npc(FacialExpression.HALF_GUILTY, "Quickly, give it to me!")
                stage = 101
            }
            ...
            900 -> {
                player(FacialExpression.HALF_GUILTY, "Why, what happened?")
                stage = 901
            }
            ...
            904 -> {
                player(FacialExpression.HALF_GUILTY, "What can we do to prevent that?")
                stage = 905
            }
            ...
            911 -> {
                player(FacialExpression.HALF_GUILTY, "What's that?")
                stage = 912
            }
    ...
    companion object {
        private val ITEMS = arrayOf(Item(1436), Item(7936))
    }
}
