        ...
        if (questStage == 0) {
            when (stage) {
                ...
                8 -> when (buttonID) {
                    ...
                    2 -> {
                        npc("Yes, I dare say it does. I wouldn't even have", "mentioned it had you not seemed to be looking for", "something to do anyway.").also { stage = END_DIALOGUE }
                    }
                }
            ...
            when (stage) {
                ...
                3 -> npc(FacialExpression.ANGRY, "YOU DID WHAT???").also { stage++ }
                4 -> npc(FacialExpression.ANNOYED, "Are you mentally deficient??? That guard dog was", "protecting the route to Morytania! Without it we could", "be in severe peril of attack!").also { stage++ }
                5 -> player("Did I make a mistake?").also { stage++ }
                6 -> npc(FacialExpression.ANNOYED, "YES YOU DID!!!!! You need to get there right now", "and find out what is happening! Before it is too late for", "us all!").also { stage++ }
                7 -> player(FacialExpression.WORRIED, "B-but Drezel TOLD me to...!").also { stage++ }
