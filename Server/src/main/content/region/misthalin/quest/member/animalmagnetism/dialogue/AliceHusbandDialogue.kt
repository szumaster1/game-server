...
class AliceHusbandDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun open(vararg args: Any): Boolean {
        ...
        when (quest!!.getStage(player)) {
            0 -> if (!inEquipment(player, Items.GHOSTSPEAK_AMULET_552, 1)) {
                npc("Woo woo!")
                stage = 16
            } else {
                npc("Hi, I don't feel like talking.").also { stage = END_DIALOGUE }
            }

            10 -> player("Your animals don't look too healthy.")
            11 -> npc("'Ave you talked to the wife for me?")
            12 -> {
                player("Your wife says she needs the family cash and wants to", "know what you did with it.")
                stage++
            }

            13 -> npc("Any luck wiv me wife?")
            14, 15 -> npc("'Ave you talked to 'er?")
            16, 17, 18 -> player(
            ...
            19 -> npc("Ahh, many thanks. Now what was it you were wanting", "again?")
            else -> if (!inEquipment(player, Items.GHOSTSPEAK_AMULET_552, 1)) {
                npc("Woo woo!")
                stage = 16
            } else {
                npc("Hello, how can I help you? I'm sellin' if you have ecto-", "tokens.").also { stage = END_DIALOGUE }
            }
        }
        return true
    }
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (quest!!.getStage(player)) {
            ...
            16, 17, 18 -> when (stage) {
                ...
                7 -> if (player.inventory.remove(AnimalMagnetism.CRONE_AMULET)) {
