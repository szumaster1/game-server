...
class DrezelDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        ...
        when (stage) {
            ...
            4 -> {
                player(FacialExpression.HALF_GUILTY, "How is a river a good defence then?")
                stage = 5
            }
            ...
            602 -> {
                player(FacialExpression.HALF_GUILTY, "How do you know that?")
                stage = 603
            }
            ...
            604 -> {
                player(FacialExpression.HALF_GUILTY, "Oh.")
                stage = 605
            }
            ...
            606 -> {
                player(FacialExpression.HALF_GUILTY, "Okay, OKAY, I get the picture!")
                stage = 607
            }
            ...
            704 -> {
                if (!player.inventory.containsItem(ITEMS[0])) {
                    end()
                    return true
                }
                if (player.inventory.remove(ITEMS[0])) {
                    player.inventory.add(ITEMS[1])
                }
                end()
                player.packetDispatch.sendMessage("The priest blesses the water for you.")
            }
    ...
    override fun getIds(): IntArray {
        return intArrayOf(7690)
    }

    companion object {
        private val ITEMS = arrayOf(Item(2953), Item(2954))
    }
}