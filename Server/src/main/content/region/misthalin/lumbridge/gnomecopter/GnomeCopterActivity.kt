...
class GnomeCopterActivity : ActivityPlugin("Gnome copters", false, false, true) {
    ...
    private fun flyGnomeCopter(player: Player, `object`: Scenery) {
        if (player.location != `object`.location.transform(0, 1, 0)) {
            return
        }
        if (`object`.charge == 88) {
        ...
        Pulser.submit(object : Pulse(1, player) {
            ...
            override fun pulse(): Boolean {
                if (++stage == 1) {
                    ...
                    ForceMovement.run(player, player.location, `object`.location, ForceMovement.WALK_ANIMATION, Animation(8955), Direction.NORTH, 8)
                    player.lock()
                } else if (stage == 3) {
                    player.equipment.replace(COPTER_ITEM, 3)
                    player.visualize(Animation.create(8956), Graphic.create(1578))
                    ...
                    `object`.charge = 88
                    player.packetDispatch.sendSceneryAnimation(`object`, Animation(5))
                } else if (stage == 16) {
                    player.walkingQueue.reset()
                    player.walkingQueue.addPath(`object`.location.x, `object`.location.y + 16, true)
                    Graphic.send(Graphic.create(1579), `object`.location)
                } else if (stage == 20) {
                    `object`.charge = 1000
                    player.packetDispatch.sendSceneryAnimation(`object`, Animation(8941))
                } else if (stage == 33) {
                    player.unlock()
        ...
        Pulser.submit(object : Pulse(1, player) {
            ...
            override fun pulse(): Boolean {
                if (++stage == 1) {
                    ...
                    ForceMovement.run(player, player.location, player.location.transform(0, -1, 0), Animation(8959), 8)
                    player.lock(2)
