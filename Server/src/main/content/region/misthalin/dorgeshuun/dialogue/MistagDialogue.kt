...
import content.region.misthalin.quest.deathtothedorgeshuun.dialogue.MistagDialogueFile
...
class MistagDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun open(vararg args: Any?): Boolean {
        ...
        if (ltStage == 45) {
            npc(FacialExpression.OLD_NORMAL,"Greetings, friend. I am sorry I panicked when I saw you.")
            loadFile(MistagLostTribeDialogue(false, ltStage))
            return true
        } else if (ltStage == 50) {
            npc(FacialExpression.OLD_NORMAL,"Hello, friend?")
            loadFile(MistagLostTribeDialogue(false, ltStage))
            return true
        }
        npc(FacialExpression.OLD_NORMAL,"Hello friend!").also { stage = 0 }
        return true
    }
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            3 -> when (buttonId) {
                1 -> player("What is this favour?").also {
                    end()
            ...
            4 -> {
                if (inInventory(player, Items.SILVER_ORE_442) || inInventory(player, Items.IRON_ORE_440)) {
                    npc(FacialExpression.OLD_NORMAL,"Certainly. I will buy any iron or silver ore you mine.", "Speak to me again when you have some.").also { stage = END_DIALOGUE }
                } else {
                    npc(FacialExpression.OLD_NORMAL,"Certainly. I will buy any iron or silver ore you mine.", "Speak to me again when you have some.").also { stage = END_DIALOGUE }
                }
            }
            ...
            6 -> npc(FacialExpression.OLD_NORMAL,"Certainly, friend!").also { stage = END_DIALOGUE }
            7 -> npc(FacialExpression.OLD_NORMAL,"Certainly. Come back soon!").also { GoblinFollower.sendToLumbridge(player); stage = END_DIALOGUE }
