...
import content.region.misthalin.dialogue.draynorvillage.WiseOldManDialogue
...
class LumbridgeAchievementDiary : DiaryEventHookBase(DiaryType.LUMBRIDGE) {
    ...
    override fun onInteracted(player: Player, event: InteractionEvent) {
        when (player.viewport.region.id) {
            ...
            12849 -> if (event.target.id == Scenery.DOOR_2406 && event.option == "open" && !inEquipmentOrInventory(player, Items.DRAMEN_STAFF_772) || !inEquipmentOrInventory(player, Items.LUNAR_STAFF_9084)) {
                finishTask(
                    player,
                    DiaryLevel.EASY,
