...
public final class GnomeCopterActivity extends ActivityPlugin {
    ...
    public boolean interact(Entity e, Node target, Option option) {
        if (target instanceof Scenery) {
            ...
            if (object.getId() == 30036) {
                GnomeCopterSign.ENTRANCE.read((Player) e);
                return true;
            }
    ...
    private void flyGnomeCopter(final Player player, final Scenery object) {
        ...
        if(!player.getInventory().containsItem(new Item(12843))) {
            player.getPacketDispatch().sendMessage("You need to have gnomecopter ticket to use this.");
            return;
        }
        setAttribute(player, "gc:flying", true);
        player.lock();
        player.getInventory().remove(new Item(12843));
        player.getPacketDispatch().sendMessage("The gnomecopter accepts the ticket and sets off for Castle Wars.");
        ...
        GameWorld.getPulser().submit(new Pulse(1, player) {
            ...
            public boolean pulse() {
                if (++stage == 1) {
                    ...
                    Graphic.send(Graphic.create(1579), object.getLocation());
                } else if (stage == 20) {
