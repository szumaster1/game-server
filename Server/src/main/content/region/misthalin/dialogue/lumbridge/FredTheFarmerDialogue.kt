...
import content.region.misthalin.quest.free.sheepshearer.dialogue.FredTheFarmerSHDialogue
...
class FredTheFarmerDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun open(vararg args: Any): Boolean {
        ...
        if (getQuestStage(player, "Sheep Shearer") in 1..99) {
            openDialogue(player, FredTheFarmerSHDialogue(getQuestStage(player, "Sheep Shearer")), npc)
        } else {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            1000 -> openDialogue(player, FredTheFarmerSHDialogue(getQuestStage(player, "Sheep Shearer")), npc)
        }
        return true
    }
