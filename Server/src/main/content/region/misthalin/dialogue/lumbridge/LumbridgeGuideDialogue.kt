    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        ...
        when (stage) {
            ...
            10 -> if (!cooksAssistantComplete) {
                npcl(FacialExpression.HALF_THINKING, "You can try talking to the Cook in the Lumbridge Castle. I hear he is always looking for some help.").also { stage = END_DIALOGUE }
            } else if (!sheepShearerComplete) {
                npcl(FacialExpression.HALF_THINKING, "You can try talking to Fred the Farmer north-west of here. I hear he is always looking for some help.").also { stage = END_DIALOGUE }
            } else {
                npcl(FacialExpression.FRIENDLY, "You are such an accomplished adventurer already; you should be telling me some good quests to go on.").also { stage = END_DIALOGUE }
            }
            20 -> if (player.properties.currentCombatLevel >= 30) {
                npcl(FacialExpression.FRIENDLY, "You're strong enough to work out what monsters to fight for yourself now, but the tutors might help you with any questions you have about the skills; they're just south of the general store.").also { stage = END_DIALOGUE }
            } else {
                npcl(FacialExpression.FRIENDLY, "There are things to kill all over the place! At your level, you might like to try wandering westwards to the Wizards' Tower or north-west to the Barbarian Village.").also { stage++ }
            }
            ...
            41 -> npcl(FacialExpression.FRIENDLY, "bit of an adventure at the same time. In fact, why don't you just head there instead? It's a lot more fun, I promise. You can find it down the hole in the middle of Barbarian Village to the north-west.").also { stage = 1 }
            50 -> npcl(FacialExpression.FRIENDLY, "You'll find a bank upstairs in Lumbridge Castle - go right to the top!").also { stage = 1 }
            51 -> npcl(FacialExpression.FRIENDLY, "Of course, but first let me give you a word of warning.").also { stage++ }
            52 -> npcl(FacialExpression.FRIENDLY, "Should you choose to step away from the path of the ironman now, you will not have the option to return.").also { stage++ }
            53 -> npcl(FacialExpression.FRIENDLY, "Now I ask you to make sure, are you sure you want to <b>permanently</b> remove ironman mode?").also { stage++ }
            54 -> showTopics(Topic(FacialExpression.FRIENDLY, "Yes, I'm sure.", 55), Topic(FacialExpression.FRIENDLY, "No, I've changed my mind.", END_DIALOGUE))
            55 -> npcl(FacialExpression.FRIENDLY, "Very well, let me just check one thing...").also { stage++ }
            56 -> if (player.ironmanManager.mode == IronmanMode.HARDCORE) {
