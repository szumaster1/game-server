...
class HieronymusAvlafrimDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when(stage) {
            ...
            2 -> npc(FacialExpression.OLD_NORMAL,"The gnomecopters are something Sasquine and I","invented. Using a unique blend of gnomic engineering","and human magic, we've launched a whole new kind of", "tour experience.").also { stage++ }
            3 -> {
                setTitle(player, 2)
            ...
            5 -> npc(FacialExpression.OLD_NORMAL, "Oh, it's very clever. Sasquine and I saw some men","riding around on big magic carpet thingies in the desert","south of Al Kharid and we couldn't resist 'borrowing'","one or two.").also { stage++ }
            6 -> player("You 'borrowed' some magic carpets?").also { stage++ }
            7 -> npc(FacialExpression.OLD_NORMAL,"Well, we didn't exactly ask permission but they're for", "research purposes. That's allowed! Anyway, we're going","off topic.").also { stage++ }
            8 -> player("Alright, so you stole some magic carpets.","What did you do then?").also { stage++ }
            9 -> npc(FacialExpression.OLD_NORMAL,"We managed to interface the carpets with the latest in","gnome glider technology. My brother's an expert flight","engineer; he helped a lot. The result is what you see","here; the gnomecopter.").also { stage++ }
            10 -> player("It doesn't look much like a gnome glider or a magic","carpet.").also { stage++ }
            11 -> npc(FacialExpression.OLD_NORMAL,"The rotors are mostly made from the fabric of the","carpets. Our expert gnomic engineering makes it far","easier to handle than the original carpet and its magic","allows the gnomecopters to do things that normal gliders").also { stage++ }
            12 -> npc(FacialExpression.OLD_NORMAL,"can't.").also { stage++ }
            13 -> player("Such a letting non-members have a preview of","members' areas.").also { stage++ }
            14 -> npc(FacialExpression.OLD_NORMAL, "Precisely! So, is there anything else you'd like to know?").also { stage = 0 }
            15 -> npc(FacialExpression.OLD_NORMAL,"Oh no, I coudn't part with one of those.","They're very hard to make.").also { stage = 0 }
            16 -> npc(FacialExpression.OLD_NORMAL,"Well, this may sound strange but we've using them to","show members' areas to people on non-member's worlds.").also { stage++ }
            17 -> player("Hang on, we're on a members' world here!","I could just go to these places myself.").also { stage++ }
            18 -> npc(FacialExpression.OLD_NORMAL,"Yes, you probably could. That said we're currently","offering our service for free and you can't beat that","price! Chat to Sasquine to get going.").also { stage++ }
            19 -> player("I'll have a think about it.").also {
