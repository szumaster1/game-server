package content.region.misc.ancientcavern
...
class DragonforgeListeners : InteractionListener {
    ...
    override fun defineListeners() {
        ...
        onUseWith(IntType.SCENERY,
            content.region.misc.ancientcavern.DragonforgeListeners.Companion.DRAGON_ANVIL, *content.region.misc.ancientcavern.DragonforgeListeners.Companion.RUINED_PIECES
        ) { player, _, _ ->
            ...
            if (!player.inventory.containsItem(content.region.misc.ancientcavern.DragonforgeListeners.Companion.FUSION_HAMMER)) {
                sendDialogue(player, "You need a hammer to work the metal with.")
                return@onUseWith false
            }

            if (!anyInInventory(player, *content.region.misc.ancientcavern.DragonforgeListeners.Companion.RUINED_PIECES)) {
            ...
            queueScript(player, 1, QueueStrength.SOFT) { stage: Int ->
                when (stage) {
                    0 -> {
                        ...
                        animate(player,
                            content.region.misc.ancientcavern.DragonforgeListeners.Companion.SMITHING_ANIMATION
                        )
                        return@queueScript delayScript(player, content.region.misc.ancientcavern.DragonforgeListeners.Companion.SMITHING_ANIMATION.duration)
                    }

                    1 -> {
                        if (removeAll(player,
                                content.region.misc.ancientcavern.DragonforgeListeners.Companion.RUINED_PIECES
                            )) {
                            sendMessage(player, "You finish your efforts...")
                            removeItem(player,
                                content.region.misc.ancientcavern.DragonforgeListeners.Companion.FUSION_HAMMER
                            )
                            player.inventory.add(content.region.misc.ancientcavern.DragonforgeListeners.Companion.DRAGON_PLATEBODY)
        ...
        on(content.region.misc.ancientcavern.DragonforgeListeners.Companion.MITHRIL_DOOR, IntType.SCENERY, "open") { player, node ->
        ...
        onUseWith(IntType.SCENERY, Items.DRAGONKIN_KEY_14471, *content.region.misc.ancientcavern.DragonforgeListeners.Companion.MITHRIL_DOOR) { player, _, node ->
            if (!inInventory(player, Items.DRAGONKIN_KEY_14471) && !hasRequirement(player, "While Guthix Sleeps", false)) {
                sendDialogue(player, "The door is solid and resists all attempts to open it. There's no way past it at all, so best to ignore it.")
                return@onUseWith true
            }
        ...
        onUseWith(IntType.NPC,
            content.region.misc.ancientcavern.DragonforgeListeners.Companion.STRANGE_KEYS, *content.region.misc.ancientcavern.DragonforgeListeners.Companion.MITHRIL_DRAGON_NPC
        ) { player, _, with ->
            val npc = with.asNpc()
            if (player.inventory.containItems(*content.region.misc.ancientcavern.DragonforgeListeners.Companion.STRANGE_KEYS) && player.equipment.containsAtLeastOneItem(
                    content.region.misc.ancientcavern.DragonforgeListeners.Companion.REQUIRED_SHIELD
                )) {
                    ...
                    override fun pulse(): Boolean {
                        when (counter++) {
                            0 -> {
                                face(player, npc)
                                visualize(npc!!,
                                    content.region.misc.ancientcavern.DragonforgeListeners.Companion.DRAGON_BREATH_ANIMATION,
                                    content.region.misc.ancientcavern.DragonforgeListeners.Companion.DRAGON_BREATH_GFX
                                )
                            }

                            1 -> {
                                visualize(player,
                                    content.region.misc.ancientcavern.DragonforgeListeners.Companion.DRAGON_SHIELD_ABSORB_ANIM,
                                    content.region.misc.ancientcavern.DragonforgeListeners.Companion.DRAGON_BREATH_ABSORB_GFX
                                )
                            }

                            3 -> {
                                player.inventory.removeAll(content.region.misc.ancientcavern.DragonforgeListeners.Companion.STRANGE_KEYS)
                                sendItemDialogue(player,
                                    content.region.misc.ancientcavern.DragonforgeListeners.Companion.DRAGONKIN_KEY, "The intense heat of the mithril dragon's breath fuses the key halves together.")
                                player.inventory.add(content.region.misc.ancientcavern.DragonforgeListeners.Companion.DRAGONKIN_KEY)
                            }
