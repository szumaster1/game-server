...
class MosleharmlessListeners : InteractionListener {
    ...
    override fun defineListeners() {
        ...
        onUseWith(IntType.ITEM,
            content.region.misc.MosleharmlessListeners.Companion.BERET_AND_MASK, NPCs.PATCHY_4359) { player, used, _ ->
            if (used.id != Items.BERET_AND_MASK_11282) {
                sendNPCDialogue(player, NPCs.PATCHY_4359, "Sorry, I can't do anythin' with that.")
            } else {
                openDialogue(player, content.region.misc.MosleharmlessListeners.SeparateItemsDialogue())
            }
            return@onUseWith true
        }

        // Listener for the shop owners when the player selects "trade".
        on(content.region.misc.MosleharmlessListeners.Companion.SHOP_OWNERS, IntType.NPC, "trade") { player, node ->
