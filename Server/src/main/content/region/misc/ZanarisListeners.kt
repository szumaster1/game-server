...
class ZanarisListeners : InteractionListener {
    ...
    override fun defineListeners() {
        ...
        on(content.region.misc.ZanarisListeners.Companion.MAGIC_DOORS, IntType.SCENERY, "open") { player, node ->
        ...
        onUseWith(IntType.SCENERY,
            content.region.misc.ZanarisListeners.Companion.REQUIRED_ITEMS,
            content.region.misc.ZanarisListeners.Companion.CHICKEN_SHRINE
        ) { player, used, _ ->
            ...
            if (!removeItem(player, used.asItem())) {
                ...
                queueScript(player, 1, QueueStrength.SOFT) { stage: Int ->
                    when (stage) {
                        0 -> {
                            animate(player, content.region.misc.ZanarisListeners.Companion.TP_ANIM_START)
                            return@queueScript keepRunning(player)
                        }

                        1 -> {
                            teleport(player, content.region.misc.ZanarisListeners.Companion.ENTER_LOCATION)
                            animate(player, content.region.misc.ZanarisListeners.Companion.TP_ANIM_ENDED)
        ...
        on(content.region.misc.ZanarisListeners.Companion.PORTAL_ENTRANCE, IntType.SCENERY, "use") { player, _ ->
            teleport(player, content.region.misc.ZanarisListeners.Companion.EXIT_LOCATION)
            return@on true
        }
        ...
        onUseWith(IntType.SCENERY,
            content.region.misc.ZanarisListeners.Companion.ROPE,
            content.region.misc.ZanarisListeners.Companion.TUNNEL_ENTRANCE
        ) { player, used, _ ->
            if (!removeItem(player, used.asItem())) {
                sendMessage(player, "Nothing interesting happens.")
            } else {
                replaceScenery(
                    core.game.node.scenery.Scenery(content.region.misc.ZanarisListeners.Companion.TUNNEL_ENTRANCE, location(2455, 4380, 0)),
                    content.region.misc.ZanarisListeners.Companion.ENTRANCE_ROPE,
                    80
                )
            }
            return@onUseWith true
        ...
        on(content.region.misc.ZanarisListeners.Companion.ENTRANCE_ROPE, IntType.SCENERY, "climb-down") { player, _ ->
        ...
        on(content.region.misc.ZanarisListeners.Companion.ROPE_SCENERY, IntType.SCENERY, "climb-up") { player, _ ->
            ClimbActionHandler.climb(
                player,
                Animation(Animations.MULTI_USE_BEND_OVER_827),
