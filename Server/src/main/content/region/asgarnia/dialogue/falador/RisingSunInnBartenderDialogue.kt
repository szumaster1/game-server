...
class RisingSunInnBartenderDialogue(player: Player? = null) : Dialogue(player) {
        ...
        when (stage) {
            START_DIALOGUE -> {
                if (hasAnyBeerGlasses()) {
                    ...
                    playerl(FacialExpression.ASKING, "What ales are you serving?").also { stage = 10 }
                }
            }
            ...
            20 -> npcl(FacialExpression.HALF_GUILTY, "Oh, we will buy those from you if you're interested. We offer 2 coins for each glass.").also { stage++ }
            21 -> showTopics(Topic(FacialExpression.HAPPY, "Yes, please!", 22), Topic(FacialExpression.NEUTRAL, "No thanks, I like my empty beer glasses.", END_DIALOGUE))
            22 -> {
    ...
    private fun ensureHasMoney(): Boolean {
        if (!inInventory(player, Items.COINS_995, 3)) {
            npcl(FacialExpression.ANGRY, "No freeloaders!").also {
    ...
    private fun purchaseBrew(brewId: Int) {
        if (removeItem(player, Item(Items.COINS_995, 3))) {
            ...
            sendItemDialogue(player, Item(brewId), "You hand 3 coins over to ${npc.name}. She gives you ${
                when (brewId) {
