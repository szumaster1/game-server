...
class TostigDialogue : DialogueFile(), InteractionListener {

    override fun handle(componentID: Int, buttonID: Int) {
        npc = NPC(NPCs.TOSTIG_1079)
        when (stage) {
            ...
            3 -> showTopics(Topic(FacialExpression.FRIENDLY, "Yes, please.", 10), Topic(FacialExpression.FRIENDLY, "No, thanks.", 20))
            10 -> {
                end()
            ...
            21 -> showTopics(Topic(FacialExpression.FRIENDLY, "Huh?", 30), Topic(FacialExpression.FRIENDLY, "Yes, and the black cat dances in the sink.", 40), Topic(FacialExpression.FRIENDLY, "No, squirrels can't sing.", 50))
    ...
    override fun defineListeners() {
        on(NPCs.TOSTIG_1079, IntType.NPC, "Talk-to") { player, _ ->
            openDialogue(player, TostigDialogue())
            return@on true
        }
    }
