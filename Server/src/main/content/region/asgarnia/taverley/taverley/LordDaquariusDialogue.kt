...
class LordDaquariusDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            14 -> player.dialogueInterpreter.sendDialogues(NPCs.SAVANT_2748, FacialExpression.NEUTRAL, "${player.username}?","I've been monitoring you.", "He's telling the truth, so don't try to bluff him by","starting a fight.").also { stage++ }
            15 -> player.dialogueInterpreter.sendDialogues(NPCs.SAVANT_2748, FacialExpression.NEUTRAL, "All of our records show that the only thing he will make","a stand for is the protection of his men, and he has little","regard for his own safety in comparison.").also { stage++ }
            16 -> player.dialogueInterpreter.sendDialogues(NPCs.SAVANT_2748, FacialExpression.NEUTRAL, "You're going to have to find some other way to","intimidate him into giving you the information we need.").also { stage++ }
            17 -> player("Understood.", "${player.username} out.").also { stage = END_DIALOGUE }

            18 -> npc(FacialExpression.SAD,"Stop!", "I will tell you what you want!","Please... leave my men be...").also { stage = END_DIALOGUE }
            ...
            21 -> npc(FacialExpression.SAD,"*sigh*", "I should have known the White Knights would be my","ruin once again...").also { stage++ }
            22 -> npc(FacialExpression.SAD, "I do not know his exact whereabouts, and when last we","met we did not leave...","on the best of terms.").also { stage++ }
            ...
            26 -> player(FacialExpression.ANGRY,"That's it?","That's your help?","You'd better not be lying Daquarius, or...").also { stage++ }
            27 -> player.dialogueInterpreter.sendDialogues(NPCs.SAVANT_2748, FacialExpression.NEUTRAL, "Calm yourself Player, he is telling the truth.").also { stage++ }
            28 -> player("How do you know?").also { stage++ }
            29 -> player.dialogueInterpreter.sendDialogues(NPCs.SAVANT_2748, FacialExpression.NEUTRAL,"I am monitoring your conversation through the","CommOrb, his physiology shows none of the changes we","usually get when someone lies to us.").also { stage++ }
            30 -> player("So this CommOrb thing can work as a lie-detector?","That's pretty useful!").also { stage++ }
            31 -> player.dialogueInterpreter.sendDialogues(NPCs.SAVANT_2748, FacialExpression.NEUTRAL, "Yes, it is, but back to the task at hand.","I suggest you head to the next subject of our","observations, he might be more help in locating Solus.").also { stage++ }
            32 -> player("You mean that Zamorakian mage in Varrock?").also { stage++ }
            33 -> player.dialogueInterpreter.sendDialogues(NPCs.SAVANT_2748, FacialExpression.NEUTRAL, "Yes, he would seem a likely bet.","Savant out.").also { stage = END_DIALOGUE }
        }
        return true
    }
