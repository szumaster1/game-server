...
/**
 * Dunstan dialogue file2
 *
 * @constructor Dunstan dialogue file2
 */
class DunstanDialogueFile2 : DialogueFile() {
    ...
    override fun handle(componentID: Int, buttonID: Int) {
        when (getQuestStage(player!!, "Troll Stronghold")) {
            in 1..10 -> {
                when (stage) {
                    START_DIALOGUE ->
                        npcl(FacialExpression.FRIENDLY, "Have you managed to rescue Godric yet?").also { stage++ }
                    ...
                    3 -> showTopics(Topic(FacialExpression.THINKING, "Is it OK if I use your anvil?", 10), Topic(FacialExpression.FRIENDLY, "Nothing, thanks.", END_DIALOGUE))
