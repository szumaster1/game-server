...
class BoyDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun open(vararg args: Any): Boolean {
        ...
        if (!player.inventory.containsItem(BALL)) {
            npc(FacialExpression.CHILD_GUILTY, "Have you gotten my ball back yet?")
        } else {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        ...
        when (stage) {
            ...
            11 -> if (!player.inventory.containsItem(BALL)) {
                player("Not yet.")
                next()
            } else {
            ...
            12 -> {
                npc(FacialExpression.CHILD_GUILTY, "Well it's in the shed in that garden.")
                finish()
            }
