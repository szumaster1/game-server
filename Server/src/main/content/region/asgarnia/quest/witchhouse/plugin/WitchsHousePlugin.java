...
public class WitchsHousePlugin extends OptionHandler {
    ...
    public boolean handle(Player player, Node node, String option) {
        ...
        final int id = node instanceof Item ? node.getId() : node instanceof Scenery ? ((Scenery) node).getId() : node instanceof NPC ? ((NPC) node).getId() : node.getId();
            ...
            UseWithHandler.addHandler(901, UseWithHandler.NPC_TYPE, new UseWithHandler(2410) {
                ...
                public boolean handle(NodeUsageEvent event) {
                    ...
                    if (useditem.getId() == MAGNET.getId() && npc.getId() == 901 && player.getAttribute("mouse_out") != null) {
                        player.getDialogueInterpreter().sendDialogue("You attach a magnet to the mouse's harness. The mouse finishes", "the cheese and runs back into its hole. You hear some odd noises", "from inside the walls. There is a strange whirring noise from above", "the door frame.");
                        removeAttribute(player, "mouse_out");
                        if (player.getInventory().remove(MAGNET))
                            setAttribute(player, "attached_magnet", true);
