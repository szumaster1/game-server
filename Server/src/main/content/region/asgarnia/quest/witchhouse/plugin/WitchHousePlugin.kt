package content.region.asgarnia.quest.witchhouse.plugin
...
class WitchHousePlugin : OptionHandler() {
    ...
    override fun handle(player: Player, node: Node, option: String): Boolean {
        ...
        when (id) {
                ...
                for (item in items) {
                    if (!inInventory(player, item)) {
                        when (item) {
                            ...
                            1059 -> {
                                addItem(player, item)
                                sendMessage(player, "You find a pair of leather gloves in the bottom of one of the boxes!")
                                return true
                            }

                            1061 -> {
                                addItem(player, item)
                                sendMessage(player, "You find a pair of leather boots in the bottom of one of the boxes!")
                                return true
                            }
            ...
            2869 -> if (player.inventory.addIfDoesntHave(MAGNET)) {
            ...
            2867 -> if (player.inventory.addIfDoesntHave(DOOR_KEY)) {
            ...
            2862 -> if (magnetAttached || player.location.y < 3466) {
                handleAutowalkDoor(player, (node as Scenery))
                removeAttribute(player, "attached_magnet")
            } else {
                sendDialogueLines(player, "Strange... I can't see any kind of lock or handle to", "open this door...")
            }

            2865, 2866 -> if (player.equipment.containsItem(LEATHER_GLOVES)) {
                handleAutowalkDoor(player, (node as Scenery))
            } else {
            ...
            2864 -> {
                if (readBook && player.inventory.addIfDoesntHave(KEY)) {
                    sendDialogueLines(player, "You search for the secret compartment mentioned in the diary.", "Inside it you find a small key. You take the key.")
                } else {
