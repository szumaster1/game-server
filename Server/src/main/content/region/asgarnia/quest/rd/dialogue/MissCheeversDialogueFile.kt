...
class MissCheeversDialogueFile(private val state: Int = 0) : DialogueFile() {
    ...
    private fun handleInitialDialogue() {
        when (stage) {
            ...
            10 -> npcl(FacialExpression.FRIENDLY, "I really can't be any more help than that I'm afraid, it is more than my job's worth to have given you the help I already have.").also { stage = 6 }
            11 -> playerl(FacialExpression.FRIENDLY, "Okay, thanks a lot, you've been very helpful!").also { stage++ }
            12 -> npcl(FacialExpression.FRIENDLY, "Best of luck with the test ${player!!.username}. I hope your application is successful.").also { stage = 8 }
            13 -> {
                end()
    ...
    private fun handleChallengeDialogue() {
        clearAttributes()
        when (stage) {
            0 -> npcl(FacialExpression.FRIENDLY, "Greetings, ${player!!.username}. Welcome to my challenge.").also { player!!.faceLocation(location(2469, 4941, 0)) }.also { stage++ }
