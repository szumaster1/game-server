package content.region.fremennik.waterbirth_island.warterbirth_dungeon

import cfg.consts.Items
import cfg.consts.NPCs
import content.region.fremennik.waterbirth_island.warterbirth_dungeon.npc.DagannothKingNPC
import content.region.fremennik.waterbirth_island.warterbirth_dungeon.npc.SpinolypNPC
                    ...
                    override fun pulse(): Boolean {
                        when (++counter) {
                            ...
                            3 -> {
                                ...
                                for (n in eggs) {
                                    n.transform(n.id + 1)
                                    val spawn = NPC.create(2454, n.location.transform(-1, 0, 0))
                                    spawn.init()
                                    spawn.isWalks = true
            ...
            when (target.id) {
                ...
                10217 -> {
                    if (isQuestComplete(player, "Horror from the Deep")) {
                        player.teleport(Location.create(1957, 4373, 1))
                    } else {
        ...
        override fun handle(player: Player, node: Node, option: String): Boolean {
            when (node.id) {
                ...
                2440, 2443, 2446 -> {
                    ...
                    var canAttack = true
                    if (x == 2545 && y == 10145 && player.location.y > y) { // top
                        // north
                        canAttack = false
                    } else if (x == 2543 && y == 10143 && player.location.x < x) { // middle
                        canAttack = false
                    } else if (x == 2545 && y == 10141 && player.location.y < y) { // south
                        canAttack = false
                    }
                    if (!canAttack) {
