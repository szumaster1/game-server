...
class RunaDialogue(player: Player? = null): Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            3 -> when (buttonId) {
                1 -> player(FacialExpression.ASKING, "Yes please.").also { stage = 10 }
            ...
            20 -> npc(FacialExpression.HALF_WORRIED, "Business is booming!").also { stage++ }
            21 -> npc(FacialExpression.HALF_WORRIED, "Now, if only I hadn't taken a loss to the beer I sold", "to those teenagers.").also { stage = END_DIALOGUE }
        }
        return true
    }
