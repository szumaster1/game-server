...
class HorrorFromTheDeep : Quest("Horror from the Deep", 77, 76, 2, Vars.VARBIT_QUEST_HORROR_FROM_THE_DEEP_PROGRESS, 0, 1, 10) {
    ...
    override fun drawJournal(player: Player?, stage: Int) {
        ...
        if (stage >= 1) {
            ...
            line(player, "and repaired the !!bridge?? to Rellekka with some planks.", line++, getAttribute(player, HFTDUtils.FIX_BRIDGE, 0) == 2)
            line++
        }
        if (stage >= 20) {
        ...
        if (stage >= 40 && getAttribute(player, HFTDUtils.STRANGE_WALL_DISCOVER, false)){
    ...
    override fun finish(player: Player?) {
        ...
        removeAttributes(player, HFTDUtils.FIX_LIGHTHOUSE_MECHANISM, HFTDUtils.FIX_BRIDGE, HFTDUtils.PUZZLE_PROGRESS, HFTDUtils.FIX_LIGHTHOUSE_MECHANISM, HFTDUtils.FIX_BRIDGE, HFTDUtils.STRANGE_WALL_DISCOVER, HFTDUtils.PUZZLE_PROGRESS, HFTDUtils.STRANGE_W_AIR, HFTDUtils.STRANGE_W_FIRE, HFTDUtils.STRANGE_W_EARTH, HFTDUtils.STRANGE_W_WATER, HFTDUtils.STRANGE_W_ARROW, HFTDUtils.STRANGE_W_SWORD)
    }
