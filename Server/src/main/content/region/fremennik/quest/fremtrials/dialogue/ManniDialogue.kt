...
class ManniDialogue(player: Player? = null): Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            161 -> {
                npcl(FacialExpression.NEUTRAL, "That is all.")
                player.incrementAttribute("sigmund-steps", 1)
                stage = END_DIALOGUE
            }
            162 -> npcl(FacialExpression.ANNOYED, "Uh... yes, the longhall barkeep has it. So could you get me my drink now please?").also { stage = END_DIALOGUE }
            165 -> npcl(FacialExpression.ANNOYED, "Not me, no.").also { stage = END_DIALOGUE }
            170 -> {
                npcl(FacialExpression.AMAZED, "...It is true! The legendary cocktail! I have waited for this day ever since I first started drinking!")
            ...
            175 -> playerl(FacialExpression.HAPPY, "I guess you're happy with the trade then!").also { stage = END_DIALOGUE }
            ...
            184 -> npcl(FacialExpression.HAPPY, "Mostly tomato juice.").also { stage = END_DIALOGUE }
            190 -> {
                npcl(FacialExpression.HAPPY, "Hey! It's ${getAttribute(player, "fremennikname", "fremmyname")}! Let me buy you a drink!")
                addItem(player, Items.BEER_1917)
                stage++
            }
            191 -> npcl(FacialExpression.HAPPY, "There ya go! Anyone who can drink like you earns my respect!").also { stage = END_DIALOGUE }
            ...
            202 -> npcl(FacialExpression.HAPPY, "He is standing just over there. He will speak for the tribe.").also { stage = END_DIALOGUE }
        }
        return true
    }

    /**
     * Class representing a Drinking Pulse.
     *
     * @param player The player involved in the drinking action.
     * @param npc The NPC involved in the drinking action.
     * @param lowAlcohol Flag indicating if the drink has low alcohol content.
    ...
    class DrinkingPulse(val player: Player?, val npc: NPC?, val lowAlcohol: Boolean? = false) : Pulse() {
        ...
        override fun pulse(): Boolean {
            if (!lowAlcohol!!) {
                when (counter++) {
                    0 -> {
                        player?.lock();npc?.lock(); npc?.isNeverWalks = true
                    }

                    1 -> {
                        player?.face(npc); npc?.face(player)
                    }

                    3 -> npc?.animator?.animate(Animation(1330, Animator.Priority.HIGH))
                    5 -> {
                        player?.animator?.animate(Animation(1330, Animator.Priority.HIGH)); player?.inventory?.remove(
                    ...
                    16 -> {
                        player?.unlock(); npc?.unlock(); player?.face(player);npc?.face(npc); npc?.isNeverWalks =
                            false; return true
                    }
                }
            } else {
                when (counter++) {
                    ...
                    3 -> npc?.animator?.animate(Animation(1330, Animator.Priority.HIGH))
                    5 -> {
                        player?.animator?.animate(Animation(1330, Animator.Priority.HIGH)); player?.inventory?.remove(Item(Items.KEG_OF_BEER_3711))
                    }
                    ...
                    22 -> {
                        player?.unlock(); npc?.unlock(); player?.face(player);npc?.face(npc); npc?.isNeverWalks = false;removeAttribute(player!!, "fremtrials:cherrybomb");removeAttribute(player, "fremtrials:manni-accepted");removeAttribute(player, "fremtrials:keg-mixed"); setAttribute(player, "/save:fremtrials:manni-vote", true); setAttribute(player, "/save:fremtrials:votes", getAttribute(player, "fremtrials:votes", 0) + 1); return true
                    }
                }
            }
            return false
