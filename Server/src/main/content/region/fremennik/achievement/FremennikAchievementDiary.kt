...
import content.region.fremennik.quest.viking.dialogue.ChieftanBrundtDialogue
import cfg.consts.Components
import cfg.consts.Items
import cfg.consts.NPCs
import cfg.consts.Scenery
import content.dialogue.jatizso.TowerGuardDialogue
import content.dialogue.rellekka.HuntingExpertRellekkaDialogue
...
class FremennikAchievementDiary : DiaryEventHookBase(DiaryType.FREMENNIK) {
    ...
    companion object {
        ...
        private val FISHING_SPOTS = arrayOf(NPCs.FISHING_SPOT_309, NPCs.FISHING_SPOT_334, NPCs.FISHING_SPOT_324, NPCs.FISHING_SPOT_322)
        private val ROCK_CRABS = arrayOf(NPCs.ROCK_CRAB_1265, NPCs.ROCK_CRAB_1267)
        private val CAVE_CRAWLERS = arrayOf(NPCs.CAVE_CRAWLER_1600, NPCs.CAVE_CRAWLER_1601, NPCs.CAVE_CRAWLER_1602, NPCs.CAVE_CRAWLER_1603)
        private val DAGANNOTHS = arrayOf(NPCs.DAGANNOTH_2455, NPCs.DAGANNOTH_2456, NPCs.DAGANNOTH_PRIME_2882, NPCs.DAGANNOTH_REX_2883, NPCs.DAGANNOTH_SUPREME_2881)
        private val ICE_TROLLS = arrayOf(NPCs.ICE_TROLL_RUNT_5521, NPCs.ICE_TROLL_MALE_5522, NPCs.ICE_TROLL_FEMALE_5523, NPCs.ICE_TROLL_GRUNT_5524, NPCs.ICE_TROLL_RUNT_5525, NPCs.ICE_TROLL_MALE_5526, NPCs.ICE_TROLL_FEMALE_5527, NPCs.ICE_TROLL_GRUNT_5528, NPCs.ICE_TROLL_RUNT_5473, NPCs.ICE_TROLL_MALE_5474, NPCs.ICE_TROLL_FEMALE_5475, NPCs.ICE_TROLL_GRUNT_5476)
        ...
        object MediumTasks {
            ...
            // Watch a shouting match between Fremennik Isles tower guards.
        ...
        object HardTasks {
            ...
            // Wear rockshell, spined or skeletal armour and have the locals use an honorific with your Fremennik name.
        ...
        when (player.viewport.region.id) {
            10042, 10554 -> if (event.npc.id in ROCK_CRABS) {
                progressIncrementalTask(
                    ...
                    ATTRIBUTE_ROCK_CRAB_KILLCOUNT, 5)
            }
            ...
            9886, 10142, 11589 -> if (event.npc.id in DAGANNOTHS) {
                progressIncrementalTask(
                    ...
                    ATTRIBUTE_DAGANNOTHS_KILLCOUNT, 3)
            }
    ...
    override fun onInterfaceClosed(player: Player, event: InterfaceCloseEvent) {
        when (event.component.id) {
            Components.YRSA_SHOE_STORE_200 -> if (player.viewport.region.id == 10297) {
                finishTask(
                    player,
                    DiaryLevel.MEDIUM,
    ...
    override fun onInteracted(player: Player, event: InteractionEvent) {
        when (player.viewport.region.id) {
            9377 -> if (event.target.id == Scenery.ROCK_16684 && event.option == "mine") {
                finishTask(
                    player,
                    DiaryLevel.HARD,
