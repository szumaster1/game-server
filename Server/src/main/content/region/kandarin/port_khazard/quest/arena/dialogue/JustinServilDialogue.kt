    ...
    override fun handle(componentID: Int, buttonID: Int): Boolean {
        when (getQuestStage(player!!, "Fight Arena")) {
            ...
            in 0..68 -> when (stage) {
                ...
                1 -> npcl(FacialExpression.FRIENDLY, "You've incurred the anger of Khazard himself. He won't let any of us go easily.").also { stage = END_DIALOGUE }
            }

            in 69..71 -> when (stage) {
                0 -> playerl(FacialExpression.FRIENDLY, "Lady Servil sent me to rescue you and your son. Come on, we have to get out of here.").also { stage++ }
            ...
            72 -> when (stage) {
                0 -> playerl(FacialExpression.FRIENDLY, "Are you alright").also { stage++ }
            ...
            in 90..97 -> when (stage) {
                0 -> playerl(FacialExpression.FRIENDLY, "You can run, Khazard's interest is in me now.").also { stage++ }

                1 -> npcl(FacialExpression.NEUTRAL, "My thanks, we'll be ok now. I suggest you run too, General Khazard is mighty and his anger fearsome.").also { stage = END_DIALOGUE }
            }
            ...
            100 -> when (stage) {
                0 -> npcl(FacialExpression.FRIENDLY, "Thank you again, ${player!!.username}. You have saved me and my boy from an awful fate.").also { stage++ }
