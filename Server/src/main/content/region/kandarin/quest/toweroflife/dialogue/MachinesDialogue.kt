...
class MachinesDialogue : DialogueFile() {
    ...
    override fun handle(componentID: Int, buttonID: Int) {
        when (stage) {
            ...
            5 -> npc("The pressure seems to be affected by holes in the pipes. Perhaps I can block them up with the balls.").also {
                stage = END_DIALOGUE
            }
    ...
    class PipeMachineDialogue : DialogueFile() {
        override fun handle(componentID: Int, buttonID: Int) {
            when (stage) {
                ...
                4 -> npc("You built the machine! It still needs calibrating, though.").also { stage++ }
                5 -> npc("Looks like the pipes need connecting, to allow the liquid to travel further up the tower!").also {
                    stage = END_DIALOGUE
                }
    ...
    class CageDialogue : DialogueFile() {
        override fun handle(componentID: Int, buttonID: Int) {
            when (stage) {
                ...
                5 -> npc("Some of the bars need to be completed - you need to get the sizes correct, though.").also {
                    stage = END_DIALOGUE
                }
    ...
    class TowerDialogue : DialogueFile() {
        override fun handle(componentID: Int, buttonID: Int) {
            when (stage) {
                0 -> npc("The tower should be in working order now! Best go and tell Effigy!").also {
                    stage = END_DIALOGUE
                }
            }
        }
