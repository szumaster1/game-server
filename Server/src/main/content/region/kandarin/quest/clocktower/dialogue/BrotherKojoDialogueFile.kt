package content.region.kandarin.quest.clocktower.dialogue

import content.region.kandarin.quest.clocktower.ClockTower
 ...
 *
 * @constructor Brother kojo dialogue file
 */
class BrotherKojoDialogueFile : DialogueFile() {

    override fun handle(componentID: Int, buttonID: Int) {
        if (getAttribute(player!!, ClockTower.ASK_ABOUT_RATS_ATTR, false) && !getAttribute(player!!, ClockTower.RATS_POISONED_ATTR, false)) {
            when (stage) {
                ...
                4 -> playerl(FacialExpression.FRIENDLY, "I guess I better go and deal with the rat problem...").also { stage = END_DIALOGUE }
            }
            return
        }
        when (getQuestStage(player!!, "Clock Tower")) {
            0 -> {
                when (stage) {
                    ...
                    30 -> npc(FacialExpression.HAPPY, "Oh, thank you kind ${if (player!!.isMale) "sir" else "madam"}!", "In the cellar below, you'll find four cogs.", "They're too heavy for me, but you should be able to", "carry them one at a time.").also { stage++ }
