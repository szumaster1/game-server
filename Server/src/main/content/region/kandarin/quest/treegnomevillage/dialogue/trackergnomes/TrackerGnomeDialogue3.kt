...
class TrackerGnomeDialogue3(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(componentID: Int, buttonID: Int): Boolean {
        ...
        when {
            ...
            questStage == 30 -> {
                when (stage) {
                    0 -> playerl(FacialExpression.ASKING, "Are you OK?").also { stage++ }
                    1 -> npcl(FacialExpression.OLD_NORMAL, "OK? Who's OK? Not me! Hee hee!").also { stage++ }
                    2 -> playerl(FacialExpression.ASKING, "What's wrong?").also { stage++ }
                    3 -> npcl(FacialExpression.OLD_NORMAL, "You can't see me, no one can. Monsters, demons, they're all around me!")
                        .also { stage++ }

                    4 -> playerl(FacialExpression.ASKING, "What do you mean?").also { stage++ }
                    5 -> npcl(FacialExpression.OLD_NORMAL, "They're dancing, all of them, hee hee.").also { stage++ }
                    6 -> sendDialogue(player!!, "He's clearly lost the plot.").also { stage++ }
                    7 -> playerl(FacialExpression.HALF_ASKING, "Do you have the coordinate for the Khazard stronghold?").also { stage++ }
                    ...
                    11 -> playerl(FacialExpression.HALF_GUILTY, "You're mad").also { stage++ }
                    12 -> npcl(FacialExpression.OLD_NORMAL, "Dance with me, and Khazard's men are beat.").also { stage++ }

                    13 -> sendDialogue(player!!, "The toll of war has affected his mind.").also { stage++ }
                    14 -> playerl(FacialExpression.HALF_GUILTY, "I'll pray for you little man.").also { stage++ }

                    15 -> {
                        npcl(FacialExpression.OLD_NORMAL, "All day we pray in the hay, hee hee.").also {
            ...
            questStage == 31 -> {
                when (stage) {
                    ...
                    1 -> npcl(FacialExpression.OLD_NORMAL, "Don't talk to me, you can't see me. No one can, just the demons.").also { stage = END_DIALOGUE }
                }
            }
            ...
            questStage > 31 -> {
                when (stage) {
                    ...
                    2 -> playerl(FacialExpression.FRIENDLY, "I think you do.").also { stage = END_DIALOGUE }
                }
            }
        }
        return true
