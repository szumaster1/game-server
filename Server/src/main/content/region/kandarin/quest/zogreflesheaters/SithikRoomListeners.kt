...
class SithikRoomListeners : InteractionListener, MapArea {
    ...
    override fun defineListeners() {
        ...
        on(SITHIK, IntType.SCENERY, "talk-to") { player, _ ->
            when {
                getAttribute(player, ZogreQuestUtils.SITHIK_DIALOGUE_UNLOCK, false) -> openDialogue(player, SithikQuestDialogueFile())
                inInventory(player, ZogreQuestUtils.STRANGE_POTION) -> openDialogue(player, SithikIntsStrangePotionDialogueFile())
                getAttribute(player, ZogreQuestUtils.TALK_WITH_ZAVISTIC_DONE, false) -> openDialogue(player, SithikDialogueFiles())
        ...
        on(SITHIK_OGRE, IntType.SCENERY, "talk-to") { player, _ ->
            when (getVarbit(player, Vars.VARBIT_QUEST_SITHIK_OGRE_TRANSFORMATION) == 1) {
                getAttribute(player, ZogreQuestUtils.TALK_WITH_SITHIK_OGRE_DONE, false) -> openDialogue(player, SithikTalkAgainAfterTransformDialogueFile())
        ...
        on(CUPBOARD, IntType.SCENERY, "search") { player, _ ->
            if(getAttribute(player, ZogreQuestUtils.ASK_SITHIK_AGAIN, false)) {
                if (!inInventory(player, ZogreQuestUtils.NECROMANCY_BOOK) && freeSlots(player) < 1) {
                    sendDialogue(player, "You see an item in the cupboard, but you don't have space to put it in your inventory.")
                } else {
                    sendItemDialogue(player, ZogreQuestUtils.NECROMANCY_BOOK, "You find a book on Necromancy.")
                    addItemOrDrop(player, ZogreQuestUtils.NECROMANCY_BOOK, 1)
                }
            } else {
        ...
        on(WARDROBE, IntType.SCENERY, "search") { player, _ ->
            if (getAttribute(player, ZogreQuestUtils.ASK_SITHIK_AGAIN, false)) {
                if (!inInventory(player, ZogreQuestUtils.HAM_BOOK) && freeSlots(player) < 1) {
                    sendDialogue(player, "You see an item in the wardrobe, but you don't have space to put it in your inventory.")
                } else {
                    sendItemDialogue(player, ZogreQuestUtils.HAM_BOOK, "You find a book on Philosophy written by the 'Humans Against Monsters' leader, Johanhus Ulsbrecht.")
                    addItemOrDrop(player, ZogreQuestUtils.HAM_BOOK, 1)
                }
            } else {
            ...
            if(getAttribute(player, ZogreQuestUtils.ASK_SITHIK_AGAIN, false)) {
                when {
                    hasBoth -> {
                        ...
                        } else if (!inInventory(player, ZogreQuestUtils.PORTRAI_BOOK)) {
                            sendItemDialogue(player, ZogreQuestUtils.PORTRAI_BOOK, "You also find a book on portraiture.")
                            addItemOrDrop(player, ZogreQuestUtils.PORTRAI_BOOK, 1)
                        } else {
                    ...
                    hasPapyrus -> {
                        ...
                        if (freeSlots(player) < 3 && !inInventory(player, ZogreQuestUtils.PORTRAI_BOOK)) {
        ...
        onUseWith(IntType.ITEM, Items.TORN_PAGE_4809, ZogreQuestUtils.NECROMANCY_BOOK) { player, used, _ ->
            if (removeItem(player, used.asItem())) {
                sendDoubleItemDialogue(player, ZogreQuestUtils.NECROMANCY_BOOK, Items.TORN_PAGE_4809, "The torn page matches exactly the part where a torn out page is missing from the book. You feel sure that this page came from this book.")
                setAttribute(player, "/save:${ZogreQuestUtils.TORN_PAGE_ON_NECRO_BOOK}", true)
            } else {
        ...
        onUseWith(IntType.SCENERY, ZogreQuestUtils.QUEST_ITEMS, SITHIK) { player, used, _ ->
            var item = used.id
            when (item) {
                ZogreQuestUtils.PORTRAI_BOOK -> openDialogue(player, SithikIntsPortraitureBookDialogueFile())
                ZogreQuestUtils.HAM_BOOK -> openDialogue(player, SithikIntsHamBookDialogueFile())
                ZogreQuestUtils.NECROMANCY_BOOK -> {
                    if (getAttribute(player, ZogreQuestUtils.TORN_PAGE_ON_NECRO_BOOK, false)) {
                ...
                ZogreQuestUtils.REALIST_PORTRAIT,
                ZogreQuestUtils.UNREALIST_PORTRAIT -> openDialogue(player, SithikIntsUsedPortraitDialogueFile())
                ZogreQuestUtils.SIGNED_PORTRAIT -> openDialogue(player, SithikIntsSignedPortraitDialogueFile())
                ZogreQuestUtils.STRANGE_POTION -> openDialogue(player, SithikIntsStrangePotionDialogueFile())
            }
            return@onUseWith true
        }
        ...
        onUseWith(IntType.GROUNDITEM, ZogreQuestUtils.STRANGE_POTION, Items.CUP_OF_TEA_4838) { player, used, _ ->
            lock(player, 2)
            animate(player, 537)
            replaceSlot(player, used.asItem().index, Item(Items.SAMPLE_BOTTLE_3377))
            sendItemDialogue(player, ZogreQuestUtils.STRANGE_POTION, "You pour some of the potion into the cup. Zavistic said it may take some time to have an effect.")
            setAttribute(player, "/save:${ZogreQuestUtils.SITHIK_TURN_INTO_OGRE}", true)
            return@onUseWith true
        }
    ...
    override fun areaEnter(entity: Entity) {
        if (entity !is Player) {
            ...
            if (getAttribute(entity.asPlayer(), ZogreQuestUtils.SITHIK_TURN_INTO_OGRE, false)) {
