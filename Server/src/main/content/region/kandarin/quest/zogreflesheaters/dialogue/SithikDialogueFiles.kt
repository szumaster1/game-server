...
class SithikDialogueFiles : DialogueFile() {
    override fun handle(componentID: Int, buttonID: Int) {
        ...
        when (stage) {
            ...
            5 -> npc(FacialExpression.HALF_GUILTY, "I'm a scholarly student of the magical arts. When I was","younger I used to be an adventurer, probably just like","yourself. But I lost interest in the constant fighting,","looting and gaining abilities.").also { stage++ }
            6 -> npc(FacialExpression.CALM, "Instead I decided to focus my attention and time to","study the purer form of the lost arts.").also { stage++ }
            7 -> player("The lost arts? What are they?").also { stage++ }
            8 -> npc(FacialExpression.HALF_GUILTY, "Ignorant people call them the 'dark arts'. I'm talking","about Necromancy, the power to bring the dead back to","life - the power of the gods! Surely the most awesome","power known to man.").also { stage++ }
            9 -> player(FacialExpression.HALF_GUILTY, "Hmm, well I guess I must be an ignorant person then,","because bringing the dead back to life sounds very","unnatural.").also { stage = 3 }
            10 -> npc(FacialExpression.HALF_GUILTY, "I'm actually quite old and not so very well and I'd like","to get over this illness I have, then I'll return to my","very serious and important studies.").also { stage = 3 }
        }
    }
}
...
class SithikPermissionDialogueFile : DialogueFile() {
    override fun handle(componentID: Int, buttonID: Int) {
        npc = NPC(NPCs.SITHIK_INTS_2061)
        when (stage) {
            0 -> npc(FacialExpression.NEUTRAL, "Hey...who gave you permission to come in here!").also { stage++ }
            1 -> npc(FacialExpression.ANNOYED,"Get out, get out I say.").also { stage++ }
        ...
        when (stage) {
            ...
            4 -> if(!getAttribute(player!!, ZogreQuestUtils.ASK_SITHIK_AGAIN, false)) {
                ...
                when (buttonID) {
                    ...
                    3 -> playerl(FacialExpression.HALF_GUILTY, "Ok, thanks.").also { stage = END_DIALOGUE }
                }
            }
            5 -> npc(FacialExpression.HALF_GUILTY, "Er...undead ogres...no, sorry, no idea what you're","talking about there.").also { stage++ }
            6 -> playerl(FacialExpression.HALF_GUILTY, "Hmm, is that right...").also { stage++ }
            7 -> npcl(FacialExpression.HALF_GUILTY, "Well, yes, yes it is. If I knew something, I'd tell you.").also { stage++ }
            8 -> npc(FacialExpression.HAPPY, "Anyways, dead ogres you say? How strange? That must","be a strange sight?").also { stage++ }
            9 -> player(FacialExpression.HALF_GUILTY, "Very well, if you don't know anything about it, you","won't mind if I look around then?").also { stage++ }
            ...
            17 -> npc(FacialExpression.HALF_GUILTY, "Ignorant people call them the 'dark arts'. I'm talking","about Necromancy, the power to bring the dead back to","life - the power of the gods! Surely the most awesome","power known to man.").also { stage++ }
            18 -> player(FacialExpression.HALF_GUILTY, "Hmm, well I guess I must be an ignorant person then,","because bringing the dead back to life sounds very","unnatural.").also { stage = 3 }
...
class SithikIntsSignedPortraitDialogueFile : DialogueFile() {
    override fun handle(componentID: Int, buttonID: Int) {
        ...
        when (stage) {
            ...
            6 -> playerl(FacialExpression.HALF_THINKING, "Ok, let's say two million...two million to keep quiet and give me the picture.").also { stage++ }
            7 -> sendDoubleItemDialogue(player!!, Items.COINS_8897, Items.COINS_8897, "Sithik shows you a chest brimming over with coins...").also { stage++ }
            8 -> playerl(FacialExpression.HALF_WORRIED, "Oh...erm...well, that is a lot of money actually...er....").also { stage++ }
            ...
            11 -> when (buttonID) {
                1 -> player(FacialExpression.GUILTY, "No, I won't take the money, I'm going to bring you to justice!").also { stage++ }
                2 -> playerl("Ok, I'll shut up for two million!").also { stage = 13 }
            }

            12 -> npcl("Oh well, suit yourself! I wasn't going to give you the money anyway! No one will believe some crazy adventurer and an Inn keep.").also { stage = END_DIALOGUE }
            13 -> npcl(FacialExpression.LAUGH, "Ha! Ha! You believed me! I'm not going to give you all my money! No one will believe a crazy adventurer and a local Inn keep!").also { stage++ }
            14 -> player(FacialExpression.SAD, "You're a mean and cruel man Sithik, a mean and cruel man!").also { stage = END_DIALOGUE }
        }
    }
}
...
class SithikIntsStrangePotionDialogueFile : DialogueFile() {
    override fun handle(componentID: Int, buttonID: Int) {
        npc = NPC(NPCs.SITHIK_INTS_2061)
        when (stage) {
            0 -> playerl(FacialExpression.HAPPY, "Here, try some of this potion, it'll make you feel better!").also { stage++ }
            1 -> npc(FacialExpression.SCARED, "Err, yuck....no way am I taking any potions or", "medication off you...I don't trust you!").also { stage = END_DIALOGUE }
        }
    }
}
...
class SithikIntsAfterTransformDialogueFile : DialogueFile() {
    override fun handle(componentID: Int, buttonID: Int) {
        ...
        when (stage) {
            ...
            5 -> when (buttonID) {
                1 -> player("How do I remove the effects of the spell from the area?", "The ogres want to get their ceremonial dance area back", "and can't do that with undead walking all over it.").also { stage++ }
            ...
            6 -> npc(FacialExpression.OLD_DEFAULT, "Unfortunately you can't. The spell is permanent, it will", "last forever, the only option you have is to move the", "ceremonial area.").also { stage++ }
            7 -> player("You're an evil man and I'm going to make you pay for", "this...you can stay like that forever as far as I'm", "concerned.").also { stage++ }
            8 -> npc(FacialExpression.OLD_DEFAULT, "No...no, let me try to make amends...please I can help", "you. Just don't leave me like this.").also { stage = 4 }
            9 -> npc(FacialExpression.OLD_DEFAULT, "Ok, similar spells have been cast before and the only", "way to deal with the resulting creatures is to cordon off", "the area and not go in there again.").also { stage++ }
            10 -> npc(FacialExpression.OLD_DEFAULT, "The undead creatures usually manifest some sort of", "disease so it's best to attack them from a distance with a", "ranged weapon.").also { stage++ }
            11 -> npc(FacialExpression.OLD_DEFAULT, "Normal missiles like arrows and darts do very little", "damage to them because they're designed to destroy", "internal organs. This is a waste of time with undead", "creatures like undead ogres.").also { stage++ }
            12 -> player("Yeah, clearly so what should we use?").also { stage++ }
            13 -> npc(FacialExpression.OLD_DEFAULT, "From my research it looks like a flat ended arrow was", "designed called a 'Brutal arrow'. This does large", "amounts of crushing damage to the creature. You can", "make them by using larger arrows.").also { stage++ }
            14 -> npc(FacialExpression.OLD_DEFAULT, "I think some Ogre hunters make them. But instead of", "adding an arrowhead, you hammer a large nail into the", "end of the shaft.").also { stage = 4 }
            15 -> npc(FacialExpression.OLD_DEFAULT, "My research shows that two jungle based herbs can be", "used, one is found near river tributaries and looks like", "a vine, the other is found in caves and grows on the", "wall.").also { stage++ }
            16 -> npc(FacialExpression.OLD_DEFAULT, "It's quite well camouflaged so it's unlikely that you'll", "find it.").also { stage++ }
            17 -> player("We'll see about that!").also { stage = 4 }
            18 -> npc(FacialExpression.OLD_DEFAULT, "But...you can't just leave me here like this!").also { stage = END_DIALOGUE }
        }
    }
}
...
class SithikTalkAgainAfterTransformDialogueFile : DialogueFile() {
    override fun handle(componentID: Int, buttonID: Int) {
        npc = NPC(NPCs.SITHIK_INTS_2062)
        when (stage) {
            0 -> npc(FacialExpression.OLD_DEFAULT, "Arghhhh..what do you want now...you've turned me into", "a beast!").also { stage++ }
            1 -> player("I've got some questions for you...and you'd", "better answer them well or else!").also { stage++ }
            2 -> npc(FacialExpression.OLD_DEFAULT, "Ok, ok, I'll tell you anything, just turn me back into a", "human again!").also { stage++ }
