...
class WaterfallPlugin : OptionHandler() {
    ...
    override fun handle(player: Player, node: Node, option: String): Boolean {
        ...
        when (id) {
            37247 -> {
                ...
                player.pulseManager.run(object : Pulse(2, player) {
                    override fun pulse(): Boolean {
                        if ((player.equipment.containsAtLeastOneItem(295) || player.inventory.contains(295, 1)) || player.getQuestRepository().isComplete("Waterfall")) {
            ...
            2014 -> if (quest.getStage(player) != 100) {
                player.packetDispatch.sendMessage("The cavern floods!")
                player.lock(6)
                player.pulseManager.run(object : Pulse(5, player) {
                    override fun pulse(): Boolean {
            ...
            1999 -> if (inInventory(player, 298, 1) || quest.getStage(player) < 30) {
            ...
            2002 -> {
                if (player.location == Location(2568, 9893)) {
                    ...
                    if (quest.getStage(player) >= 100 && node.location == Location(2566, 9901) && player.location == Location(2566, 9901)) {
            ...
            33046 -> SceneryBuilder.add(Scenery(33047, Location.create(2530, 9844, 0), 10, 1))
            42319 -> if (node.location == Location(2556, 9844)) {
            ...
            33047 -> when (option) {
                ...
                "close" -> SceneryBuilder.add(Scenery(33046, Location.create(2530, 9844, 0), 10, 1))
            }
            ...
            1990 -> if (inInventory(player, 293, 1) || quest.getStage(player) < 30) {
                sendMessage(player, "You search the crate and find nothing.")
            ...
            1991 -> if (player.location.y >= 9576) {
                handleAutowalkDoor(player, (node as Scenery))
                ...
                player.dialogueInterpreter.sendDialogues(306, FacialExpression.OLD_DEFAULT, "Hello? Ah yes, the door is still locked.", "If you want to get in here, you'll need to find the key", "that I hid in some crates in the eastern room.")
            }

            1989 -> if (player.inventory.contains(292, 1) || quest.getStage(player) != 20) {
            ...
            1987 -> {
                if (quest.getStage(player) >= 10) {
                    ...
                    player.pulseManager.run(object : Pulse(12, player) {
                        override fun pulse(): Boolean {
    ...
    fun handleObjects(add: Boolean, player: Player?) {
        if (add) {
            ...
            ROPES.clear()
            SceneryBuilder.add(Scenery(1996, Location.create(2512, 3468, 0), 10, 0))
        }
    }
    ...
    inner class WaterfallUseWithHandler : UseWithHandler(ROPE.id, KEY.id, PEBBLE.id, KEY_2.id, AMULET.id, URN.id, AIR_RUNE.id, EARTH_RUNE.id, WATER_RUNE.id) {
        ...
        override fun handle(event: NodeUsageEvent): Boolean {
            ...
            if (useditem.id == PEBBLE.id && scenery.id == 1992 && ItemDefinition.canEnterEntrana(player)) {
                ...
                player.pulseManager.run(object : Pulse(13, player) {
                    override fun pulse(): Boolean {
            ...
            if ((useditem.id == AIR_RUNE.id || useditem.id == EARTH_RUNE.id || useditem.id == WATER_RUNE.id) && scenery.id == 2004) {
                if (player.inventory.contains(555, 6) && player.inventory.contains(556, 6) && player.inventory.contains(557, 6)) {
            ...
            if (useditem.id == AMULET.id && scenery.id == 2006 && quest.getStage(player) != 100 && player.location != Location(2603, 9914)) {
                if (player.getAttribute<Any?>("waterfall_placed_runes") == null) {
