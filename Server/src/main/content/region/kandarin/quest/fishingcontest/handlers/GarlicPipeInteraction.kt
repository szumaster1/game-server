package content.region.kandarin.quest.fishingcontest.handlers
...
class GarlicPipeInteraction : PluginInteraction() {
    ...
    override fun handle(player: Player, event: NodeUsageEvent): Boolean {
        if (event.used is Item && event.usedWith is Scenery) {
            ...
            if (used.id == Items.GARLIC_1550 && usedWith.id == 41 && usedWith.location == Location.create(2638, 3446, 0) && player.getQuestRepository().getStage("Fishing Contest") > 0) {
                player.pulseManager.run(object : MovementPulse(player, usedWith.location.transform(0, -1, 0)) {
    ...
    override fun handle(player: Player, node: Node): Boolean {
        if (node is Scenery) {
            ...
            if (scenery.id == 41 && scenery.location == Location.create(2638, 3446, 0) && player.getAttribute("fishing_contest:garlic", false)) {
                player.pulseManager.run(object : MovementPulse(player, scenery.location.transform(0, -1, 0)) {
                    override fun pulse(): Boolean {
