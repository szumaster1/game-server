    ...
    override fun handle(player: Player, npc: NPC, option: Option): Boolean {
        ...
        if (!player.getAttribute("fishing_contest:fee-paid", false)) {
            if (npcLocation == Location(2637, 3444, 0)) {
                player.pulseManager.run(object : MovementPulse(player, npc.location.transform(1, 0, 0)) {
                    override fun pulse(): Boolean {
                        player.dialogueInterpreter.sendDialogues(3677, FacialExpression.NEUTRAL, "I think you will find that is", "my spot.")
                        return true
                    }
                }, PulseType.STANDARD)
                return true
                ...
                player.pulseManager.run(object : MovementPulse(player, npc.location.transform(1, 0, 0)) {
                    override fun pulse(): Boolean {
                        player.dialogueInterpreter.sendDialogues(225, FacialExpression.NEUTRAL, "Hey, you need to pay to enter the", "competition first! Only 5gp entrance fee!")
                        return true
                    }
                }, PulseType.STANDARD)
                return true
                ...
                player.pulseManager.run(object : MovementPulse(player, npc.location.transform(1, 0, 0)) {
                    override fun pulse(): Boolean {
                        player.dialogueInterpreter.sendDialogues(228, FacialExpression.NEUTRAL, "I think you will find that is my spot.")
                        return true
                    }
                }, PulseType.STANDARD)
                return true
