...
class VineInteraction : PluginInteraction() {
    ...
    override fun handle(player: Player, node: Node): Boolean {
        if (node is Scenery) {
            if (player.getQuestRepository().getStage("Fishing Contest") in 1..99) {
                player.pulseManager.run(object : MovementPulse(player, node.asScenery().location.transform(0, 0, 0)) {
