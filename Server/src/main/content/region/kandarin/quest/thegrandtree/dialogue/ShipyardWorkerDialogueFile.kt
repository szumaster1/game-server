 ...
 *
 * @constructor Shipyard worker dialogue file
 */
class ShipyardWorkerDialogueFile : DialogueFile() {
    ...
    override fun handle(componentID: Int, buttonID: Int) {
        when (stage) {
            ...
            17 -> npc(FacialExpression.HALF_ASKING, "Yeah?").also { stage++ }
            18 -> player("Yeah!").also { stage++ }
            19 -> npc(FacialExpression.CALM, "...").also { stage++ }
            20 -> player(FacialExpression.CALM, "...").also { stage++ }
            21 -> player(FacialExpression.ASKING, "So are you going to let me in then?").also { stage++ }
            22 -> npcl("No.").also { stage++ }
            23 -> player("...").also { stage++ }
            24 -> npcl("...").also { stage++ }
            25 -> player(FacialExpression.HALF_ASKING, "You bored yet?").also { stage++ }
            ...
            32 -> npc("I'm not starting that again. Maybe if", "I ignore you you'll go away...").also { stage = END_DIALOGUE }
