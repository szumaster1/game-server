...
import content.region.kandarin.quest.zogre.npc.BrentleVahnNPC.Companion.spawnHumanZombie
import content.region.kandarin.quest.zogre.npc.SlashBashNPC.Companion.spawnZogreBoss
import core.api.*
import cfg.consts.*
...
class GuTanothCryptListeners : InteractionListener {
    ...
    override fun defineListeners() {
        ...
        /*
            Stairs to underground crypt and to boss.
         */
                ...
                override fun pulse(): Boolean {
                    when (counter++) {
                        ...
                        3 -> {
                            ...
                            teleport(player, if(node.location == Location(2443,9417,2)) Location(2442, 9418, 0) else Location(2477, 9437, 2))
                        }
                    }
                    return false
                }
        ...
        /*
            Surface to base floor from boss cave and Jiggig.
         */

        on(STAIRS_TO_JIGGIG, IntType.SCENERY, "climb-up") { player, node ->
            teleport(player, if(node.location == Location(2443,9417,0)) Location(2446, 9417, 2) else Location(2485, 3045, 0))
            sendMessage(player, "You climb up the steps.")
            return@on true
        }

        /*
            Interaction with broken lecturn inside Charred area.
         */
        ...
        on(BROKEN_LECTURN, IntType.SCENERY, "search") { player, _ ->
            ...
            if (inInventory(player, TORN_PAGE) || getAttribute(player, ZogreQuestUtils.TORN_PAGE_ACQUIRED, false)) {
                sendMessage(player, "You find nothing here this time.")
            } else {
                setAttribute(player, ZogreQuestUtils.TORN_PAGE_ACQUIRED, true)
        ...
        /*
            Search the Skeleton scenery.
         */

        on(SKELETON, IntType.SCENERY, "search") { player, _ ->
            if (getAttribute(player, ZogreQuestUtils.ZOMBIE_NPC_ACTIVE, false)) {
        ...
        /*
            Talk-to interaction after search the skeleton.
         */
        ...
        /*
            Looting ogre coffins by pick-lock interaction.
         */

        on(COFFIN, IntType.SCENERY, "pick-lock") { player, node ->
            val sceneryId = node.asScenery()
            val loot = OgreCoffinLootTable.forId(sceneryId.id)?.roll()?.firstOrNull() ?: return@on false

            if (getVarbit(player, Vars.VARBIT_QUEST_ZORGE_FLESH_EATERS_PROGRESS) < 13) {
                sendNPCDialogue(player, NPCs.OGRE_GUARD_2042, "Yous best not touch dem till yous chat wiv Grish - use look at dem again more when yous do stuff for Grish.", FacialExpression.OLD_DEFAULT)
                return@on false
            }
        ...
        /*
            Looting ogre coffins by opening them with key.
         */
        ...
        /*
            Search the quest coffin, first time.
         */
                ...
                override fun handle(componentID: Int, buttonID: Int) {
                    when (stage) {
                        ...
                        2 -> {
                            ...
                            setAttribute(player, "/save:${ZogreQuestUtils.ZOMBIE_NPC_ACTIVE}", true)
                ...
                override fun handle(componentID: Int, buttonID: Int) {
                    when (stage) {
                        ...
                        5 -> {
                            ...
                            setAttribute(player, "/save:${ZogreQuestUtils.BLACK_PRISM_ACQUIRED}", 0)
                            setVarbit(player, Vars.VARBIT_QUEST_ZOGRE_COFFIN_TRANSFORM, 3, true)
                        }
                    }
                }
            })
        ...
        /*
            Search the quest coffin after used knife.
         */

        on(QUEST_COFFIN_2, IntType.SCENERY, "search") { player, _ ->
            if (inInventory(player, BLACK_PRISM) || getAttribute(player, ZogreQuestUtils.BLACK_PRISM_ACQUIRED, 0) > 3) {
                ...
                player.incrementAttribute(ZogreQuestUtils.BLACK_PRISM_ACQUIRED)
                setVarbit(player, Vars.VARBIT_QUEST_ZORGE_FLESH_EATERS_PROGRESS, 3, true)
                ...
                override fun handle(componentID: Int, buttonID: Int) {
                    when (stage) {
                        ...
                        1 -> {
                            ...
                            setAttribute(player, ZogreQuestUtils.DRAGON_TANKARD_ACQUIRED, true)
                        }
        ...
        /*
            Dragon tankard item interaction
         */
        ...
        /*
            Ogre doors scenery interaction.
         */

        on(OGRE_DOORS, IntType.SCENERY, "open") { player, node ->
            if(!inInventory(player, Items.OGRE_GATE_KEY_4839) || !getAttribute(player, ZogreQuestUtils.RECEIVED_KEY_FROM_GRISH, false)) {
        ...
        on(STAND, IntType.SCENERY, "search") { player, _ ->
            ...
            when{
                getAttribute(player, ZogreQuestUtils.SLASH_BASH_ACTIVE, false) -> return@on false
                else -> if(getVarbit(player, Vars.VARBIT_QUEST_ZORGE_FLESH_EATERS_PROGRESS) >= 8) spawnZogreBoss(player)
            }
            return@on true
        }
