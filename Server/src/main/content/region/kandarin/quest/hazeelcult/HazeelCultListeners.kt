...
import content.region.kandarin.quest.hazeelcult.npc.AlomoneNPC
import core.api.*
...
class HazeelCultListeners : InteractionListener {
    companion object {
        ...
        // Animate 801 zombie comes out of the coffin.
    }
    ...
    override fun defineListeners() {
        ...
        on(SEWERS, IntType.SCENERY, "turn-left") { player, node ->
            ...
            if (getQuestStage(player, "Hazeel Cult") >= 1) {
                lock(player, 4)
        ...
        on(SEWER_1, IntType.SCENERY, "turn-left") { player, node ->
            ...
            if (getQuestStage(player, "Hazeel Cult") >= 1 && getAttribute(player, SEWER_LEFT, 0) == 0) {
                setAttribute(player, SEWER_LEFT, 1)
            }
            if (getQuestStage(player, "Hazeel Cult") == 10 && getAttribute(player, SEWER_RIGHT_2, 0) == 1) {
        ...
        on(SEWERS, IntType.SCENERY, "turn-right") { player, node ->
            ...
            } else if (getQuestStage(player, "Hazeel Cult") == 10) {
                if (!player.location.withinDistance(node.asScenery().location, 2)) return@on true
                for (i in SEWERS) when (i) {
            ...
            lock(player, 4)
            animate(player, TURN_VALVE_ANIMATION)
            player.dialogueInterpreter.sendDialogue("Turn the large metal valve to the right. Beneth your feet you", "can heat the sudden sound of rushing water from the sewer.")
            return@on true
        }
        ...
        on(CRATE, IntType.SCENERY, "search") { player, _ ->
            if (getQuestStage(player, "Hazeel Cult") >= 1 && getAttribute(player, MAHJARRAT, true) && !getAttribute(player, CARNILLEAN, true) && freeSlots(player) > 1) {
        ...
        on(Scenery.CHEST_2857, IntType.SCENERY, "Close") { _, _ ->
            ...
            removeScenery(core.game.node.scenery.Scenery(Scenery.CHEST_2857, Location(2565, 3272, 2)))
            return@on true
        }
        ...
        fun hazeelScroll(player: Player) {
            ...
            setInterfaceText(player, scroll.joinToString("<br>"), Components.BLANK_SCROLL_222, 2)
        }
