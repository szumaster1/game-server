...
class TempleOfIkovListeners : InteractionListener {
    ...
    override fun defineListeners() {
        ...
        onUseWith(SCENERY, Items.SHINY_KEY_85, Scenery.DOOR_99) { player, _, with ->
        ...
        on(intArrayOf(Scenery.GATE_94, Scenery.GATE_95), SCENERY, "open") { player, node ->
            if (inEquipment(player, Items.PENDANT_OF_LUCIEN_86)){
                if(getQuestStage(player, "Temple of Ikov") == 1) {
                    setQuestStage(player, "Temple of Ikov", 2)
                }
                DoorActionHandler.handleAutowalkDoor(player, node.asScenery())
            } else {
        ...
        on(Scenery.LEVER_91, SCENERY, "pull") { player, node ->
            ...
            if (getAttribute(player, TempleOfIkov.attributeDisabledTrap, false)) {
                if(getQuestStage(player, "Temple of Ikov") == 2) {
                    setQuestStage(player, "Temple of Ikov", 3)
                }
            } else {
        ...
        on(intArrayOf(Scenery.GATE_89, Scenery.GATE_90), SCENERY, "open") { player, node ->
            if (getAttribute(player, TempleOfIkov.attributeIceChamberAccess, false) || getQuestStage(player,
                    "Temple of Ikov"
                ) >= 4){
        ...
        // So clicking on the same one doesn't randomize it. Good luck and hug you.
        ...
        on(Scenery.DOOR_92, SCENERY, "open") { player, node ->
            ...
            if (getQuestStage(player, "Temple of Ikov") >= 3){
                DoorActionHandler.handleAutowalkDoor(player, node.asScenery())
            } else {
        ...
        on(Scenery.DOOR_93, SCENERY, "open") { player, node ->
            if (getQuestStage(player, "Temple of Ikov") >= 4){
                DoorActionHandler.handleAutowalkDoor(player, node.asScenery())
            } else {
        ...
        on(Items.STAFF_OF_ARMADYL_84, IntType.GROUNDITEM,"take") { player, node ->
            if (getQuestStage(player, "Temple of Ikov") >= 6 && getAttribute(player,
                    TempleOfIkov.attributeChosenEnding, 0) == 1){
            ...
            if (npcs.isNotEmpty()) {
                ...
                if(getQuestStage(player, "Temple of Ikov") == 5) {
                    setQuestStage(player, "Temple of Ikov", 6)
