 ...
 *
 * @constructor Guidor dialogue file
 */
class GuidorDialogueFile : DialogueFile() {
    override fun handle(componentID: Int, buttonID: Int) {
        ...
        when (stage) {
            ...
            16 -> {
                if (!anyInInventory(player!!, Items.SULPHURIC_BROLINE_417) && !inInventory(
                        player!!,
                        Items.ETHENEA_415
                    ) && !inInventory(player!!, Items.LIQUID_HONEY_416) && !inInventory(player!!, Items.TOUCH_PAPER_419)
                ) {
                    end()
