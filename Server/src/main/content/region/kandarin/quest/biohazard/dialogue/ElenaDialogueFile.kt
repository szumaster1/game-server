        ...
        when {
            (questStage == 0) -> {
                when (stage) {
                    ...
                    4 -> npc("My distillator, I can't test any plague samples without it.", "They're holding it in the mourner quarters in West", "Ardougne.").also { stage++ }
                    5 -> npc("I must somehow retrieve that distillator if I am to find", "a cure for this awful affliction.").also { stage++ }
                    ...
                    9 -> npc("I was hoping you would say that. Unfortunately they", "discovered the tunnel and filled it in. We need another", "way over the wall.").also { stage++ }
                    10 -> player("Any ideas?").also { stage++ }
                    11 -> npc("My father's friend Jerico is in communication with", "West Ardougne. He might be able to help us, he lives", "next to the chapel.").also { stage++ }
                    12 -> {
                        end()
            ...
            (questStage == 1) -> {
                when (stage) {
                    ...
                    3 -> npc("Speak to Jerico, he will help you to cross the", "wall. He lives next to the chapel.").also { stage++ }
                    4 -> end()
                }
            }
            ...
            (questStage == 4) -> {
                when (stage) {
                    ...
                    1 -> npc("Yes, I saw. Quickly meet with Jerico's friends", "and cross the wall before the pigeons fly off.").also { stage++ }
                    2 -> end()
                }
            }
            ...
            (questStage in 10..15) -> {
                when (stage) {
                    ...
                    7 -> npc("Well, yes and no. The liquid honey isn't worth much, ", "but the others are. Especially this colourless ethenea. Be", "careful with the sulphuric broline, it's highly poisonous.").also { stage++ }
                    ...
                    10 -> npc("You'll need to go and see my old mentor Guidor. He", "lives in Varrock. Take these vials and this sample to", "him.").also { stage++ }
                    11 -> npc("But first you'll need some more touch paper. Go and", "see the chemist in Rimmington.").also { stage++ }
                    12 -> npc("Just don't get into any fights, and be careful who", "you speak to.").also { stage++ }
                    13 -> npc("Those vials are fragile, and plague carriers don't", "tend to be too popular.").also { stage++ }
                    14 -> {
                        end()
            ...
            getAttribute(player!!, BiohazardUtils.ELENA_REPLACE, false) -> {
                when (stage) {
                    ...
                    1 -> showTopics(Topic(FacialExpression.HALF_GUILTY, "I just find it hard to say goodbye sometimes.", 100), Topic(FacialExpression.GUILTY, "I'm afraid i've lost some of the stuff that you gave me...", 200), Topic(FacialExpression.HALF_ASKING, "I've forgotten what I need to do.", 300))
