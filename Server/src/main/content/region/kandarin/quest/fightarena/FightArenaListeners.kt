...
import content.region.kandarin.quest.fightarena.cutscenes.JeremyRescueCutscene
...
class FightArenaListeners : InteractionListener {
    ...
    override fun defineListeners() {
        ...
        on(A_LAZY_GUARD_1, SCENERY, "talk-to") { player, node ->
            openDialogue(player, ALazyGuardDialogueFile(), node)
            return@on true
        }
        ...
        onUseWith(IntType.SCENERY, CELL_KEY, CELL_DOOR_2) { player, _, _ ->
            if (getQuestStage(player, "Fight Arena") == 88) {
                JailAfterFightCutscene(player).start()
