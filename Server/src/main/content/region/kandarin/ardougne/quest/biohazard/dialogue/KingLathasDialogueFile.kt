...
class KingLathasDialogueFile : DialogueFile() {
    ...
    override fun handle(componentID: Int, buttonID: Int) {
        ...
        when {
            (questStage >= 16) -> {
                when (stage) {
                    0 -> player(FacialExpression.SUSPICIOUS, "I assume that you are the King of East Ardougne?").also { stage++ }
                    1 -> npc(FacialExpression.ANNOYED, "You assume correctly, but where do you get such", "impertinence.").also { stage++ }
                    ...
                    12 -> end()
                    13 -> npc("Their King, Tyras, journeyed out to the West on a", "voyage of discovery. But he was captured by the Dark", "Lord.").also { stage++ }
                    14 -> npc("The Dark Lord agreed to spare his life, but only on", "one condition... That he would drink from the Chalice of", "Eternity.").also { stage++ }
                    15 -> player("So what happened?").also { stage++ }
                    16 -> npc("The chalice corrupted him. He joined forces with the", "Dark Lord, the embodiment of pure evil, banished all", "those years ago...").also { stage++ }
                    17 -> npc("And so I erected this wall, not just to protect my", "people, but to protect all the people of Gielinor.").also { stage++ }
                    18 -> npc("Now, with the King of West Ardougne, the Dark Lord", "has an ally on the inside.").also { stage++ }
                    19 -> npc("So I'm sorry that I lied about the plague. I just hope", "that you can understand my reasons.").also { stage++ }
                    20 -> player("Well at least I know now, but what can we do about it?").also { stage++ }
                    21 -> npc("Nothing at the moment, I'm waiting for my scouts to", "come back. They will tell us how we can get through", "the mountains.").also { stage++ }
                    ...
                    25 -> npc("It's located just to the north west of Ardougne, there", "you can prepare for the challenge ahead.").also { stage++ }
                    26 -> player("Ok. There's just one thing I don't understand, how do", "you know so much about King Tyras?").also { stage++ }
                    27 -> npc("How could I not do? He was my brother.").also { stage++ }
                    28 -> {
                        end()
                        finishQuest(player!!, "Biohazard")
                    }
                }
            }
        }
