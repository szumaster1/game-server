...
class PenguinKeeperDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun open(vararg args: Any?): Boolean {
        npc = args[0] as NPC
        playerl(FacialExpression.HALF_ASKING, "Hello there. How are the penguins doing today?").also { stage = 0 }
        return true
    }
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        ...
        when (stage) {
            ...
            7 -> npcl(
                FacialExpression.FRIENDLY,
                "Fair enough. The offer still stands if you change your mind."
            ).also { stage = END_DIALOGUE }

            8 -> npcl(FacialExpression.FRIENDLY, "Wonderful!").also { stage++ }
            9 -> npcl(
                FacialExpression.FRIENDLY,
                "Here you go - this egg will hatch into a baby penguin."
            ).also { stage++ }
