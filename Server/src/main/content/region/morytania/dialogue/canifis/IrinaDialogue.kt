...
@Initializable
class IrinaDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        ...
        if (inEquipment(player, Items.RING_OF_CHAROS_4202) || inEquipment(player, Items.RING_OF_CHAROSA_6465)) {
            when (randomWithRing) {
                ...
                5 -> npc(FacialExpression.HALF_GUILTY, "I bet you have wonderful paws.").also { stage = END_DIALOGUE }6 -> npc(FacialExpression.HALF_GUILTY, "Seen any humans around here?", "I'm v-e-r-y hungry.").also { stage = END_DIALOGUE }
                7 -> npc(FacialExpression.HALF_GUILTY, "Good day to you, my friend.").also { stage = END_DIALOGUE }8 -> npc(FacialExpression.HALF_GUILTY, "You smell familiar...").also { stage = END_DIALOGUE }
    ...
    override fun newInstance(player: Player): Dialogue {
        return IrinaDialogue(player)
    }

    override fun getIds(): IntArray {
        return intArrayOf(NPCs.IRINA_6035)
    }
