...
@Initializable
class BrosDialogue(player: Player? = null) : Dialogue(player) {

    /*
     *  Info: Citizen of Canifis.
     *  Location: Boris: 3489,3490 | Lev: 3502,3487
     */
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        ...
        if (inEquipment(player, Items.RING_OF_CHAROS_4202) || inEquipment(player, Items.RING_OF_CHAROSA_6465)) {
            when (randomWithRing) {
                0 -> npc(FacialExpression.HALF_GUILTY, "I haven't smelt you around here before...").also {
                    stage = END_DIALOGUE
                }

                1 -> npc(FacialExpression.HALF_GUILTY, "If you catch anyone promise me you'll share.").also {
                    stage = END_DIALOGUE
                }
            ...
            when (randomWithoutRing) {
                ...
                20 -> player(FacialExpression.STRUGGLE, "Oh! Er... I just ate one is why!").also {
                    stage = END_DIALOGUE
                }
            }
        }
    ...
    override fun newInstance(player: Player): Dialogue {
        return BrosDialogue(player)
    }

    override fun getIds(): IntArray {
        return intArrayOf(NPCs.BORIS_6026, NPCs.LEV_6032)
    }
