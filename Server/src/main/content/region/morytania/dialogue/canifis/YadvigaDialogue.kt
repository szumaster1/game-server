...
@Initializable
class YadvigaDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        ...
        if (inEquipment(player, Items.RING_OF_CHAROS_4202) || inEquipment(player, Items.RING_OF_CHAROSA_6465)) {
            ...
            when (randomWithoutRing) {
                10 -> npc(
                    FacialExpression.HALF_GUILTY, "I have no interest in talking to a pathetic", "meat bag like yourself.").also { stage = END_DIALOGUE }
    ...
    override fun newInstance(player: Player): Dialogue {
        return YadvigaDialogue(player)
    }

    override fun getIds(): IntArray {
        return intArrayOf(NPCs.YADVIGA_6041)
    }
