package content.region.morytania.quest.druidspirit.dialogue

import cfg.consts.Items
import content.region.morytania.quest.druidspirit.BlessingPulse
import core.api.*
    ...
    override fun handle(componentID: Int, buttonID: Int) {
        ...
        if (questStage <= 5) {
            when (stage) {
                ...
                24 -> playerl(FacialExpression.FRIENDLY, "I'll do my very best, don't worry, if he's in there and he's still alive I'll definitely find him.").also {
                    stage = END_DIALOGUE; player!!.questRepository.getQuest("Nature Spirit").start(player!!)
                }
            }
            ...
            npcl(FacialExpression.NEUTRAL, "There you go my friend, you're now blessed. It's funny, now I look at you, there seems to be something of the faith about you. Anyway, good luck with your quest!").also {
                stage = END_DIALOGUE; player!!.questRepository.getQuest("Nature Spirit").setStage(player!!, 45)
            }
        } else if(questStage == 100) {
            when(stage){
                0 -> npcl(FacialExpression.HALF_GUILTY, "Greetings again adventurer. How go your travels in Morytania? Is it as evil as I have heard?").also { stage++ }
                1 -> playerl(FacialExpression.FRIENDLY, "I've lost my wolfbane dagger.").also { stage++ }
                2 -> npcl(FacialExpression.FRIENDLY,"Yes, I know! Luckily for you it washed up on the banks of the Salve earlier!").also { stage++ }
                3 -> if(freeSlots(player!!) == 0) {
                    end()
                    sendDialogue(player!!, "You don't have enough inventory space.")
                } else {
                    npcl(FacialExpression.FRIENDLY,"Here, take it again, but please try and be more careful this time.")
                    addItem(player!!, Items.WOLFBANE_2952)
                    stage = 4
                }
                4 -> npcl(FacialExpression.FRIENDLY, "It's a family heirloom after all!").also { stage++ }
                5 -> playerl(FacialExpression.FRIENDLY, "Thanks for that!").also { stage = END_DIALOGUE }
            }
        } else {
            when (stage) {
