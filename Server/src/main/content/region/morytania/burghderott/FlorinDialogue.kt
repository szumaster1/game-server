...
class FlorinDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            3 -> {
                npcl(FacialExpression.HALF_GUILTY, "Well met! My name's Florin, I do a bit of this, a bit of that. Just try to survive really, there's nothing much else we can do.")
                stage = 1
            }
            4 -> {
            ...
            6 -> {
                npcl(FacialExpression.HALF_GUILTY, "Not as far as I know, we have some basic amenities now which is so very useful, we're all incredibly grateful to you. I've never really seen or known a hero before, so I have nothing to compare one to, but I suspect in")
                stage++
            }
            7 -> {
                npcl(FacialExpression.HALF_GUILTY, "most people's judgement, you would be one! I for one am very pleased to have met you.")
                stage = 1
            }
        }
        return true
