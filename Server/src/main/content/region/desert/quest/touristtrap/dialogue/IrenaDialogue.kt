package content.region.desert.quest.touristtrap.dialogue
...
class IrenaDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun open(vararg args: Any): Boolean {
        ...
        when (quest!!.getStage(player)) {
            ...
            else -> npc(FacialExpression.HALF_CRYING, "*Sob* Have you found my daughter yet?")
        }
        return true
    }
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            900 -> {
                sendNPCDialogueLines(player,
                    823, FacialExpression.HALF_GUILTY,false,
        ...
        when (quest!!.getStage(player)) {
            ...
            0 -> when (stage) {
                0 -> if (quest!!.hasRequirements(player)) {
                    npc(FacialExpression.CRYING, "Boo hoo! Oh dear, my only daughter....")
                    stage++
                } else {
                    npc(FacialExpression.CRYING, "Boo hoo! Go away!")
                    stage = 99
                }
            ...
            10 -> when (stage) {
                ...
                1 -> {
                    npc(FacialExpression.HALF_CRYING, "Please! Hurry up.")
                    stage++
                }
