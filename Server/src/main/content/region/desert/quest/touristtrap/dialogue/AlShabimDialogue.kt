...
/**
 * Represents the Al shabim dialogue.
 */
class AlShabimDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (quest!!.getStage(player)) {
            54 -> when (stage) {
                0 -> if (player.inventory.containsItem(TouristTrap.PROTOTYPE_DART)) {
                ...
                22 -> if (player.inventory.containsItem(TouristTrap.TECHNICAL_PLANS)) {
            ...
            51, 52, 53 -> when (stage) {
                0 -> if (player.inventory.containsItem(TouristTrap.TECHNICAL_PLANS)) {
                    npc(
