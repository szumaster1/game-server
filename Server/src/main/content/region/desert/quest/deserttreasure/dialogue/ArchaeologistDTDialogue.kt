...
class ArchaeologistDTDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun open(vararg args: Any?): Boolean {
        ...
        if (!player.questRepository.hasStarted("Desert Treasure")) {
            if (player.questRepository.isComplete("Dig Site") &&
                player.questRepository.isComplete("The Tourist Trap") &&
                player.questRepository.isComplete("Temple of Ikov") &&
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            20 -> npc(FacialExpression.HALF_THINKING, "Well, it's funny you should say that.", "I'm not sure if I would really call it a quest as such,", "but I found this ancient stone tablet in one of my", "excavations, and it would really help me out if you").also { stage++ }
            21 -> npc(FacialExpression.FRIENDLY, "could go and take it back to the digsite for me and get", "it examined.").also { stage++ }
            22 -> npc(FacialExpression.NEUTRAL, "It's very old, and I don't recognise and of the", "inscriptions on it.").also { stage++ }
            23 -> options("Yes, I'll help you.", "No thanks, I don't want to help.").also { stage++ }
            24 -> when (buttonId) {
                1 -> player(FacialExpression.FRIENDLY, "Sure, I was heading that way anyways.", "Any particular person at the digsite you want me to", "talk to?").also { stage = 30 }
                // 2 -> todo
            }
            30 -> npc(FacialExpression.NEUTRAL, "His name's Terry Balando. Give it to nobody but him.", "I'm sorry, I can't entrust you with the actual tablet I", "found, but it is far too valuable to give away, but I took", "these etchings - they should be enough for him to make").also { stage++ }
            31 -> npc(FacialExpression.NEUTRAL, "a preliminary translation on.", "Come back and let me know what he says, I would hate", "to waste my time excavating anything that isn't worth", "my time as a world famous archaeologist!").also {
