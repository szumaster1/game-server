...
class PyramidAreaFirstThree: MapArea {
    ...
    override fun entityStep(entity: Entity, location: Location, lastLocation: Location) {
        if (entity is Player) {
            ...
            if ((1..30).random() == 1) {
                PyramidMapArea.spawnScarabs(entity)
            }

            if ((1..15).random() == 1) {
                // A mummy would jump out if you walk near a sarcophagus of 2 radius.
                val sarcoph = PyramidMapArea.nearSarcophagus(entity.location)
                if (sarcoph != null) {
                    PyramidMapArea.spawnMummy(entity, sarcoph)
                }
            }

            if (randomValue > averageLevel) {
                PyramidMapArea.trapdoorTrap(entity)
            }
        }
    }
}
