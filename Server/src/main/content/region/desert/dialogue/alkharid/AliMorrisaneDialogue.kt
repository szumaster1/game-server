...
class AliMorrisaneDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            0 -> playerl(FacialExpression.FRIENDLY, "Hello to you too.").also { stage++ }
            1 -> npcl(FacialExpression.ASKING, "My name is Ali Morrisane - the greatest salesman in the world.").also { stage++ }
            ...
            10 -> npcl(FacialExpression.FRIENDLY, "Well one can only do and sell so much. If I had more staff I'd be able to sell more").also { stage++ }
            11 -> npcl(FacialExpression.FRIENDLY, "rather than wasting my time on menial things I could get on with selling sand to the Bedabin and useless tourist trinkets to everyone.").also { stage++ }
            12 -> options("I'm far too busy - adventuring is a full time job you know.", "I'd like to help you but....").also { stage++ }
            13 -> when (buttonId) {
                1 -> playerl(FacialExpression.FRIENDLY, "I'm far too busy - adventuring is a full time job you know.").also { stage = 16 }
                2 -> playerl(FacialExpression.FRIENDLY, "I'd like to help you but.....").also { stage++ }
            }
            ...
            15 -> end()
            16 -> npcl(FacialExpression.FRIENDLY, "No problem my friend, perhaps another time.").also { stage++ }
            17 -> npcl(FacialExpression.FRIENDLY, "Anyway, have a look at my wares.").also { stage++ }
