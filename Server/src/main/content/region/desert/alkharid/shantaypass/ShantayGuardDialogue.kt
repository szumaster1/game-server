package content.region.desert.alkharid.shantaypass
...
class ShantayGuardDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun open(vararg args: Any): Boolean {
        ...
        if (npc != null && npc.id != 837) {
            npc(FacialExpression.HALF_GUILTY, "Hello there!")
            stage = 0
        } else {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            0 -> npc(FacialExpression.HALF_GUILTY, "What can I do for you?").also { stage++ }
            1 -> options("I'd like to go into the desert please.", "Nothing thanks.").also { stage++ }
            2 -> when (buttonId) {
                1 -> player(FacialExpression.HALF_GUILTY, "I'd like to go into the desert please.").also { stage = 10 }
                2 -> end()
            }
            10 -> npc(FacialExpression.HALF_GUILTY, "Of course!").also { stage++ }
            ...
            15 -> if (player.inventory.remove(PASS)) {
                val dest = if (player.location.y < 3304) Location.create(3303, 3117, 0) else Location.create(3305, 3117, 0)
