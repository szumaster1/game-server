...
class HAMBook : InteractionListener {
    ...
    private fun display(player: Player, pageNum: Int, buttonID: Int): Boolean {
        BookInterfaceListener.pageSetup(player, BookInterfaceListener.FANCY_BOOK_3_49,
            content.data.item.book.HAMBook.Companion.TITLE,
            content.data.item.book.HAMBook.Companion.CONTENTS
        )
        return true
    }
            ...
            if(player.interfaceManager.close()){
                openDialogue(player, object : DialogueFile() {
                    override fun handle(componentID: Int, buttonID: Int) {
                        when (stage) {
                            0 -> sendDialogue(player, "You read this book for a while, it seems to be some sort of political manifesto about how the king doesn't do enough to safeguard the citizens of the realm from the monsters that still thrive within the borders. It sends out a rallying cry to all people who would want to stop monsters, to join the HAM movement.").also { stage++ }
                            1 -> sendPlayerDialogue(player, "Hmmm, Sithik must really hate monsters then, I wonder if he hates ogres in particular?").also { stage = END_DIALOGUE }
                        }
                    }
                })
            }
