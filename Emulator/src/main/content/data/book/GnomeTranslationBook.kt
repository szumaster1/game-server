...
/**
 * Represents the Gnome translation book.
 */
class GnomeTranslationBook : InteractionListener {
    ...
    companion object {
        ...
        private fun display(player: Player, pageNum: Int, buttonID: Int): Boolean {
            BookInterfaceListener.pageSetup(player, BookInterfaceListener.FANCY_BOOK_3_49,
                content.data.book.GnomeTranslationBook.Companion.TITLE,
                content.data.book.GnomeTranslationBook.Companion.CONTENTS
            )
            return true
        }
    }
    ...
    override fun defineListeners() {
        on(Items.TRANSLATION_BOOK_784, IntType.ITEM, "read") { player, _ ->
            setAttribute(player, "bookInterfaceCallback", content.data.book.GnomeTranslationBook.Companion::display)
            setAttribute(player, "bookInterfaceCurrentPage", 0)
            content.data.book.GnomeTranslationBook.Companion.display(player, 0, 0)
            return@on true
        }
    }
}
