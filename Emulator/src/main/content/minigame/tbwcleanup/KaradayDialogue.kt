...
class KaradayDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            101 -> npcl(FacialExpression.FRIENDLY, "If you want me to make you a machete we can do a deal if you like.").also { stage = 102 }
            ...
            210 -> interpreter.sendItemMessage(Items.OPAL_MACHETE_6313, "Opal bladed machete - Safta doc needs the following to make<br> this machete: 3 opals - cut or uncut, 1 gout tuber plant<br> and 300 trading sticks.").also { stage = 211 }
            211 -> showTopics(
                Topic("Order the Opal machete.", 213,true),
                Topic("Go back to machete descriptions", 201,true)
            )
            213 -> if (tryToMakeMachete(player, Items.OPAL_MACHETE_6313, Items.OPAL_1609, Items.UNCUT_OPAL_1625, 300)) {
            ...
            214 -> npcl(FacialExpression.FRIENDLY, "Ok Bwana, come back when you have them.").also { stage = END_DIALOGUE }
            220 -> interpreter.sendItemMessage(Items.JADE_MACHETE_6315, "Jade bladed machete - Safta doc needs the following to make<br> this machete: 3 jades - cut or uncut, 1 gout tuber plant<br> and 600 trading sticks.").also { stage = 221 }
            221 -> { showTopics(
                Topic("Order the Jade machete.", 223,true),
                Topic("Go back to machete descriptions", 201,true))
            }
            223 -> {
                if (tryToMakeMachete(player, Items.JADE_MACHETE_6315, Items.JADE_1611, Items.UNCUT_JADE_1627, 600)) {
                    npcl(FacialExpression.FRIENDLY, "Here you are Bwana, take good care of it.").also { stage = END_DIALOGUE }
                } else {
            ...
            234 -> npcl(FacialExpression.FRIENDLY, "Ok Bwana, come back when you have them.").also { stage = END_DIALOGUE }
            300 -> when(random(1,6)) {
