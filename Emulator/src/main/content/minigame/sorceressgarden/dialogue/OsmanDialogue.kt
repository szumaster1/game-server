...
class OsmanDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun open(vararg args: Any): Boolean {
        ...
        when (quest!!.getStage(player)) {
            ...
            60 -> {
                npc("The prince is safe on his way home with Leela.", "You can pick up your payment from the chancellor.")
                stage = 0
                return true
            }
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (quest!!.getStage(player)) {
            ...
            20 -> when (stage) {
                ...
                2 -> end()
                10 -> {
                    npc("The prince is guarded by some stupid guards and a", "clever woman. The woman is our only way to get the", "prince out. Only she can walk freely about the area.")
                    stage = 11
                }

                11 -> {
                    npc("I think you will need to tie her up. One coil of rope", "should do for that. Then, disguise the prince as her to", "get him out without suspicion.")
                    stage = 12
                }
                ...
                13 -> {
                    npc("Only enough to fool the guards at a distance. Get a", "skirt like hers. Same colour, same style. We will only", "have a short time.")
                    stage = 14
                }

                14 -> {
                    npc("Get a blonde wig, too. that is up to you to make or", "find. Something to colour the skin of the prince.")
                    stage = 15
                }

                15 -> {
                    npc("My daughter and top spy, Leela, can help you. She has", "sent word that she has discovered where they are", "keeping the prince.")
                    stage = 16
                }
                ...
                17 -> {
                    ...
                    options("Explain the first thing again.", "What is the second thing you need?", "Okay, I better go find some things.")
                    stage = 50
                }

                20 -> {
                    npc("We need the key, or we need a copy made. If you can", "get some soft clay then you can copy the key...")
                    stage = 21
                }

                21 -> {
                    npc("...If you can convince Lady Keli to show it to you", "for a moment. She is very boastful.", "It should not be too hard.")
                    stage = 22
                }
                ...
                23 -> {
                    quest!!.setStage(player, 20)
                    options("What is the first thing I must do?", "What exactly is the second thing you need?", "Okay, I better go find some things.")
                    stage = 24
                }
                ...
                25 -> {
                    npc("May good luck travel with you. Don't forget to find", "Leela. It can't be done without her help.")
                    stage = 26
                }
            ...
            10 -> when (stage) {
                0 -> {
                    npc("Our prince is captive by the Lady Keli. We just need", "to make the rescue. There are two things we need", "you to do.")
                    stage = 1
                }
    ...
    fun handleSqirks(buttonId: Int) {
        when (stage) {
            0 -> if (!hasSqirks()) {
                options("Where do I get sq'irks?", "Why can't you get the sq'irks yourself?", "How should I squeeze the fruit?", "Is there a reward for getting these sq'irks?", "What's so good about sq'irk juice then?")
                stage = 200
            } else {
            ...
            300 -> if (hasSqirkJuice()) {
                ...
                sendDialogueLines(player, "Osman imparts some Thieving advice to", "you ( $experience Thieving experience points )", "as a reward for the sq'irk juice.")
                stage = 304
            } else {
    ...
    companion object {
        ...
        private val JUICES = intArrayOf(Items.SPRING_SQIRKJUICE_10848, Items.SUMMER_SQIRKJUICE_10849, Items.AUTUMN_SQIRKJUICE_10850, Items.WINTER_SQIRKJUICE_10851)
        private val FRUITS = intArrayOf(Items.SPRING_SQIRK_10844, Items.SUMMER_SQIRK_10845, Items.AUTUMN_SQIRK_10846, Items.WINTER_SQIRK_10847)
    }
}