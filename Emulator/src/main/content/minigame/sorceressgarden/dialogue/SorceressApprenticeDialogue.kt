...
class SorceressApprenticeDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun open(vararg args: Any): Boolean {
        ...
        if (player.getSavedData().globalData.hasSpokenToApprentice()) {
            player(FacialExpression.HALF_GUILTY, "Hey apprentice, do you want to try out", "your teleport skills again?")
        } else {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            0 -> npc(FacialExpression.HALF_GUILTY, "Okay, here goes - and remember, to", "return just drink from the fountain.").also { stage++ }
            1 -> { teleport(npc, player); end() }
    ...
    companion object {
        fun teleport(npc: NPC, player: Player) {
            if (!hasRequirement(player, QuestName.PRINCE_ALI_RESCUE)) return
