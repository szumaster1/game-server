...
class ElnockInquisitorDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            11 -> npc("Firstly you will need a butterfly net in which to catch", "them and at least one special impling jar to store an", "impling.").also { stage++ }
            12 -> npc("You will also require some experience as a Hunter", "since these creatures are elusive. The more immature", "implings require less experience, but some of the rarer", "implings are extraordinarily hard to find and catch.").also { stage++ }
            13 -> npc("Once you have caught one, you may break the jar", "open and obtain the object the impling is carrying.", "Alternatively, you may exchange certain combinations of", "jars with me. I will return the jars to my clients. In").also { stage++ }
            14 -> npc("exchange I will be able to provide you with some", "equipment that may help you hunt butterflies more", "effectively.").also { stage++ }
            15 -> npc("also beware. Those imps walking around the maze do", "not like the fact that their kindred spirits are being", "captured and will attempt to steal any full jars you have", "on you, setting the implings free.").also { stage = END_DIALOGUE }
            20 -> {
                end()
            ...
            30 -> {
                if (!player.getSavedData().activityData.isElnockSupplies) {
                    ...
                    npc("Since I have already given you some equipment for free,", "I'll be willing to sell you some now.").also { stage = 36 }
                }
            }
            31 -> npc("If you are ready to start hunting implings, then enter", "the main part of the maze.").also { stage++ }
            32 -> npc("Just push through the wheat that surrounds the centre", "of the maze and get catching!").also { stage = END_DIALOGUE }
            34 -> options("Yes, please.", "No, thanks.").also { stage++ }
            35 -> when (buttonId) {
                1 -> sendItemDialogue(player, Items.IMPLING_SCROLL_11273, "Elnock gives you a scroll. If you check it whilst in the maze, you will see how many of each impling you have captured.").also {
            ...
            38 -> if (inInventory(player, Items.COINS_995, 25000)) {
            ...
            39 -> if (!removeItem(player, Item(Items.COINS_995, 25000))) {
                player("Actually, I don't have that many coins at the moment.").also { stage = END_DIALOGUE }
            } else {
