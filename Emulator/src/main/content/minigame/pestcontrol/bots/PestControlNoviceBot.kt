...
import content.minigame.pestcontrol.PestControlHelper
...
class PestControlTestBot(l: Location) : PvMBots(legitimizeLocation(l)) {
    ...
    val myBoat = PestControlHelper.BoatInfo.NOVICE
    ...
    fun attackNPCs() {
        if (PestControlHelper.outsideGangplankContainsLoc(getLocation())) {
            ...
            )//.also { println("Novice - We think we is in pest control ${this.username}.") }
        }
