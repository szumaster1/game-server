...
import content.region.misc.keldagrim.dialogue.BlastFurnaceDoorDialogue
...
class BlastFurnaceListeners : InteractionListener {
    ...
    override fun defineListeners() {
        ...
        on(Items.SPADEFUL_OF_COKE_6448, ITEM, "empty") { p, n ->
            val animDuration = Animation(Animations.USE_SPADEFUL_OF_COKE_BLAST_FURNACE_2442).duration
            if (removeItem(p, n)) {
            ...
            openDialogue(p, object : DialogueFile() {
                override fun handle(componentID: Int, buttonID: Int) {
                    when (stage) {
                        ...
                        1 -> showTopics(
                            Topic("Yes.", 100, true), Topic("Nevermind.", END_DIALOGUE, true)
                        )
