...
class RechargeDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun open(vararg args: Any): Boolean {
        ...
        sendDialogueLines(player,"Mrrrh, how do you have this? You shouldn't.", "Nevertheless, I can recharge this for you.")
        return true
    }

    override fun handle(dialogId: Int, buttonId: Int): Boolean {
        when (stage) {
            0 -> sendDialogueOptions(player, "Recharge sceptre?", "Yes, please.", "No, thanks.").also { stage++ }
            1 -> when (buttonId) {
                1 -> sendDialogueOptions(player, "Recharge with?", "Clay/Ivory Artefacts(24)", "Stone Artefacts(12)", "Gold Artefacts(6)").also { stage = 20 }
                2 -> stage = 100
            }
            ...
            20 -> when (buttonId) {
                1 -> {
                    ...
                    var i = 0
                    while (i < 24) {
                        player.inventory.remove(Item(player.inventory.getId(clayRecharge[i])), clayRecharge[i], true)
                        i++
                    }
                ...
                2 -> {
                    ...
                    var i = 0
                    while (i < 12) {
                        player.inventory.remove(Item(player.inventory.getId(stoneRecharge[i])), stoneRecharge[i], true)
                        i++
                    }
                    removeItem(player, usedSceptre)
                    addItem(player, Items.PHARAOHS_SCEPTRE_9044)
                ...
                3 -> {
                    ...
                    var i = 0
                    while (i < 6) {
                        player.inventory.remove(Item(player.inventory.getId(goldRecharge[i])), goldRecharge[i], true)
                        i++
                    }
