...
abstract class FlagsHandler : DialogueFile() {
    ...
    companion object {
        ...
        enum class FARMER_FLAG_LINES(val line: String) {
            FIND_FLAG("Ah, another flag to clear. Let's see what's there."), FIND_SEED("Ah! A seed. Points for everyone near me!"), NO_SEED(
                "Hmm, no seeds planted here, I'm afraid."
            ),
            KEEP_FLAG("I'll have to keep this 'ere flag. Sorry."), FIND_PLANT("Hmm. Looks like there's a plant here."), DEAD_PLANT(
        ...
        when (stage) {
            ...
            21 -> {
                ...
                if (flags > 0) {
                    if (!player!!.inventory.add(Item(Items.FLAG_12625, flags))) {
                        npcl(FacialExpression.OLD_NORMAL, lines[1])
                        stage = END_DIALOGUE
                    } else {
                        setAttribute(player!!, "/save:vinesweeper:stored-flags", 0)
                        npcl(FacialExpression.OLD_NORMAL, lines[2])
                        stage++
                    }
                } else {
                    stage++
            ...
            22 -> {
                ...
                if (flags < 10) {
                    ...
                    npcl(FacialExpression.OLD_NORMAL, String.format(lines[3], price))
                    stage = 220
                } else {
                    stage = 23
                    handle(componentID, buttonID)
            ...
            222 -> {
                ...
                if (player!!.inventory.containsItem(price) && player!!.inventory.remove(price)) {
                    if (player!!.inventory.add(Item(Items.FLAG_12625, 10 - flags))) {
                        npcl(FacialExpression.OLD_NORMAL, lines[4])
                        stage = 22
                        stage = END_DIALOGUE
                    } else {
                        npcl(FacialExpression.OLD_NORMAL, lines[5])
                        // Refund the coins, can't fail because we just removed them
                        player!!.inventory.add(price)
                        stage = END_DIALOGUE
                    }
                } else {
                    npcl(FacialExpression.OLD_NORMAL, lines[6])
                    stage = END_DIALOGUE
                }
            }

            223 -> npcl(FacialExpression.OLD_NORMAL, lines[7]).also { stage = END_DIALOGUE }
            23 -> npcl(FacialExpression.OLD_NORMAL, lines[8]).also { stage = END_DIALOGUE }
        }
    }
}
