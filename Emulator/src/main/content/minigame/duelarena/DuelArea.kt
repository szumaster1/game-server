...
@JvmOverloads constructor(
    ...
    fun duel(session: DuelSession) {
        ...
        Pulser.submit(object : Pulse(4, session.player, session.other) {
            var count = 0
    ...
    override fun move(entity: Entity, from: Location, to: Location): Boolean {
        if (entity.isPlayer) {
    ...
    override fun actionButton(player: Player, interfaceId: Int, buttonId: Int, slot: Int, itemId: Int, opcode: Int): Boolean {
        ...
        if (inter != null && interfaceId == inter.id) {
            when (buttonId) {
                8, 10 -> if (session.hasRule(DuelRule.NO_SPECIAL_ATTACKS)) {
        ...
        when (interfaceId) {
            ...
            271 -> if (session.hasRule(DuelRule.NO_PRAYER)) {
            ...
            430 -> if (session.hasRule(DuelRule.NO_MAGIC)) {
                player.sendMessage("Use of prayer has been turned off for this duel.")
                return true
            }
        }
    ...
    companion object {
        ...
        private val FIGHT_OPTION = Option("Fight", 0).setHandler(object : OptionHandler() {
            override fun handle(player: Player, node: Node, option: String): Boolean {
