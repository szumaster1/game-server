...
class CastleWarsListeners : InteractionListener {
    ...
    override fun defineListeners() {
        ...
        on(CastleWars.cwTableItemRewardMap.keys.toIntArray(), SCENERY, "take-from") { player, node ->
            ...
            if (addItem(player, rewardItem)) {
                ...
                val formattedItemName =
                    (ItemDefinition.forId(rewardItem).name.lowercase() + "s.") // Get the formatted item name string
                        .replace("pes.", "pe.")                                     // Replacement for ropes. -> rope.
                        .replace(
                            "bronze ", ""
                        )                                     // Replacement for bronze pickaxes. -> pickaxes.
        ...
        onUseWith(
            ...
            replaceScenery(
                wall.asScenery(), CastleWars.cwCastleBattlementsMap.getValue(wall.id), CastleWars.ropeAliveTicks
            )
    ...
    private fun hasNonCombatItems(container: Container): Boolean {
        for (item in container.toArray()) {
            if (item?.id != null && (item.id == Items.COINS_995 || item.definition?.noteId == item.id)) return true
        }
        return false
    }
