 ...
 * @property staff      The staff associated with the talisman.
 * @property tiara      The id of the tiara associated with the talisman.
 */
enum class TalismanStaff(val items: Item, val staff: Staff, val tiara: Int) {
    AIR(
    ...
    companion object {
         ...
         * @return The corresponding [TalismanStaff] instance or `null` if not found.
         */
        infix fun from(item: Item): TalismanStaff? {
            return TalismanStaff.values().firstOrNull { it.items.id == item.id }
        }
    }
}
