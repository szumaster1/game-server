...
class DeadfallSetting : TrapSetting(intArrayOf(28935, 19205), arrayOf(Item(946)), intArrayOf(28937, 19206), intArrayOf(10138, 6006, 12574, 341, 2132), "set-trap", 23, -1, Animation(5208), Animation(9726), true) {
    ...
    override fun createHook(wrapper: content.global.skill.hunter.TrapWrapper): content.global.skill.hunter.TrapHook {
        return content.global.skill.hunter.TrapHook(wrapper, getLocations(wrapper.`object`).toTypedArray())
    }
    ...
    override fun isSuccess(player: Player, node: content.global.skill.hunter.TrapNode): Boolean {
        return true
    }

    override fun getTransformId(wrapper: content.global.skill.hunter.TrapWrapper, node: content.global.skill.hunter.TrapNode): Int {
        val dir = wrapper.`object`.attributes.getAttribute("kebbit-dir", 0)
        val id = if (dir == 0) node.objectIds[0] else node.objectIds[1]
        return id
    }

    override fun getFinalId(wrapper: content.global.skill.hunter.TrapWrapper, node: content.global.skill.hunter.TrapNode): Int {
    ...
    override fun exceedsLimit(player: Player): Boolean {
        return content.global.skill.hunter.HunterManager.getInstance(player).trapAmount > 0
    }
