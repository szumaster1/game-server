 ...
 */
class TrapDismantlePulse(player: Player?, node: Scenery?, wrapper: content.global.skill.hunter.TrapWrapper) : SkillPulse<Scenery?>(player, node) {

    val wrapper: content.global.skill.hunter.TrapWrapper = wrapper
    val trap: Traps = wrapper.type

    private var ticks = 0
    private val instance: content.global.skill.hunter.HunterManager = content.global.skill.hunter.HunterManager.getInstance(player)

    /*
     * Instantiates a new Trap dismantle pulse.
     */
    init {
