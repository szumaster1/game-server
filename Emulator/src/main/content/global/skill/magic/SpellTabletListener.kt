...
class SpellTabletListener : InteractionListener {
    ...
    override fun defineListeners() {
        ...
        on(B2B_TABLET, IntType.ITEM, "break") { player, node ->
            breakTablet(player)
            SpellListeners.run(Modern.BONES_TO_BANANAS, content.global.skill.magic.SpellListener.NONE, "modern", player)
        ...
        on(B2P_TABLET, IntType.ITEM, "break") { player, node ->
            breakTablet(player)
            SpellListeners.run(Modern.BONES_TO_PEACHES, content.global.skill.magic.SpellListener.NONE, "modern", player)
            player.inventory.remove(Item(node.id))
            return@on true
        }
