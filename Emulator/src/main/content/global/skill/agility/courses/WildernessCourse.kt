...
@JvmOverloads constructor(player: Player? = null) : content.global.skill.agility.AgilityCourse(player, 5, 499.0) {
    ...
    private fun handleSteppingStones(player: Player, `object`: Scenery) {
        ...
        submitWorldPulse(object : Pulse(2, player) {
            ...
            override fun pulse(): Boolean {
                if (counter == 3 && fail) {
                    content.global.skill.agility.AgilityHandler.fail(player, -1, Location.create(3001, 3963, 0), Animation.create(771), (player.skills.lifepoints * 0.26).toInt(), "...You lose your footing and fall into the lava.")
                    return true
                }
                content.global.skill.agility.AgilityHandler.forceWalk(player, if (counter == 5) 2 else -1, player.location, player.location.transform(-1, 0, 0), Animation.create(741), 10, if (counter == 5) 20.0 else 0.0, if (counter != 0) null else "You carefully start crossing the stepping stones...")
                if (++counter == 6) {
                    unlock(player)
    ...
    private fun handleLogBalance(player: Player, `object`: Scenery) {
        ...
        content.global.skill.agility.AgilityHandler.walk(player, if (failed) -1 else 3, player.location, end, Animation.create(155), if (failed) 0.0 else 20.0, if (failed) null else "You skillfully edge across the gap.")
        if (failed) {
            GameWorld.Pulser.submit(object : Pulse(5, player) {
                override fun pulse(): Boolean {
                    player.faceLocation(Location.create(2998, 3944, 0))
                    content.global.skill.agility.AgilityHandler.fail(player, 3, Location.create(2998, 10345, 0), Animation.create(770), getHitAmount(player), "You slip and fall onto the spikes below.")
                    return true
                }
            })
            return
    ...
    private fun handleRockClimb(player: Player, `object`: Scenery) {
        content.global.skill.agility.AgilityHandler.forceWalk(player, 4, Location.create(2994, 3937, 0), Location.create(2994, 3933, 0), Animation.create(740), 8, 0.0, "You reach the top.")
    ...
    override fun createInstance(player: Player): content.global.skill.agility.AgilityCourse {
        return content.global.skill.agility.courses.WildernessCourse(player)
    }
