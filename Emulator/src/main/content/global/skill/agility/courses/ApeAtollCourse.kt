...
@JvmOverloads constructor(player: Player? = null) : content.global.skill.agility.AgilityCourse(player, 6, 450.0) {

    override fun createInstance(player: Player?): content.global.skill.agility.AgilityCourse {
        return content.global.skill.agility.courses.ApeAtollCourse(player)
    }
    ...
    private fun jumpSteppingStone(player: Player, `object`: Scenery) {
        ...
        } else if (!player.location.withinDistance(`object`.asScenery().location, 2)) {
            return
        } else {
            player.lock(3)
            val toTile2 = Location(if (player.location.x == 2755) 2753 else 2755, 2742, `object`.asScenery().location.z)
            val waterTile = Location(2756, 2746, `object`.asScenery().location.z)
    ...
    private fun climbUpTropicalTree(player: Player, `object`: Scenery) {
        if (!hasLevelDyn(player, Skills.AGILITY, 48)) {
            sendMessage(player, "You need an agility level of at least 48 to do this.")
        } else if (!inEquipment(player, Items.MONKEY_GREEGREE_4024)) {
            sendMessage(player, "You need to transform into a ninja monkey to use the ape atoll courses.")
        } else {
            if (!player.location.withinDistance(`object`.asScenery().location, 2)) return
            player.lock(3)
    ...
    private fun climbUpSkullSlope(player: Player, `object`: Scenery) {
        if (!hasLevelDyn(player, Skills.AGILITY, 48)) {
            sendMessage(player, "You need an agility level of at least 48 to do this.")
        } else if (!inEquipment(player, Items.MONKEY_GREEGREE_4024)) {
            sendMessage(player, "You need to transform into a ninja monkey to use the ape atoll courses.")
                ...
                override fun pulse(): Boolean {
                    when (counter++) {
                        2 -> {
                            ...
                            forceMove(player, player.location.transform(toTile), toTile, 25, 60, player.location.direction, 3489)
    ...
    private fun crossMonkeyBars(player: Player, `object`: Scenery) {
        if (!hasLevelDyn(player, Skills.AGILITY, 48)) {
            sendMessage(player, "You need an agility level of at least 48 to do this.")
        } else if (!inEquipment(player, Items.MONKEY_GREEGREE_4024)) {
            sendMessage(player, "You need to transform into a ninja monkey to use the ape atoll courses.")
    ...
    private fun swingRope(player: Player, `object`: Scenery) {
        if (!hasLevelStat(player, Skills.AGILITY, 48)) {
            ...
            player.lock(4)
            val toTile = location(2756, 2731, `object`.location.z)
            animate(player, 1388)
            animateScenery(player, `object`.asScenery(), 2231, true)
            player.walkingQueue.addPath(toTile.x, toTile.y, false)
    ...
    private fun climbDownTropicalTree(player: Player, `object`: Scenery) {
        if (!hasLevelDyn(player, Skills.AGILITY, 48)) {
            sendMessage(player, "You need an agility level of at least 48 to do this.")
        } else if (!inEquipment(player, Items.MONKEY_GREEGREE_4024)) {
            sendMessage(player, "You need to transform into a ninja monkey to use the ape atoll courses.")
