...
/**
 * Represents the Ardougne log shortcut.
 */
class ArdougneLogShortcut : InteractionListener {
    ...
    private fun determineFailParameters(node: Node): Triple<Animation, Location, Boolean> {
        return when (node.id) {
            Scenery.LOG_BALANCE_35997 -> Triple(Animation(Animations.FALL_OFF_LOG_2582), Location(2603, 3330, 0), true)
            else -> Triple(Animation(Animations.FALL_OFF_LOG_2581), Location(2598, 3333, 0), false)
        }
    }
