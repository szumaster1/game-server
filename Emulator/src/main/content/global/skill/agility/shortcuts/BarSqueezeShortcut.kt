...
class BarSqueezeShortcut : content.global.skill.agility.AgilityShortcut {
    ...
    override fun run(player: Player, obj: Scenery, option: String, failed: Boolean) {
        ...
        val (dir, start) = when {
            obj.id == 9334 && logicalDirection == Direction.NORTH -> Direction.WEST to Location.create(3424, 3476, 0)
            obj.id == 9337 && logicalDirection == Direction.NORTH -> if (player.location.y < obj.location.y) Direction.NORTH to player.location else Direction.SOUTH to player.location
