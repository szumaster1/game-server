 ...
 */
class PotteryCraftPulse(player: Player?, node: Item?, var amount: Int, val pottery: Pottery) :
    ...
    override fun reward(): Boolean {
        ...
        if (player.inventory.remove(SOFT_CLAY)) {
            ...
            sendMessage(
                player,
                "You make the soft clay into " + (if (StringUtils.isPlusN(pottery.unfinished.name)) "an" else "a") + " " + pottery.unfinished.name.lowercase() + "."
            )
