 ...
 */
class DragonCraftPulse(player: Player?, node: Item?, val hide: Leather.DragonHide, var amount: Int) :
    ...
    override fun checkRequirements(): Boolean {
        if (getStatLevel(player, Skills.CRAFTING) < hide.level) {
            sendDialogue(
                player,
                "You need a crafting level of " + hide.level + " to make " + ItemDefinition.forId(hide.product).name + "."
            )
            amount = 0
            return false
        }
        ...
        if (!inInventory(player, hide.leather, hide.amount)) {
            sendDialogue(
                player,
                "You need " + hide.amount + " " + getItemName(hide.leather).lowercase() + " to make this."
            )
            amount = 0
            return false
        }
    ...
    override fun reward(): Boolean {
        ...
        if (removeItem(player, Item(hide.leather, hide.amount))) {
            if (hide.name.contains("VAMBS")) {
                ...
                sendMessage(
                    player,
                    "You make " + (if (StringUtils.isPlusN(getItemName(hide.product).lowercase())) "an" else "a") + " " + getItemName(
                        hide.product
                    ).lowercase() + "."
                )
            }
