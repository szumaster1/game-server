...
/**
 * Represents the skill pulse of the granite splitting items.
 */
class GraniteSplittingPulse(player: Player, node: Item, var amount: Int) : SkillPulse<Item>(player, node) {
    ...
    override fun checkRequirements(): Boolean {
        ...
        for (i in SLOTS.indices) if (freeSlots(player) < i) {
            sendDialogue(
                player,
                "You'll need ${4 - i} empty inventory ${if (freeSlots(player) != 1) "spaces" else "space"} to hold the granite once you've split this block."
            )
            return false
        }
