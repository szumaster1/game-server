...
class SpikedVambracesCraftingListener : InteractionListener {
    ...
    override fun defineListeners() {
        onUseWith(IntType.ITEM, Items.KEBBIT_CLAWS_10113, *itemIDs) { player, _, with ->
            ...
            when (with.id) {
                Items.LEATHER_VAMBRACES_1063 -> createVambraces(
                    player = player,
                    vamb = with.asItem(),
                    product = Items.SPIKY_VAMBRACES_10077,
                    leather = "leather"
                )
                Items.GREEN_DHIDE_VAMB_1065 -> createVambraces(
                    player = player,
                    vamb = with.asItem(),
                ...
                Items.BLUE_DHIDE_VAMB_2487 -> createVambraces(
                    player = player,
                    vamb = with.asItem(),
                ...
                Items.RED_DHIDE_VAMB_2489 -> createVambraces(
                    player = player,
                    vamb = with.asItem(),
                ...
                Items.BLACK_DHIDE_VAMB_2491 -> createVambraces(
                    player = player,
                    vamb = with.asItem(),
    ...
    private fun createVambraces(player: Player, vamb: Item, product: Int, leather: String) {
        if (removeItem(player, Items.KEBBIT_CLAWS_10113, Container.INVENTORY)) {
            replaceSlot(player, vamb.slot, Item(product))
