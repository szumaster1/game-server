...
class DragonLeatherCraftingPulse(player: Player?, node: Item?, val leather: DragonLeather, var amount: Int) : SkillPulse<Item?>(player, node) {
    ...
    override fun checkRequirements(): Boolean {
        if (getStatLevel(player, Skills.CRAFTING) < leather.level) {
            sendDialogue(player, "You need a crafting level of " + leather.level + " to make " + getItemName(leather.product).lowercase() + ".")
            amount = 0
            return false
        }
        ...
        if (!inInventory(player, leather.leather, leather.amount)) {
            sendDialogue(player, "You need " + leather.amount + " " + getItemName(leather.leather).lowercase() + " to make this.")
            amount = 0
            return false
        }
    ...
    override fun reward(): Boolean {
        ...
        if (removeItem(player, Item(leather.leather, leather.amount))) {
            if (leather.name.contains("VAMBS")) {
                ...
                sendMessage(player, "You make " + (if (StringUtils.isPlusN(getItemName(leather.product).lowercase())) "an" else "a") + " " + getItemName(leather.product).lowercase() + ".")
            }
