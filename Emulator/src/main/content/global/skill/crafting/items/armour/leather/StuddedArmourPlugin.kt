...
@Initializable
class StuddedArmourPlugin : UseWithHandler(Items.STEEL_STUDS_2370) {
            ...
            object : SkillDialogueHandler(player, SkillDialogue.ONE_OPTION, armour!!.studded) {
                override fun create(amount: Int, index: Int) {
                    submitIndividualPulse(
                        entity = player,
                        pulse = StuddedArmourPulse(player, event.baseItem, armour, amount)
                    )
                }
     ...
     */
    class StuddedArmourPulse(player: Player?, node: Item?, private val armour: StuddedArmour?, private var amount: Int) : SkillPulse<Item?>(player, node) {
