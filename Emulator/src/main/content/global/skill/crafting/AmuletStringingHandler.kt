...
class AmuletStringingHandler : UseWithHandler(
    ...
    override fun handle(event: NodeUsageEvent): Boolean {
        ...
        val data =
            Jewellery.JewelleryItem.forProduct(if (event.usedItem.id == Items.ONYX_AMULET_6579) Items.ONYX_AMULET_6579 else event.usedWith.id)
                ?: return true
        ...
        if (player.inventory.remove(event.usedItem, event.baseItem)) {
            player.inventory.add(Item(if (data === Jewellery.JewelleryItem.ONYX_AMULET) Items.ONYX_AMULET_6581 else data.sendItem + 19))
        }
        return true
    }
}
