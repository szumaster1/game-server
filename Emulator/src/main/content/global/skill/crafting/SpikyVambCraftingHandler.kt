...
class SpikyVambCraftingHandler : UseWithHandler(Items.KEBBIT_CLAWS_10113) {
    ...
    override fun handle(event: NodeUsageEvent?): Boolean {
        ...
        when (vamb) {
            Items.LEATHER_VAMBRACES_1063 -> craftVamb(player, vamb, Items.SPIKY_VAMBRACES_10077, "leather")
            Items.GREEN_DHIDE_VAMB_1065 -> craftVamb(player, vamb, Items.GREEN_SPIKY_VAMBS_10079, "green dragonhide")
            Items.BLUE_DHIDE_VAMB_2487 -> craftVamb(player, vamb, Items.BLUE_SPIKY_VAMBS_10081, "blue dragonhide")
            Items.RED_DHIDE_VAMB_2489 -> craftVamb(player, vamb, Items.RED_SPIKY_VAMBS_10083, "red dragonhide")
            Items.BLACK_DHIDE_VAMB_2491 -> craftVamb(player, vamb, Items.BLACK_SPIKY_VAMBS_10085, "black dragonhide")
        }
        return true
    }
    ...
    private fun craftVamb(player: Player, vamb: Int, product: Int, vambLeather: String) {
        if (getStatLevel(player, Skills.CRAFTING) >= 32) {
            if (removeItem(player, vamb, Container.INVENTORY) && removeItem(
                    player,
                    Items.KEBBIT_CLAWS_10113,
                    Container.INVENTORY
                )
            ) {
