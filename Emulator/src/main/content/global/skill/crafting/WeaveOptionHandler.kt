...
class WeaveOptionHandler : OptionHandler() {
    ...
    override fun handle(player: Player, node: Node, option: String): Boolean {
        object : SkillDialogueHandler(
            ...
            WeavingItem.SACK.product,
            WeavingItem.BASKET.product,
            WeavingItem.CLOTH.product
        ) {
    ...
    class WeavePulse(player: Player?, node: Scenery?, private val type: WeavingItem, private var amount: Int) :
            ...
            if (!inInventory(player, type.required.id)) {
                sendMessage(
                    ...
                    "You need " + type.required.amount + " " + type.required.name.lowercase().replace(
                        "ball",
                        "balls"
                    ) + (if (type == WeavingItem.SACK) "s" else if (type == WeavingItem.CLOTH) "" else "es") + " to weave " + (if (StringUtils.isPlusN(
        ...
        override fun reward(): Boolean {
            ...
            if (removeItem(player, type.required)) {
                ...
                if (type == WeavingItem.BASKET && node!!.id == 8717 && withinDistance(
                        player,
