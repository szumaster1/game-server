 ...
 */
class DoughMaking : InteractionListener {
    ...
    private class DoughMakeDialogue(val waterContainer: Item, val flourContainer: Item) : DialogueFile() {
        companion object {
            ...
            private enum
            class DoughProduct(val itemId: Int, val itemName: String, val requiredCookingLevel: Int) {
                ...
                STAGE_PROCESS_OPTION -> runTask(player!!, 1) {
                    ...
                    if (hasLevelDyn(player!!, Skills.COOKING, selectedDoughProduct.requiredCookingLevel)) {
                        ...
                        if (removeItem(player!!, waterContainer) && removeItem(player!!, flourContainer)) {
                            ...
                            sendMessage(
                                player!!,
                                "You mix the flower and the water to make some ${selectedDoughProduct.itemName.lowercase()}."
                            )
                        }
                    } else {
                        sendDialogue(
                            player!!,
                            "You need a Cooking level of at least ${selectedDoughProduct.requiredCookingLevel} in order to make ${selectedDoughProduct.itemName.lowercase()}."
                        )
                    }
                }
            }
