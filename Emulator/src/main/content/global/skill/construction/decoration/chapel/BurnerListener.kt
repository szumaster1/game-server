...
class BurnerListener : InteractionListener {
    ...
    override fun defineListeners() {
        on(burnerIds, IntType.SCENERY, "light") { player, node ->
            if (checkRestriction(player)) return@on true
            ...
            if (removeItem(player, Item(Items.CLEAN_MARRENTILL_251))) {
                sacrificingBones(player, node)
            }
            return@on true
        }
    }

    private fun checkRestriction(player: Player): Boolean {
        return player.ironmanManager.checkRestriction() && !player.houseManager.isInHouse(player)
    }
    ...
    private fun sacrificingBones(player: Player, node: Node) {
        lock(player, 1)
