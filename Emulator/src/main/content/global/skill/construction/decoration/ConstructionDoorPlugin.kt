...
class ConstructionDoorPlugin : OptionHandler() {
    ...
    override fun newInstance(arg: Any?): Plugin<Any> {
        for (style in content.global.skill.construction.HousingStyle.values()) {
        ...
        for (deco in content.global.skill.construction.BuildHotspot.DUNGEON_DOOR_LEFT.decorations) {
        ...
        for (deco in content.global.skill.construction.BuildHotspot.DUNGEON_DOOR_RIGHT.decorations) {
            SceneryDefinition.forId(deco.objectId).handlers["option:open"] = this
            SceneryDefinition.forId(deco.objectId).handlers["option:pick-lock"] = this
            SceneryDefinition.forId(deco.objectId).handlers["option:force"] = this
        }
