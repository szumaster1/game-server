...
class ToolLeprechaunInterface : InterfaceListener {
    ...
    private fun doStackedWithdrawal(
        player: Player?,
        item: Int,
        amount: Int,
        updateQuantityMethod: (Player?, Int) -> Unit,
    ...
    private fun getWateringCan(player: Player?): Int {
        var can = getVarbit(
            player!!,
            1439
        ) //Watering cans are stored in the Varp as a number between 1 and 9. Watering Can(0) is 1 and Watering Can(8) is 9
        if (can == 1) can = 0
