...
class FarmerPayOptionDialogue(val patch: Patch, val quickPay: Boolean = false) : DialogueFile() {
    ...
    override fun handle(componentID: Int, buttonID: Int) {
        val facialExpression =
            if (npc!!.id in intArrayOf(1037, 2343)) FacialExpression.OLD_NORMAL else FacialExpression.HALF_GUILTY
        when (stage) {
            START_DIALOGUE -> {
                if (patch.patch.type == PatchType.TREE_PATCH && patch.plantable != null && patch.isGrown()) {
                    // This is for the right-click "Pay" option; full dialogue is in GardenerDialogue
                    showTopics(
                    ...
                    ).also { stage = END_DIALOGUE } // this dialogue is not authentic
            ...
            20 -> {
                if (removeItem(player!!, item) || removeItem(player!!, note(item!!))) {
                    ...
                    npc(facialExpression, "This shouldn't be happening. Please report this.").also {
                        stage = END_DIALOGUE
                    }
                }
            }
