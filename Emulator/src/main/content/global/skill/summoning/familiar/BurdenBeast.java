...
public abstract class BurdenBeast extends Familiar {
    ...
    public boolean isAllowed(Player owner, Item item) {
        ...
        if ((!SummoningPouch.get(getPouchId()).getAbyssal() && (item.getId() == 1436 || item.getId() == 7936)) || !item.getDefinition().getConfiguration(ItemConfigParser.BANKABLE, true)) {
            owner.getPacketDispatch().sendMessage("You can't store " + item.getName().toLowerCase() + " in this familiar.");
            return false;
        }
        if (SummoningPouch.get(this.getPouchId()).getAbyssal()) {
