...
class WoodcuttingListener : InteractionListener {
    ...
    private fun calculateExperience(player: Player, resource: WoodcuttingNode, amount: Int): Double {
         ...
         */
        if (reward == 1517 && player.achievementDiaryManager.getDiary(DiaryType.SEERS_VILLAGE)!!.isComplete(1) && player.equipment.get(EquipmentContainer.SLOT_HAT) != null && player.equipment.get(EquipmentContainer.SLOT_HAT).id == Items.SEERS_HEADBAND_1_14631) {
            experience *= 1.10
        }
