...
/**
 * Handles the signpost content.
 */
class Signpost : InteractionListener {
    ...
    override fun defineListeners() {
        ...
        registerSignpost(
            Scenery.SIGNPOST_18493, listOf(
                Location(3235, 3228) to listOf(
        ...
        registerSignpost(
            Scenery.SIGNPOST_24263, listOf(
                Location(3268, 3332) to listOf(
                    "Sheep lay this way.",
        ...
        registerSignpost(
            Scenery.SIGNPOST_4134, listOf(
                Location(2651, 3606) to listOf(
                    "North to Rellekka.",
                    "South to Seers' Village.",
                    "East to Death Plateau.",
                    "West to the Lighthouse."
                ),
                Location(3100, 3418) to listOf(
                    "North to Edgeville.",
    ...
    private fun registerSignpost(sceneryId: Int, locations: List<Pair<Location, List<String>>>) {
        on(sceneryId, IntType.SCENERY, "read") { player, node ->
            val location = node.asScenery().location
            val directions = locations.firstOrNull { it.first == location }?.second ?: listOf(
                "North to unknown.", "South to unknown.", "East to unknown.", "West to unknown."
            )
    ...
    private fun sendSignpostDirections(player: Player, directions: List<String>) {
        sendString(player, directions[0], 135, 3)   // North
        sendString(player, directions[1], 135, 9)   // South
        sendString(player, directions[2], 135, 8)   // East
        sendString(player, directions[3], 135, 12)  // West
