...
/**
 * Represents the coal truck option interaction listener.
 */
class CoalTruckListener : InteractionListener {
    ...
    private fun calculateMaxCoalCapacity(player: Player): Int {
        return when {
            inEquipment(player, Items.SEERS_HEADBAND_1_14631) && isDiaryComplete(player, DiaryType.SEERS_VILLAGE, 2) -> 196
            inEquipment(player, Items.SEERS_HEADBAND_1_14631) && isDiaryComplete(player, DiaryType.SEERS_VILLAGE, 1) -> 168
            inEquipment(player, Items.SEERS_HEADBAND_1_14631) && isDiaryComplete(player, DiaryType.SEERS_VILLAGE, 0) -> 140
            else -> 120
        }
    }
    ...
    override fun defineListeners() {
        ...
        /*
         * Check how many is stored into truck.
         */
        ...
        on(Scenery.COAL_TRUCK_2114, IntType.SCENERY, "investigate") { player, _ ->
            ...
            sendDialogue(player, "There is currently $coalInTruck coal in the truck. The truck has space for $availableSpace more coal.")
            return@on true
        }
