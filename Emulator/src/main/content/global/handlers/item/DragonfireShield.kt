 ...
 */
class DragonfireShield : InteractionListener {
    ...
    val dfsRecharge = if (Configuration.BETTER_DFS) secondsToTicks(30) else minutesToTicks(2)
    ...
    override fun defineListeners() {
        ...
        on(dragonfireShields, IntType.ITEM, "operate") { player, node ->
            ...
            val attack = SwitchAttack(
                null,
                Animation.create(6696),
                Graphic.create(1165),
                Graphic(1167, 96),
                Projectile.create(player, null, 1166, 36, 36, 80, 70, 0, 11)
            )
        ...
        on(dragonfireShields, IntType.ITEM, "drop") { player, node ->
            ...
            if (shield.id == Items.DRAGONFIRE_SHIELD_11283) {
                replaceSlot(
                    player, shield.slot, Item(Items.DRAGONFIRE_SHIELD_11284), Item(Items.DRAGONFIRE_SHIELD_11283)
                )
