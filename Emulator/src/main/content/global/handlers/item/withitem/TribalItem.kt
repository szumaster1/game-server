 ...
 */
class TribalItem : InteractionListener {
    ...
    override fun defineListeners() {
        onUseWith(IntType.ITEM,  Items.HAMMER_2347, TRIBAL_MASK_GREEN) { player, _, _ ->
            animate(player, 2410)
            return@onUseWith makeShield(player, BROODOO_SHIELD_GREEN, TRIBAL_MASK_GREEN)
        }

        onUseWith(IntType.ITEM,  Items.HAMMER_2347, TRIBAL_MASK_YELLOW) { player, _, _ ->
            animate(player, 2411)
            return@onUseWith makeShield(player, BROODOO_SHIELD_YELLOW, TRIBAL_MASK_YELLOW)
        }

        onUseWith(IntType.ITEM,  Items.HAMMER_2347, TRIBAL_MASK_WHITE) { player, _, _ ->
            animate(player, 2409)
