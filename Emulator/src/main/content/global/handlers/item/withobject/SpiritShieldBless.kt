...
/**
 * Handles bless a spirit shields.
 */
class SpiritShieldBless : InteractionListener {

    companion object {
        private val COMPONENTS = intArrayOf(Items.HOLY_ELIXIR_13754, Items.SPIRIT_SHIELD_13734)
        private val ALTARS = intArrayOf(Scenery.ALTAR_409, Scenery.ALTAR_2640, Scenery.ALTAR_4008, Scenery.ALTAR_18254, Scenery.ALTAR_19145, Scenery.ALTAR_24343, Scenery.SARADOMIN_ALTAR_26287, Scenery.ALTAR_27661, Scenery.ALTAR_34616, Scenery.ALTAR_36972, Scenery.ALTAR_39842)
        private val ANVILS = intArrayOf(Scenery.ANVIL_2782, Scenery.ANVIL_2783, Scenery.ANVIL_4306, Scenery.ANVIL_6150, Scenery.ANVIL_22725, Scenery.LATHE_26817, Scenery.ANVIL_26822, Scenery.ANVIL_37622)
    ...
    override fun defineListeners() {
        ...
        onUseWith(IntType.SCENERY, SIGIL_MAP.keys.toIntArray(), *ANVILS) { player, used, _ ->
            ...
            if (!hasLevelDyn(player, Skills.PRAYER, 90) || !hasLevelDyn(player, Skills.SMITHING, 85)) {
                sendMessage(player, "You need a Prayer level of 90 and a Smithing level of 85 in order to attach the sigil.")
                return@onUseWith true
            }
                ...
                when (buttonId) {
                    2 -> {
                        if (removeItem(player, used) && removeItem(player, Items.BLESSED_SPIRIT_SHIELD_13736)) {
                            ...
                            sendItemDialogue(player, product.id, "You successfully attach the " + used.name.lowercase() + " to the blessed spirit shield.")
                        }
                    }
                }
            }
