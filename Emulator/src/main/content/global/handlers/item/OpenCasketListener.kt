...
/**
 * Handles the opening of casket interaction.
 */
class OpenCasketListener : InteractionListener {
    ...
    override fun defineListeners() {
        on(Items.CASKET_405, IntType.ITEM, "open") { player, node ->
            ...
            if (removeItem(player, casket, Container.INVENTORY)) {
                ...
                player.dialogueInterpreter.sendItemMessage(finalLoot[0], "You open the casket. Inside you find " + (if (finalLoot[0].amount > 1) "some" else if (StringUtils.isPlusN(finalLoot[0].name)) "an" else "a") + " " + finalLoot[0].name.lowercase() + ".")
            }
            return@on true
        }
    }
