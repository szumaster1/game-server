...
class ThessaliaInterface : ComponentPlugin() {
    ...
    override fun open(player: Player?, component: Component?) {
        ...
        setAttribute(player!!, "orig-torso", player.appearance.torso.look)
        setAttribute(player, "orig-torso-color", player.appearance.torso.color)
        setAttribute(player, "orig-arms", player.appearance.arms.look)
        setAttribute(player, "orig-arms-color", player.appearance.arms.color)
        setAttribute(player, "orig-legs", player.appearance.legs.look)
            ...
            181, 180, 297 -> pay(player)
                ...
                THESSALIA_MALE_COMPONENT -> {
                    ...
                    if (maleColorButtonRange.contains(button)) updateColor(
                        player, button, true, player.getAttribute(
                            "thes-type", colorType.TORSO
                        )
                    )
                }
                ...
                THESSALIA_FEMALE_COMPONENT -> {
                    ...
                    if (femaleColorButtonRange.contains(button)) updateColor(
                        player, button, false, player.getAttribute(
                            "thes-type", colorType.TORSO
                        )
                    )
                }
