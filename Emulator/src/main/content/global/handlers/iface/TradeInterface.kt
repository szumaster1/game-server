...
class TradeInterface : ComponentPlugin() {
    ...
    override fun handle(player: Player, component: Component, opcode: Int, button: Int, slot: Int, itemId: Int): Boolean {
        ...
        when (component.id) {
            Components.TRADECONFIRM_334 -> when (button) {
                20 -> module.setAccepted(true, true)
                21 -> module.decline()
            }

            Components.TRADEMAIN_335 -> when (opcode) {
                155 -> when (button) {
                ...
                234 -> sendInputDialogue(player, false, "Enter the amount:") { value: Any ->
            ...
            Components.TRADESIDE_336 -> when (opcode) {
