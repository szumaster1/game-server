...
import core.game.ge.GrandExchange.Companion.dispatch
...
/**
 * Handles grand exchange item search.
 */
class StockMarket : InterfaceListener {
    ...
    override fun defineInterfaceListeners() {
        ...
        on(Components.STOCKSIDE_107) { player, _, op, button, slot, _ ->
            ...
            when (op) {
                ...
                155 -> {
                    ...
                    var id = item.id
                    if (!item.definition.isUnnoted) id = item.noteChange
                    ...
                    offer.sell = true
                    offer.player = player
                    offer.offeredValue = getRecommendedPrice(id)
        ...
        on(Components.STOCKMARKET_105) { player, _, op, button, _, _ ->
            ...
            when (button) {
                ...
                168 -> {
                    val amt = if (tempOffer.sell) getInventoryAmount(player, tempOffer.itemID)
                    else tempOffer.amount + 1000
                ...
                180 -> updateOfferValue(player, tempOffer, getRecommendedPrice(tempOffer.itemID))
                177 -> updateOfferValue(
                    player,
                    tempOffer,
                    (getRecommendedPrice(tempOffer.itemID) * 0.95).toInt()
                )

                183 -> updateOfferValue(
                    player,
                    tempOffer,
                    (getRecommendedPrice(tempOffer.itemID) * 1.05).toInt()
                )
    ...
    fun abortOffer(player: Player, offer: GrandExchangeOffer?) {
        ...
        if (offer.sell) offer.addWithdrawItem(offer.itemID, offer.amountLeft)
        else offer.addWithdrawItem(995, offer.amountLeft * offer.offeredValue)
        offer.update()
    ...
    fun confirmOffer(player: Player, offer: GrandExchangeOffer, index: Int): OfferConfirmResult {
        ...
        if (offer.sell) {
            ...
            val removeNoted =
                if (amountLeft > 0) removeItem(player, Item(itemDefinition(offer.itemID).noteId, amountLeft)) else true
            if (!removedUnnoted || !removeNoted) return OfferConfirmResult.ItemRemovalFailure
            if (dispatch(player, offer)) {
                removeAttribute(player, "ge-temp")
            } else {
                if (amountUnnoted > 0) addItem(player, offer.itemID, offer.amount - amountLeft)
                if (amountLeft > 0) addItem(player, itemDefinition(offer.itemID).noteId, amountLeft)
            ...
            if (dispatch(player, offer) && removeItem(player, Item(995, total))) {
                removeAttribute(player, "ge-temp")
            }
        }
    ...
    companion object {
        ...
        fun openFor(player: Player) {
            if (player.ironmanManager.checkRestriction()) return
        ...
        fun updateVarbits(player: Player, offer: GrandExchangeOffer?, index: Int, sale: Boolean? = null) {
            ...
            var lowPrice = 0
            var highPrice = 0
            val recommendedPrice = getRecommendedPrice(offer?.itemID ?: 0)
