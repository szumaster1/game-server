...
/**
 * Exchange item sets listener.
 */
class ExchangeItemSetsListener : InterfaceListener {

    // This method defines the interface listeners for the ExchangeItemSetsListener class.
    ...
    companion object {
        ...
        @JvmStatic
        // This method is used to open the Exchange Item Sets interface for a player.
    ...
    private class InventoryListener(val player: Player) : ContainerListener {
        ...
        // This method is called when the container is updated.
        override fun update(c: Container?, event: ContainerEvent?) {
            createContainers(player);
        }

        // This method is called when the container is refreshed.
        override fun refresh(c: Container?) {
            createContainers(player);
        }

        private fun createContainers(player: Player) {
            setAttribute(player, "container-key", InterfaceContainer.generateItems(player, player.inventory.toArray(), arrayOf("Examine", "Exchange", "Components"), Components.EXCHANGE_SETS_SIDE_644, 0, 7, 4));
