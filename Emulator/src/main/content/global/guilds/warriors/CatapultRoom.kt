package content.global.guilds.warriors
...
class CatapultRoom : MapZone("wg catapult", true), Plugin<Any> {
    ...
    companion object {
        ...
        private val pulse: Pulse = object : Pulse(10) {
                        ...
                        for (p in players) {
                            if (p.isActive && p.location == TARGET) {
                                if (p.equipment.getNew(EquipmentContainer.SLOT_SHIELD).id != SHIELD_ID) {
                                    p.dialogueInterpreter.sendDialogues(4287, FacialExpression.FURIOUS, "Watch out! You'll need to equip the shield as soon as", "you're on the target spot else you could get hit! Speak", "to me to get one, and make sure both your hands are", "free to equip it.")
