...
class SeamanDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun open(vararg args: Any): Boolean {
        npc = args[0] as NPC
        if (args.size > 1 && isQuestComplete(player, "Pirate's Treasure")) {
        ...
        npc(FacialExpression.HALF_GUILTY, "Do you want to go on a trip to Karamja?")
        stage = 0
        return true
    }
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            0 -> npc(FacialExpression.HALF_GUILTY, "The trip will cost you 30 coins.").also { stage++ }
            1 -> {
                var charos = false
            ...
            2 -> when (buttonId) {
                ...
                2 -> player(FacialExpression.HALF_GUILTY, "No, thank you.").also { stage = 20 }
                3 -> player(FacialExpression.HALF_GUILTY, "Or I could pay you nothing at all...").also { stage = 5 }
            }

            5 -> npc(FacialExpression.HALF_GUILTY, "Mmmm ... Nothing at all you say ...").also { stage++ }
            ...
            9 -> npc(FacialExpression.HALF_GUILTY, "Wait a minute... Aren't those Karamja gloves?", "Thought I'd seen you helping around the island.", "You can go on half price - 15 coins.").also { stage++ }
            10 -> pay(15)
            11 -> pay(30)
            20 -> end()
            30 -> travel()
