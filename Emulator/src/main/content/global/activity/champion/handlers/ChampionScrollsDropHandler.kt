...
class ChampionScrollsDropHandler : ChampionScrollsEventHookBase() {
    companion object {
        ...
        private val LESSER_DEMONS = intArrayOf(NPCs.LESSER_DEMON_82, NPCs.LESSER_DEMON_4694, NPCs.LESSER_DEMON_4695, NPCs.LESSER_DEMON_4696, NPCs.LESSER_DEMON_4697, NPCs.LESSER_DEMON_6101)

        private val ZAKLN = NPCs.ZAKLN_GRITCH_6206

        val SCROLLS = intArrayOf(Items.CHAMPION_SCROLL_6803, Items.CHAMPION_SCROLL_6801, Items.CHAMPION_SCROLL_6806, Items.CHAMPION_SCROLL_6807, Items.CHAMPION_SCROLL_6800, Items.CHAMPION_SCROLL_6802, Items.CHAMPION_SCROLL_6799, Items.CHAMPION_SCROLL_6798, Items.CHAMPION_SCROLL_6804, Items.CHAMPION_SCROLL_6805)

        val idMap = HashMap<Int, Int>()

        init {
            for (id in IMPS) idMap[id] = 0
            for (id in GOBLINS) idMap[id] = 1
            for (id in SKELETONS) idMap[id] = 2
            for (id in ZOMBIES) idMap[id] = 3
            for (id in GIANTS) idMap[id] = 4
            for (id in HOBGOBLINS) idMap[id] = 5
            for (id in JOGRES) idMap[id] = 8
            for (id in LESSER_DEMONS) idMap[id] = 9
