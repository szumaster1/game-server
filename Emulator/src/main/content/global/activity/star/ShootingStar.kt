...
class ShootingStar(var level: ShootingStarType = ShootingStarType.values().random()) {
    ...
    var starObject = Scenery(level.objectId, crashLocations[location])
    ...
    /*
     * Degrades a ShootingStar (or removes the starObject and spawns a Star Sprite
     * if it's the last star).
     */
    ...
    /*
     * Rebuilds some variables with new information.
     */
    ...
    fun rebuildVars() {
        ...
        maxDust = level.totalStardust
        dustLeft = level.totalStardust
        starObject = Scenery(level.objectId, crashLocations[location])
    ...
    /*
     * Decrement the amount of dust left in the current star and degrade it if
     * the amount left is 0 or less.
     */
    ...
    fun mine(player: Player) {
        player.pulseManager.run(
            ShootingStarMiningPulse(player, starObject, this)
        )
    }
