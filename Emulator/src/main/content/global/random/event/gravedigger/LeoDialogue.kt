    ...
    override fun handle(componentID: Int, buttonID: Int) {
        ...
        if (correctTask == 1) {
            ...
            when (stage) {
                ...
                3 -> npcl("Try looking in the coffins to get a better idea of who is in them, and then read the gravestones to find who needs to be in there.").also {
                    stage = 4
                }

                4 -> playerl("All right, I'll give it another shot.").also { stage = 5 }
                5 -> npcl("Don't forget to store any items that you don't need in the mausoleum. I'll take them to the bank while you work.").also {
                    stage = END_DIALOGUE
                }
            ...
            when (stage) {
                0 -> npcl("Can you come and help me, ${player!!.username}? I've got a problem with some graves.").also { stage++ }
                1 -> options("Okay, I'll help with your graves.", "Sorry, I'm busy.").also { stage++ }
                2 -> when (buttonID) {
                    1 -> {
                        end()
                        setAttribute(player!!, GraveUtils.LEO_LOCATION, player!!.location)
                        registerLogoutListener(player!!, GraveUtils.LEO_LOGOUT) { p ->
                            p.location = getAttribute(p, GraveUtils.LEO_LOCATION, player!!.location)
                        }
