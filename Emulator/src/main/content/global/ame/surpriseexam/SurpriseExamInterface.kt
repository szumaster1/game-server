    ...
    override fun defineInterfaceListeners() {
        ...
        on(COMPONENT) { player, _, _, buttonID, _, _ ->
            ...
            if (index == correctIndex) {
                ...
                openDialogue(
                    player, MordautDialogue(examComplete = done, questionCorrect = true, fromInterface = true), NPC(NPCs.MR_MORDAUT_6117)
                )
            } else {
                openDialogue(
                    player, MordautDialogue(examComplete = false, questionCorrect = false, fromInterface = true), NPC(NPCs.MR_MORDAUT_6117)
                )
            }
            return@on true
        }
