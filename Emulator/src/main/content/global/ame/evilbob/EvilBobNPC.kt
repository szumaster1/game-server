        ...
        queueScript(player, 4, QueueStrength.SOFT) { stage: Int ->
            when (stage) {
                ...
                1 -> {
                    ...
                    setAttribute(player, EvilBobUtils.prevLocation, player.location)
                    player.properties.teleportLocation = Location.create(3419, 4776, 0)
                    resetAnimator(player)
                    openDialogue(player, EvilBobDialogue(), NPCs.EVIL_BOB_2479).also {
                        face(player, findLocalNPC(player, NPCs.EVIL_BOB_2479)!!)
                    }
