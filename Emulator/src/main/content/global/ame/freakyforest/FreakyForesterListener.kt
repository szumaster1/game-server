 ...
 * Freak listeners.
 */
class FreakyForesterListener : InteractionListener, MapArea {
    ...
    override fun defineListeners() {
        on(freakNpc, IntType.NPC, "talk-to") { player, node ->
            if (inBorders(player, FreakyForesterUtils.freakArea)) {
                if (getAttribute(player, FreakyForesterUtils.freakTask, -1) == -1) FreakyForesterUtils.giveFreakTask(player)
        ...
        on(pheasants, IntType.NPC, "attack") { player, node ->
            if (getAttribute(player, FreakyForesterUtils.freakComplete, false)) {
                sendDialogue(player, "You don't need to attack any more pheasants. You're allowed to leave.")
            } else if (inInventory(player, Items.RAW_PHEASANT_6178) || inInventory(player, Items.RAW_PHEASANT_6179) || getAttribute(player,
                    FreakyForesterUtils.pheasantKilled, false)) {
