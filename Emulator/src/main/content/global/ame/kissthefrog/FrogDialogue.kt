...
class FrogDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        ...
        if (failRandom == 1) {
            when (stage) {
                0 -> npcl(FacialExpression.OLD_ANGRY1, "Don't talk to me! Speak to the frog " + (if (player!!.isMale) "Princess" else "Prince") + "!").also { stage = END_DIALOGUE }
            }
        } else {
            when (stage) {
                0 -> {
                    lock(player!!, 1000)
                    sendNPCDialogue(player!!, NPCs.FROG_2471, "Well, we'll see how you like being a frog!", FacialExpression.OLD_NORMAL).also { stage++ }
                }
