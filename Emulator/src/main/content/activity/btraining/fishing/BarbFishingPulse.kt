package content.activity.btraining.fishing
...
class BarbFishingPulse(player: Player) : SkillPulse<NPC>(player, NPC(NPCs.FISHING_SPOT_1176)) {
    ...
    override fun checkRequirements(): Boolean {
        ...
        if (getStatLevel(player, Skills.AGILITY) < 15 || getStatLevel(player, Skills.STRENGTH) < 15) {
            player.sendMessages("You need a ", if (getStatLevel(player, Skills.AGILITY) < 15 && getStatLevel(player, Skills.STRENGTH) < 15) "agility and strength" else if(getStatLevel(player, Skills.AGILITY) < 15)  "agility" else "strength", " level of at least 15 to fish here.")
            return false
        }
