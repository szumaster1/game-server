...
class BlazeSharpeyeDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            115 -> npc("Yes. Yes I do. Now, why don't you go over there and", "try lighting that beacon. Show us what you've got.").also { stage++; player.questRepository.getQuest(QuestName.ALL_FIRED_UP).setStage(player, 20) }
            ...
            204 -> npc("My colleague, Squire Fyre, is tending that beacon.", "She'll help you out if you run into any trouble.").also { stage = 116; player.questRepository.getQuest(QuestName.ALL_FIRED_UP).setStage(player, 40) }
            ...
            305 -> npc("You wouldn't mind topping this one up for me, would", "you? Oh, how I love to see things burn!").also { stage++; player.questRepository.getQuest(QuestName.ALL_FIRED_UP).setStage(player, 70) }
            ...
            413 -> player("I'll get right on that.").also { stage = 1000; player.questRepository.getQuest(QuestName.ALL_FIRED_UP).setStage(player, 90) }
    ...
    override fun getIds(): IntArray {
        return intArrayOf(NPCs.BLAZE_SHARPEYE_5191)
    }
