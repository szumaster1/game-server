...
class DemonSlayerPlugin : OptionHandler() {
    ...
    override fun newInstance(arg: Any?): Plugin<Any> {
        ...
        NPCDefinition.forId(DemonSlayerCutscene.DELRITH).handlers["option:attack"] = this
        NPCDefinition.forId(DemonSlayerCutscene.WEAKENED_DELRITH).handlers["option:banish"] = this
        return this
    }
    ...
    override fun handle(player: Player, node: Node, option: String): Boolean {
        ...
        when (id) {
            ...
            882 -> when (option) {
                "climb-down" -> if (node.location == Location(3237, 3458, 0)) {
            ...
            17429 -> if (quest.getStage(player) == 20 && player.inventory.add(DemonSlayerUtils.FIRST_KEY)) {
                setVarp(player, 222, 4757762, true)
                removeAttribute(player, "demon-slayer:poured")
                removeAttribute(player, "demon-slayer:just-poured")
                player.dialogueInterpreter.sendItemMessage(DemonSlayerUtils.FIRST_KEY.id, "You pick up an old rusty key.")
            } else {
