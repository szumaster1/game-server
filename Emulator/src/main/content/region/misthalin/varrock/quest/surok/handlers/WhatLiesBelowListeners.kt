...
class WhatLiesBelowListeners : InteractionListener {
    ...
    override fun defineListeners() {
        ...
        on(Scenery.PORTAL_23095, IntType.SCENERY, "use") { player, node ->
        ...
        on(Items.BEACON_RING_11014, IntType.ITEM, "summon", "operate") { player, _ ->
            if (getQuestStage(player, "What Lies Below") > 70) {
                ...
                if (getQuestStage(player, "What Lies Below") == 70 && player.zoneMonitor.isInZone("What Lies below")) {
                    val cutscene = player.getAttribute<content.region.misthalin.varrock.quest.surok.cutscene.WhatLiesBelowCutscene>("cutscene", null)
                    cutscene.summonZaff()
                    return@on true
                }
        ...
        /*
         * Handle "read" option related items.
         */
        ...
        on(QUEST_ITEMS, IntType.ITEM, "read") { player, node ->
            when (node.id) {
                ...
                11008, 11007, 11006 -> sendDialogueLines(
                    player,
                ...
                11009, 11010 -> {
                    ...
                    openInterface(
                        player,
                        if (node.id == Items.RATS_LETTER_11009) Components.SUROK_LETTER1_249 else Components.SUROK_LETTER2_250
                    )
                }
            }
            return@on true
        ...
        /*
         * Search the bookcase in Varrock Library for Dagon hai history book.
         */
