    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            START_DIALOGUE -> npcl(FacialExpression.NEUTRAL, "Welcome to Varrock Museum!").also {
                if (player.getQuestRepository().points >= 50 && !hasDiaryTaskComplete(player, DiaryType.VARROCK, 0, 12)) {
            ...
            13 -> npcl(FacialExpression.FRIENDLY, "The letter here says you name is ${player.username}.").also { stage++ }
            14 -> npcl(FacialExpression.FRIENDLY, "Well ${player.username}, I wouldn't normally do this for just anyone, but as you did us such a great service with the Shield of Arrav I don't see why not.").also { stage++ }
            15 -> npcl(FacialExpression.FRIENDLY, "Run this letter back to the Examiner to begin your adventure into the world of Earth Sciences. Enjoy your studies, Student!").also { stage++ }
            16 -> npc(FacialExpression.FRIENDLY, "There you go, good luck student... Be sure to come", "back and show me your certificates. I would like to see", "how you get on.").also {
                if (removeItem(player, Items.UNSTAMPED_LETTER_682)) {
                    addItemOrDrop(player, Items.SEALED_LETTER_683)
                }
                stage++
            }
            17 -> playerl(FacialExpression.FRIENDLY, "Ok, I will. Thanks, see you later.").also {
                if(getQuestStage(player, "The Dig Site") == 1) {
