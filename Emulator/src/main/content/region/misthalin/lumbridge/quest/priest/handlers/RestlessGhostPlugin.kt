...
class RestlessGhostPlugin : OptionHandler() {
    ...
    override fun handle(player: Player, node: Node, option: String): Boolean {
        ...
        when (option) {
            ...
            "search" -> when (id) {
                ...
                15051 -> {
                    if (!isQuestComplete(player, QuestName.THE_RESTLESS_GHOST) && !inBank(player, Items.SKULL_964) && !inInventory(player, Items.SKULL_964)) {
                        addItem(player, Items.SKULL_964)
                        sendMessage(player, "You find another skull.")
                    }
                    setQuestStage(player, QuestName.THE_RESTLESS_GHOST, 40)
                }
    ...
    private fun toggleCoffin(player: Player, scenery: Scenery) {
        ...
        if (open && !isQuestComplete(player, QuestName.THE_RESTLESS_GHOST)) {
