...
class ImpCatcher : Quest("Imp Catcher", 21, 20, 1, Vars.VARP_QUEST_IMP_CATCHER_PROGRESS, 0, 1, 2) {
    ...
    override fun drawJournal(player: Player, stage: Int) {
        ...
        if (getStage(player) == 0) {
            ...
            line++
            line(player, if (!inInventory(player, Items.BLACK_BEAD_1474)) "!!1 Black Bead??" else "<str>1 Black Bead", line++)
            line(player, if (!inInventory(player, Items.RED_BEAD_1470)) "!!1 Red Bead??" else "<str>1 Red Bead", line++)
            line(player, if (!inInventory(player, Items.WHITE_BEAD_1476)) "!!1 White Bead??" else "<str>1 White Bead", line++)
            line(player, if (!inInventory(player, Items.YELLOW_BEAD_1472)) "!!1 Yellow Bead??" else "<str>1 Yellow Bead", line++)
        } else {
            line++
