...
class TzhaarMejJalDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun open(vararg args: Any): Boolean {
        npc = args[0] as NPC
        player(FacialExpression.HALF_GUILTY, "I have a fire cape here.")
        stage = 0
        return true
    }
            ...
            1 -> when (buttonId) {
                1 -> {
                    ...
                    if (!removeItem(player, Item(Items.FIRE_CAPE_6570))) {
                        npc(FacialExpression.OLD_NORMAL, "You not have firecape, JalYt.")
                        return true
                    } else {
                        npc(FacialExpression.OLD_NORMAL, "Hand your cape here, young JalYte.")
