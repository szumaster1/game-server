...
class TzhaarFightPitsPlugin : ActivityPlugin("fight pits", false, true, true, ZoneRestriction.CANNON) {
    ...
    companion object {
        ...
        private const val INTERFACE_ID = 373
        ...
        private fun sendDialogue(string: String) {
            for (p in WAR_PLAYERS) {
                p.dialogueInterpreter.sendDialogues(2618, FacialExpression.HALF_GUILTY, string)
            }
        }
            ...
            when (stage) {
                1 -> for (p in WAR_PLAYERS) {
                    ...
                    while (i < 2 + ((minutes - 4) shr 2)) {
                        val n = NPC.create(2734, zoneDestination)
                        n.init()
                        n.isAggressive = true
                ...
                2 -> for (p in WAR_PLAYERS) {
                    var i = 0
                    while (i < 2 + ((minutes - 4) shr 2)) {
                        val n = NPC.create(2739, zoneDestination)
                        n.init()
                        n.properties.combatPulse.attack(p)
                        i++
                    }
        ...
        private fun startGameSession() {
            ...
            if (lastVictor != null) {
                victor = "Current Champion: " + championName // JalYt-Mej-" +
        ...
        private val championName: String
            get() {
                ...
                for (i in 5..22) {
                    var skill: Int
                    if ((lastVictor!!.getSkills().getStaticLevel(i)
                            .also { skill = it }) > strength && skill > defence
                    ) {
                        if (++count == 5) {
        ...
        fun addTokkul(player: Player?) {
            ...
            if (player!!.equipment.getNew(EquipmentContainer.SLOT_CAPE).id == 6570) {
                amount *= 2
            }
            if (amount > 0) {
                if (!player.inventory.add(Item(6529, amount))) {
                    player.packetDispatch.sendMessage("Your Tokkul reward was added to your bank.")
                    player.bank.add(Item(6529, amount))
                }
            }
        }
