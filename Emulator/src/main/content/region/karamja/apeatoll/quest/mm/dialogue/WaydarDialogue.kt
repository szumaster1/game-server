...
class WaydarDialogue: DialogueFile(){
    ...
    override fun handle(componentID: Int, buttonID: Int) {
        when (stage) {
            0 -> {
                if (getQuestStage(player!!, QuestName.MONKEY_MADNESS) <= 22)
...
class WaydarCrashIslandDialogue: DialogueFile(){
    ...
    override fun handle(componentID: Int, buttonID: Int) {
        ...
        when (stage) {
            0 -> {
                if (getQuestStage(player!!, QuestName.MONKEY_MADNESS) == 24) {
            ...
            22 -> npcl(FacialExpression.OLD_NORMAL,"Enough. Return here when you are done.").also {
                ...
                setQuestStage(player!!, QuestName.MONKEY_MADNESS, 25)
            }
        }
    }
}
