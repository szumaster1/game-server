...
class LumdoDialogue: DialogueFile() {
    ...
    override fun handle(componentID: Int, buttonID: Int) {
        when (stage) {
            ...
            6 -> sendItemDialogue(player!!, Items.GNOME_ROYAL_SEAL_4004, "You show Lumdo the Royal Seal.").also { stage++ }
            ...
            12 -> npcl("We were one gnome to a glider, so each was extremely light. Like leaves in a wind we were blown south before we even landed on the island.").also { stage++ }
            13 -> playerl("Did you crash straight here?").also { stage++ }
            14 -> npcl("Yes. The winds drove us into the treetops, which destroyed the canvas of our gliders. We dragged what remained of the gliders out onto this beach.").also { stage++ }
            15 -> playerl("What did you do then?").also { stage++ }
            16 -> npcl("Whilst we were falling, Sergeant Garkor noticed a large populated atoll to our west. You cannot see it from here, but it is within sailing distance.").also { stage++ }
            17 -> playerl("Presumably you are to guard the gliders until they return?").also { stage++ }
            18 -> npcl("Affirmative.").also { stage++ }
            19 -> playerl("You must take us to the island. I have orders from the High Tree Guardians to make contact with your Sergeant.").also { stage++ }
