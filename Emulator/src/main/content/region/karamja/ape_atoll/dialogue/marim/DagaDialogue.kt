package content.region.karamja.ape_atoll.dialogue.marim
...
class DagaDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun open(vararg args: Any?): Boolean {
        npc = args[0] as NPC
        npc(FacialExpression.OLD_DEFAULT, "Would you like to buy or sell some scimitars?")
        return true
    }
            ...
            32 -> when (buttonId) {
                1 -> {
                    if(!removeItem(player, Item(Items.COINS_995, 100000))){
                        ...
                        npcl(FacialExpression.OLD_NORMAL, "Sorry but you don't have enough to buy one, at the moment it costs 100,000 gold coins.")
                    } else {
