...
import content.region.karamja.ape_atoll.quest.mm.cutscenes.DungeonPlanCutScene
import core.api.*
...
/**
 * Represents the Zooknock dialogue file.
 */
class ZooknockDialogueFile(val it: Int) : DialogueFile() {
    ...
    override fun handle(componentID: Int, buttonID: Int) {
        ...
        when (stage) {
            ...
            2 -> npcl(FacialExpression.ASKING, "Nicely done!").also { stage++ }
            3 -> {
                if (validateTalismanItemsGiven() != null) {
                    npcl(FacialExpression.OLD_CALM_TALK1, validateTalismanItemsGiven()).also { stage = END_DIALOGUE }
                } else {
            ...
            6 -> sendItemDialogue(player!!, Items.MONKEY_GREEGREE_4024, "Zooknock hands you back the talisman. It seems to glow slightly.").also { // Air blast (end) graphics
                addItemOrDrop(player!!, Items.MONKEY_GREEGREE_4024, 1)
                npc!!.graphics(Graphic(Graphics.LIGHTENING_AND_A_PURPLE_PINK_CIRCLE_134, 96))
                stage = 8
            }
            ...
            10 -> npcl("Furthermore, you will not be able to attack whilst using this, so be careful. Perhaps when I refine my spells I could look into making this possible.").also {
                ...
                content.region.karamja.ape_atoll.quest.mm.cutscenes.DungeonPlanCutScene(player!!).start()
                setQuestStage(player!!, "Monkey Madness", 32)
                stage = 14
            }
            14 -> {
                ...
                player!!.packetDispatch.sendItemZoomOnInterface(Items.MSPEAK_AMULET_4022, 230, interfaceIdentification, 5)

                for (i in 0..17) {
                    when (i) {
                        3 -> player!!.packetDispatch.sendString("Monkey Madness: Chapter 3", interfaceIdentification, i)
                        9 -> player!!.packetDispatch.sendString("In which our ${if (player!!.isMale) "hero finds himself" else "heroine finds herlself"} contending with life as a", interfaceIdentification, i)
            ...
            16 -> npc(FacialExpression.OLD_CALM_TALK1, "Nicely done.").also { stage++ }
            17 -> {
                if (validateAmuletItemsGiven() != null) {
                    npcl(FacialExpression.OLD_CALM_TALK1, validateAmuletItemsGiven()).also { stage = END_DIALOGUE }
                } else {
            ...
            31 -> npc(FacialExpression.OLD_CALM_TALK1, "Nicely done.").also { stage++ }
            32 -> {
                if (validateAmuletItemsGiven() != null) {
                    npcl(FacialExpression.OLD_CALM_TALK1, validateAmuletItemsGiven()).also { stage = END_DIALOGUE }
                } else {
            ...
            43 -> npcl(FacialExpression.OLD_CALM_TALK1, "Somewhere in the village. It out to be obvious. Now give me a moment.").also {
                npc!!.graphics(Graphic(Graphics.LIGHTENING_AND_A_PURPLE_PINK_CIRCLE_134, 96))
                stage++
            }
            44 -> {
    ...
    private fun validateTalismanItemsGiven(): String? {
        ...
        return when {
            ...
            missingItems.size == 1 -> "We still need the ${missingItems.first().name}."
                ...
                "We still need the ${Item(Items.MONKEY_BONES_3179).name} and the ${Item(Items.MONKEY_TALISMAN_4023).name}."
            }
        }
    }
}
