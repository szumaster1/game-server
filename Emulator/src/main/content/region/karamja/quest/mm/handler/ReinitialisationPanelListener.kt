package content.region.karamja.quest.mm.handler
        ...
        on(Components.TRAIL_PUZZLE_363) { player, _, _, buttonID, slot, _ ->
            ...
            when (buttonID) {
                6 -> {
                    ...
                    setAttribute(player, "/save:MonkeyMadnessPuzzleState", currentPiecesPosition)
                    if (isPuzzleSolved(currentPiecesPosition, puzzlePieces)) {
                        setAttribute(player, "/save:MonkeyMadnessPuzzleSolved", true)
                        //Shake camera + cutscene
                    }
                }
            }
            return@on true
