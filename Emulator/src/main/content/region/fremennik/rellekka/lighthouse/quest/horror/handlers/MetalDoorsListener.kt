...
class MetalDoorsListener : InteractionListener, InterfaceListener {
    ...
    override fun defineListeners() {
        ...
        on(HorrorUtils.METAL_DOORS, IntType.SCENERY, "study") { player, _ ->
            when (player.location.y) {
                4626 -> {
                    if (getQuestStage(player, "Horror from the Deep") >= 50) {
                        ...
                        openInterface(player, 142)
                        setAttribute(player, HorrorUtils.STRANGE_WALL_DISCOVER, true)
                    }
                }
        ...
        onUseWith(IntType.SCENERY, HorrorUtils.STRANGE_W_REQ_ITEMS, *HorrorUtils.METAL_DOORS) { player, used, _ ->
        ...
        on(HorrorUtils.STRANGE_DOORS, IntType.SCENERY, "open") { player, node ->
            if (getQuestStage(player, "Horror from the Deep") >= 50) {
                queueScript(player, 1, QueueStrength.SOFT) { stage: Int ->
                    ...
                    if (stage == 1) {
                        ...
                        removeAttribute(player, HorrorUtils.STRANGE_WALL_DISCOVER)
    ...
    override fun defineInterfaceListeners() {
        ...
        onOpen(Components.HORROR_METALDOOR_142) { player, _ ->
            setComponentVisibility(player, 142, 2, getAttribute(player, HorrorUtils.STRANGE_W_FIRE, 0) != 1)
            setComponentVisibility(player, 142, 3, getAttribute(player, HorrorUtils.STRANGE_W_AIR, 0) != 1)
            setComponentVisibility(player, 142, 4, getAttribute(player, HorrorUtils.STRANGE_W_EARTH, 0) != 1)
            setComponentVisibility(player, 142, 5, getAttribute(player, HorrorUtils.STRANGE_W_WATER, 0) != 1)
            setComponentVisibility(player, 142, 6, getAttribute(player, HorrorUtils.STRANGE_W_ARROW, 0) != 1)
            setComponentVisibility(player, 142, 7, getAttribute(player, HorrorUtils.STRANGE_W_SWORD, 0) != 1)

            if (getAttribute(player, HorrorUtils.PUZZLE_PROGRESS, 0) > 5) {
