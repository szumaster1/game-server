...
class StrangeWallListener : InteractionListener, InterfaceListener {
    ...
    override fun defineListeners() {
        ...
        on(STRANGE_WALL, IntType.SCENERY, "study") { player, _ ->
            when (player.location.y) {
                4626 -> {
                    if (getQuestStage(player, QuestName.HORROR_FROM_THE_DEEP) >= 50) {
            ...
            if (getQuestStage(player, QuestName.HORROR_FROM_THE_DEEP) >= 50) {
                queueScript(player, 1, QueueStrength.SOFT) { stage: Int ->
                    ...
                    if (stage == 1) {
                        ...
                        setQuestStage(player, QuestName.HORROR_FROM_THE_DEEP, 55)
        ...
        onOpen(Components.HORROR_METALDOOR_142) { player, _ ->
            ...
            if (getAttribute(player, HorrorFromTheDeepUtils.UNLOCK_DOOR, 0) > 5) {
                ...
                queueScript(player, 1, QueueStrength.SOFT) {
                    ...
                    setQuestStage(player, QuestName.HORROR_FROM_THE_DEEP, 50)
                    return@queueScript stopExecuting(player)
                }
            }
            return@onOpen true
    ...
    companion object {
        ...
        private val STRANGE_W_REQ_ITEMS = intArrayOf(Items.BRONZE_ARROW_882, Items.BRONZE_SWORD_1277, Items.AIR_RUNE_556, Items.FIRE_RUNE_554, Items.EARTH_RUNE_557, Items.WATER_RUNE_555)
    }
}
