...
/**
 * Horror from the deep quest. (Lighthouse Quest)
 */
@Initializable
class HorrorFromTheDeep : Quest(QuestName.HORROR_FROM_THE_DEEP, 77, 76, 2, Vars.VARBIT_QUEST_HORROR_FROM_THE_DEEP_PROGRESS, 0, 1, 10) {
    ...
    override fun drawJournal(player: Player, stage: Int) {
        ...
        if (stage >= 1) {
            ...
            line(player, "and repaired the !!bridge?? to Rellekka with some planks.", line++, getAttribute(player, HorrorFromTheDeepUtils.UNLOCK_BRIDGE, 0) == 2)
            line++
        }
        if (stage >= 20) {
    ...
    override fun finish(player: Player) {
        ...
        player ?: return
        var ln = 10
        player.packetDispatch.sendString("You have survived the ${QuestName.HORROR_FROM_THE_DEEP}!", 277, 4)
