...
class LighthouseBridgeListener : InteractionListener {
    ...
    override fun defineListeners() {
        ...
        onUseWith(IntType.SCENERY, Items.PLANK_960, HorrorUtils.BROKEN_BRIDGE_1) { player, _, _ ->
            if (getQuestStage(player, "Horror from the Deep") == 0) {
                sendDialogue(player, "That won't help fix the bridge.")
                return@onUseWith false
            }
            if (getQuestStage(player, "Horror from the Deep") >= 10 && getAttribute(player, HorrorUtils.fixBridge, 0) == 1) {
            ...
            queueScript(player, 1, QueueStrength.SOFT) {
                setAttribute(player, HorrorUtils.fixBridge, 1)
                animate(player, 898)
        ...
        onUseWith(IntType.SCENERY, Items.PLANK_960, HorrorUtils.BROKEN_BRIDGE_2) { player, _, _ ->
            if (getQuestStage(player, "Horror from the Deep") < 10) {
                sendDialogue(player, "That won't help fix the bridge.")
                return@onUseWith false
            }

            if (getAttribute(player, HorrorUtils.fixBridge, 0) == 1 && !inInventory(player, Items.HAMMER_2347, 1) && inInventory(player, Items.PLANK_960, 1) && amountInInventory(player, Items.STEEL_NAILS_1539) >= 30) {
                sendDialogue(player, "You need a hammer to force the nails in with.")
                return@onUseWith false
            }
            ...
            removeItem(player, Item(Items.PLANK_960, 1))
            queueScript(player, 1, QueueStrength.SOFT) {
                setAttribute(player, HorrorUtils.fixBridge, 2)
                animate(player, 898)
        ...
        on(HorrorUtils.BROKEN_BRIDGE_1, IntType.SCENERY, "Cross") { player, _ ->
            if (getAttribute(player, HorrorUtils.fixBridge, 0) == 1) {
                lock(player, 6)
            ...
            if (getAttribute(player, HorrorUtils.fixBridge, 0) == 2 || isQuestComplete(player, "Horror from the Deep")) {
                lock(player, 6)
        ...
        on(HorrorUtils.BROKEN_BRIDGE_2, IntType.SCENERY, "Cross") { player, _ ->
            if (getAttribute(player, HorrorUtils.fixBridge, 0) == 2 || isQuestComplete(player, "Horror from the Deep")) {
                lock(player, 6)
                submitIndividualPulse(player, object : Pulse(1) {
                    var count = 0
