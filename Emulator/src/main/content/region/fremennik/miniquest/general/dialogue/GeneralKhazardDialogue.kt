...
class GeneralKhazardDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun open(vararg args: Any): Boolean {
        ...
        */
        ...
        } else if (getAttribute(player, GeneralShadowUtils.GS_START, false)) {
            npc(FacialExpression.OLD_NORMAL,"Do you make progress in finding my scouts?").also { stage = 34 }
        } else if (getAttribute(player, GeneralShadowUtils.GS_PROGRESS, 0) == 4) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            34 -> if (getAttribute(player, GeneralShadowUtils.GS_PROGRESS, 0) == 0) {
                player("I haven't found any. Where were they headed?").also { stage++ }
            } else {
                player("I've found ${getAttribute(player, GeneralShadowUtils.GS_PROGRESS, 0)}, but they're kind of hard to find.", "Where were they headed again?").also { stage++ }
            }
