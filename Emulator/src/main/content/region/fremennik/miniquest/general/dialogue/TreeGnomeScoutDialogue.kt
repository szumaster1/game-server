...
class TreeGnomeScoutDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun open(vararg args: Any?): Boolean {
        if (getAttribute(player, GeneralShadowUtils.GS_PROGRESS, 0) == 3 && inEquipment(player, Items.GHOSTSPEAK_AMULET_552)) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            4 -> npc(FacialExpression.ANNOYED, "I'll be glad when all this is over. The General will be", "strong again and we will push back this tide of filth that", "is humanity.").also { stage++ }
            ...
            8 -> if (isQuestComplete(player, QuestName.BIOHAZARD) || isQuestComplete(player, QuestName.PLAGUE_CITY)) {
            ...
            10 -> if (isQuestComplete(player, QuestName.HAZEEL_CULT)) {
            ...
            11 -> if (isQuestComplete(player, QuestName.THE_GRAND_TREE)) {
