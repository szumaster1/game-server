...
class AhabDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            0 -> options("Arrr!", "Are you going to sit there all day?", "Do you have anything for trade?").also { stage++ }
            1 -> when (buttonId) {
            ...
            24 -> npc("Nay, young laddie, this be a false leg. For years I had me a", "sturdy wooden peg-leg, but now I wear this dainty little", "feller.").also { stage++ }
            ...
            38 -> npc("Nothin' at the moment, but then again the Customs", "Agents are on the warpath right now.").also { stage = END_DIALOGUE }
