    ...
    fun handleBall(player: Player) {
        if (player.getAttribute("witchs_house:experiment_killed", false)) {
            if (player.inventory.containsItem(Item(2407))) {
                ...
                player.packetDispatch.sendMessage(RED + "The experiment glares at you, and you feel yourself weaken.</col>")
                WitchExperimentNPC(player.getAttribute("witchs_house:experiment_id", 897), Location.create(2936, 3463, 0), player).init()
