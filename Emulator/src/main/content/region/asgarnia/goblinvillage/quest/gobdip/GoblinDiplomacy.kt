package content.region.asgarnia.goblinvillage.quest.gobdip
...
class GoblinDiplomacy :
    ...
    override fun getConfig(player: Player, stage: Int): IntArray {
        if (stage == 0) {
            return intArrayOf(Vars.VARP_QUEST_GOBLIN_DIPLOMACY_QUEST_PROGRESS, 0)
        }
        return when (stage) {
            10 -> intArrayOf(Vars.VARP_QUEST_GOBLIN_DIPLOMACY_QUEST_PROGRESS, 1)
            20 -> intArrayOf(Vars.VARP_QUEST_GOBLIN_DIPLOMACY_QUEST_PROGRESS, 4)
            30 -> intArrayOf(Vars.VARP_QUEST_GOBLIN_DIPLOMACY_QUEST_PROGRESS, 5)
            100 -> intArrayOf(Vars.VARP_QUEST_GOBLIN_DIPLOMACY_QUEST_PROGRESS, 6)
            else -> intArrayOf(Vars.VARP_QUEST_GOBLIN_DIPLOMACY_QUEST_PROGRESS, 0)
        }
    }
