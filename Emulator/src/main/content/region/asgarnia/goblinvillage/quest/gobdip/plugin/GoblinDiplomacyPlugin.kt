package content.region.asgarnia.goblinvillage.quest.gobdip.plugin

import content.region.asgarnia.goblinvillage.quest.gobdip.plugin.GoblinDiplomacyPlugin.GoblinMailPlugin.GoblinMail
...
class GoblinDiplomacyPlugin : OptionHandler() {
    ...
    class GoblinMailPlugin : UseWithHandler(1763, 1769, 1765, 1771, 1767, 1773, 4622, 11808, 6955) {
        ...
        override fun handle(event: NodeUsageEvent): Boolean {
            ...
            if (player.inventory.remove(mail.dye, event.baseItem)) {
                player.inventory.add(mail.product)
                player.packetDispatch.sendMessage("You dye the goblin armour " + mail.name.lowercase(Locale.getDefault()) + ".")
            }
            return true
        }
