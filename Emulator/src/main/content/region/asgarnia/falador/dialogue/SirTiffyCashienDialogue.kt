...
class SirTiffyCashienDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun open(vararg args: Any?): Boolean {
        npc = args[0] as NPC
        if (isQuestInProgress(player!!, "Recruitment Drive", 1, 99)) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            3 -> {
                if(!isQuestComplete(player, "Recruitment Drive")){
                    playerl(FacialExpression.HALF_GUILTY, "Erm...goodbye.").also { stage = END_DIALOGUE }
                } else {
                    options("Do you have any jobs for me yet?", "Can you explain the Gaze of Saradomin to me?", "Can I buy some armours?", "Can I switch respawns please?", "Goodbye.").also { stage = 10 }
                }
            }
            ...
            10 -> when (buttonId) {
                ...
                4 -> if(player.properties.spawnLocation== Configuration.HOME_LOCATION){
                        playerl(FacialExpression.FRIENDLY, "Hi Tiffy, I was wondering, can I change my respawn point to Falador?").also { stage = 22 }
                    } else {
                    playerl(FacialExpression.FRIENDLY, "Can I change my respawn point back to Lumbridge?").also { stage = 21 }
                }
                5 -> playerl(FacialExpression.FRIENDLY, "Goodbye.").also { stage = END_DIALOGUE }
            }
            ...
            12 -> npcl(FacialExpression.FRIENDLY, "Anything else I can do for you in the meantime?").also { stage = 10 }
            ...
            18 -> npcl(FacialExpression.FRIENDLY, "Was there something else you wanted to ask good old Tiffy, " + (if (player!!.isMale) "sirrah" else "milady") + "?").also { stage = 10 }
            ...
            21 -> npcl(FacialExpression.HALF_GUILTY,"What? You're saying you want to respawn in Lumbridge? Are you sure?").also { stage = 26 }
            22 -> npcl(FacialExpression.FRIENDLY, "Ah, so you'd like to respawn in Falador, the good old homestead! Are you sure?").also { stage++ }
            ...
            24 -> when(buttonId){
                ...
                2 -> playerl(FacialExpression.FRIENDLY, "Actually, no thanks. I like my respawn point.").also { stage = 29 }
            }
            25 -> {
            ...
            28 -> {
                ...
                player.properties.spawnLocation = Configuration.HOME_LOCATION
                stage = END_DIALOGUE
            }
            29 -> npcl(FacialExpression.FRIENDLY, " As you wish, what? Ta-ta for now.").also { stage = END_DIALOGUE }
        }
        return true
    }
