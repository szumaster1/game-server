...
class MissCheeversRoomListener : InteractionListener {
    ...
    override fun defineListeners() {
        val searchActions = mapOf(
            Scenery.OLD_BOOKSHELF_7327 to { player: Player ->
                RecruitmentDriveUtils.searchingHelper(player, magnet, Items.MAGNET_5604, "You search the bookshelves...", "Hidden amongst the books you find a magnet.")
            },
            Scenery.OLD_BOOKSHELF_7328 to { player: Player ->
                if (getAttribute(player, "/save:rd:help", -1) < 3) {
                    ...
                    RecruitmentDriveUtils.searchingHelper(player, book, Items.ALCHEMICAL_NOTES_5588, "You search the bookshelves...", "You find a book that looks like it might be helpful.")
                }
            },
            Scenery.OLD_BOOKSHELF_7329 to { player: Player ->
                RecruitmentDriveUtils.searchingHelper(player, knife, Items.KNIFE_5605, "You search the bookshelves...", "Hidden amongst the books you find a knife.")
            },
            Scenery.OLD_BOOKSHELF_7330 to { player: Player ->
                RecruitmentDriveUtils.searchingHelper(player, "", 0, "You search the bookshelves...", "")
            }
        )
        ...
        crateInteractions.forEach { (scenery, attributes) ->
            on(scenery, IntType.SCENERY, "search") { player, node ->
                ...
                if (node.location == RecruitmentDriveUtils.getLocationForScenery(node.asScenery())) {
                    RecruitmentDriveUtils.searchingHelper(player, attribute, item, "You search the crate...", "Inside the crate you find a ${getItemName(item).lowercase()}.")
                } else {
                    RecruitmentDriveUtils.searchingHelper(player, "", 0, "You search the crate...", "")
                }
                return@on true
            }
        }
        ...
        on(Scenery.OPEN_CHEST_7351, IntType.SCENERY, "search") { player, _ ->
            RecruitmentDriveUtils.searchingHelper(player, shears, Items.SHEARS_5603, "You search the chest...", "Inside the chest you find some shears.")
            return@on true
        }
        ...
        onUseWith(IntType.ITEM, Items.TIN_5600, Items.GYPSUM_5579) { player, used, with ->
            RecruitmentDriveUtils.processItemUsageAndReturn(player, used.asItem(), with.asItem(), Item(Items.TIN_5592))
        ...
        onUseWith(IntType.ITEM, Items.TIN_5592, Items.VIAL_OF_LIQUID_5582) { player, used, with ->
            RecruitmentDriveUtils.processItemUsage(player, used.asItem(), with.asItem(), Item(Items.TIN_5593))
        ...
        onUseWith(IntType.ITEM, Items.TIN_5594, *potionIDs) { player, used, with ->
            RecruitmentDriveUtils.processItemUsageAndReturn(player, used.asItem(), with.asItem(), Item(Items.TIN_5595))
            return@onUseWith true
        }
        ...
        onUseWith(IntType.ITEM, Items.TIN_5595, *potionIDs) { player, used, with ->
            RecruitmentDriveUtils.processItemUsageAndReturn(player, used.asItem(), with.asItem(), Item(Items.TIN_5596))
            return@onUseWith true
        }
        ...
        onUseWith(IntType.SCENERY, DoorVials.doorVialsArray, Scenery.STONE_DOOR_7344) { player, used, _ ->
            RecruitmentDriveUtils.handleVialUsage(player, used.asItem())
            return@onUseWith true
        }
        ...
        on(Scenery.STONE_DOOR_7344, SCENERY, "pull-spade") { player, _ ->
            RecruitmentDriveUtils.handleSpadePull(player)
            return@on true
        }
        ...
        on(Scenery.OPEN_DOOR_7345, SCENERY, "walk-through") { player, _ ->
            RecruitmentDriveUtils.handleDoorWalkThrough(player)
            return@on true
        }
    ...
    private fun updatePlayerAttribute(player: Player, count: Int) {
        if (specialAttribute != null) {
            ...
            flaskIdsArray.forEach { id ->
                setAttribute(player, MissCheeversRoomListener.Companion.Vials.vialMap[id]!!.attribute, true)
            }
        }
    }
