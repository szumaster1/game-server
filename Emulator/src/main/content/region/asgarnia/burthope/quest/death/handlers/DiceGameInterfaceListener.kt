...
class DiceGameInterfaceListener : InterfaceListener {
    ...
    override fun defineInterfaceListeners() {
        onOpen(Components.DEATH_DICE_99) { player, _ ->
            var goldID = 0
            var wager = player.getAttribute("deathplateau:wager", 50)
            ...
            sendString(player, "Harold rolls...", Components.DEATH_DICE_99, 13)
            preRoll(player)
            sendString(player, player.username, Components.DEATH_DICE_99, 6)
    ...
    private fun submitRollPulse(player: Player, isNPCTurn: Boolean) {
        ...
        if (isNPCTurn) {
            roll1 = getAttribute<Int?>(player,
                ATTRIBUTE_HAROLD_DICE_1, null) ?: return
            roll2 = getAttribute<Int?>(player,
                ATTRIBUTE_HAROLD_DICE_2, null) ?: return
        } else {
            roll1 = getAttribute<Int?>(player,
                ATTRIBUTE_PLAYER_DICE_1, null) ?: return
            roll2 = getAttribute<Int?>(player,
                ATTRIBUTE_PLAYER_DICE_2, null) ?: return
        }
