package content.region.asgarnia.burthope.quest.hero.handlers
...
class HeroesQuestListener : InteractionListener {
    ...
    override fun defineListeners() {
        on(Scenery.DOOR_2626, IntType.SCENERY, "open") { player, node ->
            if (getQuestStage(player, QuestName.HEROES_QUEST) >= 2 && getAttribute(player,
                    HeroesQuest.attributeGruborLetsYouIn, false) && HeroesQuest.isBlackArm(player)
            ) {
        ...
        on(Scenery.DOOR_2628, IntType.SCENERY, "open") { player, node ->
            if (getQuestStage(player, QuestName.HEROES_QUEST) >= 3 && HeroesQuest.isPhoenix(player)) {
                DoorActionHandler.handleAutowalkDoor(player, node.asScenery())
            } else {
        ...
        on(Scenery.WALL_2629, IntType.SCENERY, "push") { player, node ->
            if (getQuestStage(player, QuestName.HEROES_QUEST) >= 4 && HeroesQuest.isPhoenix(player)) {
                DoorActionHandler.handleAutowalkDoor(player, node.asScenery())
            } else {
        ...
        on(Scenery.DOOR_2627, IntType.SCENERY, "open") { player, node ->
            if (getQuestStage(player, QuestName.HEROES_QUEST) >= 4 && HeroesQuest.isBlackArm(player)) {
                DoorActionHandler.handleAutowalkDoor(player, node.asScenery())
            } else {
        ...
        on(Scenery.CHEST_2633, IntType.SCENERY, "search") { player, _ ->
            if (inInventory(player, Items.PETES_CANDLESTICK_1577)) {
                ...
                if (getQuestStage(player, QuestName.HEROES_QUEST) == 4) {
                    setQuestStage(player, QuestName.HEROES_QUEST, 5)
                }
