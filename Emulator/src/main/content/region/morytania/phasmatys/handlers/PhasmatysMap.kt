 ...
 */
@Initializable
class PhasmatysArea : MapZone("Port phasmatys", true), Plugin<Any?> {
    ...
    override fun interact(e: Entity, target: Node, option: Option): Boolean {
        if (e is Player) {
            ...
            if (target is NPC) {
                ...
                if ((target.name.lowercase().contains("ghost") || target.name.equals("velorina", ignoreCase = true) || target.name.contains("husband")) && !inEquipment(player, Items.GHOSTSPEAK_AMULET_552)) {
    ...
    private fun worship(player: Player?) {
        val bone = player!!.inventory.toArray().firstNotNullOfOrNull { Bones.forBoneMeal(it?.id ?: -1) }
        ...
        if (removeItem(player, Item(bone.bonemealId!!)) && removeItem(player, Item(Items.BUCKET_OF_SLIME_4286, 1))) {
            lock(player, 1)
            animate(player, Animations.ECTOFUNTUS_PRAYING_1651)
