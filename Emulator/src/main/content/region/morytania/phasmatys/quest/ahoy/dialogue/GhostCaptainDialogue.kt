...
/**
 * Represents the Ghost Captain dialogue.
 */
@Initializable
class GhostCaptainDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            1 -> npc("I can take you to Dragontooth Island, which is a most", "pleasant island in the sea between Morytania and the", "lands of the east.").also { stage++ }
            2 -> player("How much does it cost?").also { stage++ }
            3 -> npc("It'll cost you 25 ectotokens for the return trip.").also { stage++ }
            4 -> if (amountInInventory(player, Items.ECTO_TOKEN_4278) >= 25) {
                options("I have the ectotokens here - please take me to Dragontooth Island.", "No thanks, not at the moment.").also { stage++ }
            } else {
            ...
            5 -> when (buttonId) {
                1 -> player("I have the ectotokens here - please take me", "to Dragontooth Island.").also { stage++ }
