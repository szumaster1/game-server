...
import content.region.morytania.quest.fenk.npc.GardenerGhostNPC
import core.api.*
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        ...
        if (gardenerGhost.target == player) {
            ...
            if (gardenerGhost.location.withinDistance(gardenerGhost.graveLocation, 5)) {
                ...
                sendChat(gardenerGhost, "Go " + gardenerGhost.location.deriveDirection(gardenerGhost.graveLocation).name.lowercase().replace('_', '-') + ", mate")
...
class GardenerGhostDialogueFile : DialogueBuilderFile() {
    override fun create(b: DialogueBuilder) {
        b.onPredicate { player -> !player.equipment.containsAtLeastOneItem(Items.GHOSTSPEAK_AMULET_552) }
            ...
            .end()
        b.onQuestStages("Creature of Fenkenstrain", *(0..8).toIntArray(), 100)
            .options().let { optionBuilder ->
                optionBuilder.option("Tell me about Fenkenstrain.")
                    ...
                    .item(
                        ...
                        "You feel power emanate from the Amulet of Ghostspeak",
                        "and the air around you vibrates with the ghostly voice",
                        "of the headless gardener."
                    )
                    ...
                    .end()
                optionBuilder.optionIf("Do you know where the key to the shed is?") { player ->
                ...
                optionBuilder.optionIf("Do you know where I can find a lightning conductor mould is?") { player ->
                    return@optionIf getQuestStage(
                        player,
                        "Creature of Fenkenstrain"
                    ) == 4
                ...
                optionBuilder.optionIf("What's your name?") { player ->
                    return@optionIf getQuestStage(
                        player,
                        "Creature of Fenkenstrain"
                    ) < 4
