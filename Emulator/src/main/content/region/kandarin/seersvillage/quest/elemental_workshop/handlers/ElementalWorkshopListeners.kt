...
import content.region.kandarin.seersvillage.quest.elemental_workshop.handlers.EWUtils.BELLOWS_STATE
import content.region.kandarin.seersvillage.quest.elemental_workshop.handlers.EWUtils.FURNACE_STATE
import content.region.kandarin.seersvillage.quest.elemental_workshop.handlers.EWUtils.LEFT_WATER_CONTROL_STATE
import content.region.kandarin.seersvillage.quest.elemental_workshop.handlers.EWUtils.RIGHT_WATER_CONTROL_STATE
import content.region.kandarin.seersvillage.quest.elemental_workshop.handlers.EWUtils.WATER_WHEEL_STATE
import content.region.kandarin.seersvillage.quest.elemental_workshop.handlers.EWUtils.currentStage
import core.api.*
    ...
    override fun defineListeners() {
        ...
        on(Scenery.BOOKCASE_26113, IntType.SCENERY, "search") { player, _ ->
            ...
            if (player.inventory.containsItem(slashedBook)) {
                sendItemDialogue(
                    player, Item(Items.SLASHED_BOOK_9715),
        ...
        onUseWith(IntType.ITEM, Items.KNIFE_946, Items.BATTERED_BOOK_2886) { player, _, with ->
            ...
            queueScript(player, 1, QueueStrength.SOFT) { stage ->
                when (stage) {
                    0 -> {
                        sendMessage(player, "You make a small cut in the spine of the book.")
        ...
        on(
            ...
            if (player.location == Location.create(2710, 3496, 0) || player.location == Location.create(
                    2709,
                    3496,
                    0
                )
        ...
        on(Scenery.BELLOWS_18516, IntType.SCENERY, "fix") { player, _ ->
