package content.region.kandarin.seersvillage.quest.elemental_workshop

import content.region.kandarin.seersvillage.quest.elemental_workshop.handlers.EWUtils
import core.api.*
...
class ElementalWorkshop : Quest("Elemental Workshop I", 52, 51, 1), Commands {
    ...
    override fun drawJournal(player: Player, stage: Int) {
        ...
        if (stage == 0) {
            ...
            line(
                player,
                if (getStatLevel(player, Skills.MINING) >= 20) "---Level 20 Mining/--" else "!!Level 20 Mining??",
                line++
            )
            line(
                player,
                if (getStatLevel(player, Skills.SMITHING) >= 20) "---Level 20 Smithing/--" else "!!Level 20 Smithing??",
                line++
            )
            line(
                player,
                if (getStatLevel(player, Skills.CRAFTING) >= 20) "---Level 20 Crafting/--" else "!!Level 20 Crafting??",
                line++
            )
        } else {
