package content.region.kandarin.seersvillage.quest.scorpcatcher
...
class ScorpionCatcher : Quest("Scorpion Catcher", 108, 107, 1, Vars.VARP_QUEST_SCORPION_CATCHER_PROGRESS, 0, 1, 6) {
    ...
    companion object {
        ...
        val ATTRIBUTE_NPC = "/save:scorpion_catcher:dialogues"
        val ATTRIBUTE_CAGE = "/save:scorpion_catcher:cage-dialogue"
        val ATTRIBUTE_TAVERLEY = "scorpion_catcher:caught_taverly"
        val ATTRIBUTE_BARBARIAN = "scorpion_catcher:caught_barb"
    ...
    override fun drawJournal(player: Player, stage: Int) {
        ...
        if (stage >= 10 && getAttribute(player, ATTRIBUTE_SECRET, false)) {
            line(player, "I've spoken to a !!Seer?? and been given the location of ", line++, getAttribute(player, ATTRIBUTE_SECRET, false) || stage == 100)
            line(player, " one of the !!Kharid Scorpions??. ", line++, getAttribute(player, ATTRIBUTE_SECRET, false) || stage == 100)
            line++
            line(player, "The first !!Kharid Scorpion?? is in a secret room near some", line++, getAttribute(player, ATTRIBUTE_TAVERLEY, false) || stage == 100)
            line(player, "!!nasty spiders?? with two !!coffins?? nearby", line++, getAttribute(player, ATTRIBUTE_TAVERLEY, false) || stage == 100)
            line++
            line(player, "I'll need to talk to a !!Seer?? again once I've caught the first", line++, getAttribute(player, ATTRIBUTE_NPC, false) || stage == 100)
            line(player, "!!Kharid Scorpion??.", line++, getAttribute(player, ATTRIBUTE_NPC, false) || stage == 100)
            line++
        }

        if (stage >= 10 && getAttribute(player, ATTRIBUTE_NPC, false) || getAttribute(player, ATTRIBUTE_BARBARIAN, false)) {
            line(player, "The second !!Kharid Scorpion?? has been in a !!village of??", line++, getAttribute(player, ATTRIBUTE_BARBARIAN, false) || stage == 100)
            line(player, "!!uncivilised-looking warriors in the east.?? It's been picked up", line++, getAttribute(player, ATTRIBUTE_BARBARIAN, false) || stage == 100)
            line(player, "by some sort of !!merchant??.", line++, getAttribute(player, ATTRIBUTE_BARBARIAN, false) || stage == 100)
            line++
        }

        if (stage >= 10 && getAttribute(player, ATTRIBUTE_NPC, false) || getAttribute(player, ATTRIBUTE_MONK, false)) {
            line(player, "The third !!Kharid Scorpion?? is in some sort of !!upstairs room??", line++,  getAttribute(player, ATTRIBUTE_MONK, false) || stage == 100)
            line(player, "with !!brown clothing on a table??.", line++,  getAttribute(player, ATTRIBUTE_MONK, false) || stage == 100)
            line++
        }

        if (stage >= 10 && getAttribute(player, ATTRIBUTE_TAVERLEY, false) && getAttribute(player, ATTRIBUTE_BARBARIAN, false) && getAttribute(player, ATTRIBUTE_MONK, false)) {
