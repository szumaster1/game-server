...
class PyreSitePlugin : OptionHandler() {
    ...
    override fun newInstance(arg: Any?): Plugin<Any> {
        ...
        PluginManager.definePlugin(FerociousBarbarianNPC())
        return this
    }
    ...
    private fun ritual(player: Player, `object`: Scenery) {
        player.lock()
        USED_LOCATIONS.add(`object`.location)
        player.faceLocation(`object`.location)
        GameWorld.Pulser.submit(getPulse(player, `object`))
    }

    private fun getPulse(player: Player, `object`: Scenery): Pulse {
        ...
        return object : Pulse(1, player) {
                    ...
                    if (objectId == 25291) {
                        if (player.inventory.remove(Item(logType.log.logId), bones)) {
                            ...
                            if (bones.id == CHEWED_BONES.id) {
                                ...
                                val barb = NPC.create(752, `object`.location.transform(`object`.direction, 1))
            ...
            override fun stop() {
                ...
                USED_LOCATIONS.remove(`object`.location)
            }
        }
    }
    ...
    /**
     * Represents the Ferocious Barbarian NPC.
     */
