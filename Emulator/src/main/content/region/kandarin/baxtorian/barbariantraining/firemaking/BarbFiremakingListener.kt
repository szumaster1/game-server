...
import content.global.skill.firemaking.Firemaking
...
class BarbFiremakingListener : InteractionListener {
    ...
    override fun defineListeners() {
        onUseWith(IntType.ITEM, (tools + crystalEquipment), *Firemaking.logs) { player, used, with ->
            ...
            if (completeBarbarianFiremaking || barbarianFiremaking) {
                ...
                submitIndividualPulse(
                    entity = player,
                    pulse = BarbFiremakingPulse(player, with.asItem(), null)
                )
            } else {
        ...
        onUseWith(IntType.GROUNDITEM, tools, *Firemaking.logs) { player, used, with ->
            var barbarianFiremaking = player.getAttribute(BarbarianTraining.FM_BASE, false) == true
            var completeBarbarianFiremaking = player.getAttribute(BarbarianTraining.FM_FULL, false) == true
            ...
            if (completeBarbarianFiremaking || barbarianFiremaking) {
                ...
                submitIndividualPulse(
                    entity = player,
                    pulse = BarbFiremakingPulse(player, with.asItem(), with as GroundItem)
                )
            } else {
                sendMessage(player, "In order to be able to lighting fires, Otto Godblessed must be talked to.")
            }
