...
import content.data.item.book.GeneralRuleBook.Companion.openBook
...
/**
 * Represents the Town crier dialogue.
 */
@Initializable
class TownCrierDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            173 -> npc("Play by the rules! The rules of " + settings!!.name + " are enforced", "for a reason, to make the game a fair and enjoyable", "environment for all.").also { stage++ }
            174 -> npc("Report accurately! When " + settings!!.name + " consider an account for", "review they look for quality, not quantity. Ensure your", "reports are of a high quality by following the report", "guidelines.").also { stage++ }
            175 -> npc("Be nice to each other! Treat others as you would", "want to be treated yourself. Respect your fellow player.", "More information can be found on the website.").also { stage = 153 }
    ...
    override fun getIds(): IntArray {
        return intArrayOf(NPCs.TOWN_CRIER_6135, NPCs.TOWN_CRIER_6136, NPCs.TOWN_CRIER_6137, NPCs.TOWN_CRIER_6138, NPCs.TOWN_CRIER_6139)
    }
