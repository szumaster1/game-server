...
import content.region.kandarin.ardougne.plaguecity.elena.cutscene.UndergroundCutscene
    ...
    override fun handle(componentID: Int, buttonID: Int): Boolean {
        ...
        when (getQuestStage(player!!, "Plague City")) {
            ...
            3 -> when (stage) {
                ...
                2 -> playerl(FacialExpression.NEUTRAL, "I still need to pour " + (4 - bucketUses) + " more buckets of water on the soil.").also { stage = END_DIALOGUE }
            ...
            100 -> when (stage) {
                1 -> npcl(FacialExpression.FRIENDLY, "Ah hello again, and thank you again for rescuing my daughter.").also { stage++ }
                2 -> if (!hasAnScroll) {
                    options("Do you have any more of those scrolls?", "No problem.").also { stage++ }
                } else {
                    playerl(FacialExpression.NEUTRAL, "No problem.").also { stage = END_DIALOGUE }
                }
                3 -> when (buttonID) {
                    1 -> playerl(FacialExpression.NEUTRAL, "Do you have any more of those scrolls?").also { stage++ }
                    2 -> playerl(FacialExpression.NEUTRAL, "No problem.").also { stage = END_DIALOGUE }
                }
                4 -> npcl(FacialExpression.FRIENDLY, "Here take this magic scroll, I have little use for it but it may help you.").also {
