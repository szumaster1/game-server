...
class KingArthurHGDialogue : DialogueFile() {
    ...
    override fun handle(componentID: Int, buttonID: Int) {
        ...
        when (stage) {
            0 -> {
                if (isQuestComplete(player!!, QuestName.HOLY_GRAIL)) {
                    ...
                    if (getQuestStage(player!!, QuestName.HOLY_GRAIL) == 50 && player!!.hasItem(Item(Items.HOLY_GRAIL_19, 1))) {
                        stage = 60
                    } else {
                        stage = 30
                    }
            ...
            17 -> npcl(FacialExpression.NEUTRAL, "Go speak to Merlin. He may be able to give a better clue as to where it is now you have freed him from that crystal.").also {
                setQuestStage(player!!, QuestName.HOLY_GRAIL, 10)
            ...
            62 -> {
                ...
                stage = END_DIALOGUE
                getQuest(player!!, QuestName.HOLY_GRAIL).finish(player!!)
            }
