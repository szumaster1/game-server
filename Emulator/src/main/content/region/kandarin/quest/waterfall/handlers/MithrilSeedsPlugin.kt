...
class MithrilSeedsPlugin : OptionHandler() {
    ...
    override fun handle(player: Player, node: Node, option: String): Boolean {
        ...
        if (getObject(player.location) != null) {
            player.packetDispatch.sendMessage("You can't plant a flower here.")
            return true
        }
        player.animate(ANIMATION)
        player.inventory.remove(ITEM)
        val scenery: Scenery = SceneryBuilder.add(Scenery(getFlower(if (RandomFunction.random(100) == 1) RARE else FLOWERS), player.location), 100)
        player.moveStep()
        player.lock(3)
        ...
        player.packetDispatch.sendMessage("You open the small mithril case and drop a seed by your feet.")
        return true
    }
