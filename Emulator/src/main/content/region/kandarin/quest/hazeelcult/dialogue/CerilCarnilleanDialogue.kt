...
/**
 * Represents the Ceril Carnillean dialogue.
 */
@Initializable
class CerilCarnilleanDialogue(player: Player? = null) : Dialogue(player) {

    override fun open(vararg args: Any?): Boolean {
        npc = args[0] as NPC
        if (getQuestStage(player, "Hazeel Cult") == 1) {
            playerl(FacialExpression.FRIENDLY, "Hello, Ceril.").also { stage++ }
        } else if (getAttribute(player!!, CARNILLEAN, true) && getQuestStage(player, "Hazeel Cult") == 4) {
            playerl(FacialExpression.FRIENDLY, "I've killed the cult leader, and he dropped your armour!").also { stage++ }
        } else if (getAttribute(player!!, CARNILLEAN, true) && getQuestStage(player, "Hazeel Cult") == 5 && inBorders(player, 2571, 3267, 2575, 3273)) {
        ...
        when (getQuestStage(player!!, "Hazeel Cult")) {
            ...
            5 -> when (stage) {
                ...
                8 -> {
                    ...
                    finishQuest(player, "Hazeel Cult")
                }
            }
