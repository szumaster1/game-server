        ...
        on(Scenery.BOOKCASE_10273, IntType.SCENERY, "study") { player, _ ->
            openDialogue(player, object : DialogueFile() {
                override fun handle(componentID: Int, buttonID: Int) {
                    when (stage) {
                        0 -> options("The Times of Lathas", "The History of the Outpost", "The Mysterious Adventurer").also { stage++ }
                        1 -> when (buttonID) {
                            1 -> sendDialogue(player, "You pick up a new looking book: 'The Times of Lathas'. You skim over the heavy book. It talks about the heritage of the line of kings who carry the name Ardignas. They only came into power 68 years ago, but in which time there have been five kings, the current being King Lathas.").also { stage = END_DIALOGUE }
                            2 -> {
                                end()
