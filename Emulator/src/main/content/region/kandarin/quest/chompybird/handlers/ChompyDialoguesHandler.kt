...
class ChompyDialoguesHandler(val quest: Quest) : DialogueFile() {
    ...
    private fun handleQuestStartDialogue(player: Player?, buttonId: Int) {
        when (stage) {
            ...
            ).also { stage = END_DIALOGUE; quest.start(player) }
        }
    }
    ...
    private fun handleToadsiesDialogue(player: Player?, buttonId: Int) {
        ...
        when (stage) {
            ...
            11 -> npcl(
                FacialExpression.OLD_NORMAL,
                "Ok creature? You got dat? Over here by der no tree's place."
            ).also { stage = 4 }
    ...
    private fun handlePlaceToadsiesDialogue(player: Player?, buttonId: Int) {
        ...
        registerHintIcon(
            player,
            BigChompyBirdHunting.TOAD_LOCATION,
            5
        ) //TODO: ADD OFFSET
        stage = END_DIALOGUE
    }
    ...
    private fun handlePlacedBaitDialogue(player: Player?, buttonId: Int) {
        when (stage) {
            ...
            4 -> playerl("What? I have to get more bait if there's none there? Does this Chompy Bird even exist I wonder?").also {
                stage = END_DIALOGUE
            }
        }
    }
    ...
    private fun handleRantzSucksDialogue(player: Player?, buttonId: Int) {
        when (stage) {
            ...
            3 -> when (buttonId) {
                1 -> playerl("Oh, keep trying then... you might hit one through pure luck.").also {
                    stage = END_DIALOGUE
                }
    ...
    private fun handleWaitingForChompyDialogue(player: Player?, buttonId: Int) {
        ...
        when (stage) {
            ...
            2 -> npcl(FacialExpression.OLD_NORMAL, "Well hurry up and get some, we is hungry!").also {
                stage = END_DIALOGUE
            }
            ...
            103 -> npcl(
                FacialExpression.OLD_NORMAL,
                "But's we's particular about our chompy yumms. Me's wants ${getItemName(rantzIngredient)} wiv mine! Fycie and Bugs want something wiv der's as well, go and ask 'em wat dey want."
            ).also { stage++ }
