...
class BigChompyBirdHunting : Quest("Big Chompy Bird Hunting", 35, 34, 2, Vars.VARP_QUEST_CHOMPY_PROGRESS, 0, 1, 65),
    ...
    override fun defineListeners() {
        ...
        onUseWith(IntType.SCENERY, Items.RAW_CHOMPY_2876, 3375) { player, used, _ ->
            ...
            val bugsIngredient = getAttribute(
                player,
                ATTR_ING_BUGS, -1
            )
            val fycieIngredient = getAttribute(
                player,
                ATTR_ING_FYCIE, -1
            )
            ...
            if (
                ...
                runTask(player, 4) {
                    ...
                    sendItemDialogue(player, Items.SEASONED_CHOMPY_2882, "You use the ${getItemName(rantzIngredient).lowercase()}, ${getItemName(bugsIngredient).lowercase()} and the ${getItemName(fycieIngredient)
                    } with the chompy bird to make a seasoned chompy.")
                    if (removeItem(player, used.asItem()) && removeItem(player, rantzIngredient) && removeItem(player, bugsIngredient) && removeItem(player, fycieIngredient)) addItem(player, Items.SEASONED_CHOMPY_2882)
