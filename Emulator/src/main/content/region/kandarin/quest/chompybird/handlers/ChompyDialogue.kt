...
class ChompyDialogue(val quest: Quest) : DialogueFile() {
    ...
    private fun handlePlaceToadsiesDialogue(player: Player?, buttonId: Int) {
        ...
        registerHintIcon(player, BigChompyBirdHunting.TOAD_LOCATION, 5) //TODO: ADD OFFSET
        stage = END_DIALOGUE
    }
    ...
    private fun assignRandomIngredients(player: Player) {
        val possibleIngredients = arrayListOf(Items.ONION_1957, Items.DOOGLE_LEAVES_1573, Items.EQUA_LEAVES_2128, Items.TOMATO_1982, Items.POTATO_1942, Items.CABBAGE_1965)
        setAttribute(player, BigChompyBirdHunting.ATTR_ING_RANTZ, possibleIngredients.random().also { possibleIngredients.remove(it) })
        setAttribute(player, BigChompyBirdHunting.ATTR_ING_BUGS, possibleIngredients.random().also { possibleIngredients.remove(it) })
        setAttribute(player, BigChompyBirdHunting.ATTR_ING_FYCIE, possibleIngredients.random().also { possibleIngredients.remove(it) })
    }
}
