package content.region.kandarin.quest.zogre.npc
...
class ZavisticRarveNPC(id: Int = 0, location: Location? = null) : AbstractNPC(id, location) {
    ...
    override fun handleTickActions() {
        ...
        if (player != null) {
            if (player.location.getDistance(getLocation()) > 4 || !player.isActive || clearTime++ > 300) {
                poofClear(this)
                removeAttribute(player, ZogreQuestUtils.NPC_ACTIVE)
            }
        }
    }
    ...
    companion object {
        fun spawnWizard(player: Player) {
            ...
            GameWorld.Pulser.submit(object : Pulse(0, wizard) {
                override fun pulse(): Boolean {
                    wizard.init()
                    setAttribute(player, ZogreQuestUtils.NPC_ACTIVE, true)
