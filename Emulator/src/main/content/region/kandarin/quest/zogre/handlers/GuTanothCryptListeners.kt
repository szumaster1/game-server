    ...
    override fun defineListeners() {
        ...
        on(COFFIN, IntType.SCENERY, "pick-lock") { player, node ->
            ...
            submitIndividualPulse(player, object : Pulse(2) {
                var table = ThievingPickpocketListener.pickpocketRoll(player, 84.0, 240.0, OgreCoffinLootTable.OGRE_COFFIN.table)
        ...
        on(OGRE_DOORS, IntType.SCENERY, "open") { player, node ->
            if(!inInventory(player, Items.OGRE_GATE_KEY_4839) || !getAttribute(player,
                    ZUtils.RECEIVED_KEY_FROM_GRISH, false)) {
