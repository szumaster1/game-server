package content.region.desert.quest.desertrescue.plugin
...
import content.region.desert.quest.desertrescue.plugin.TouristTrapPlugin.AnnaCartHandler.AnnaCartCutscene
import content.region.desert.quest.desertrescue.plugin.TouristTrapPlugin.BedabinAnvilHandler.AnnaWinchHandler
import core.api.*
...
class TouristTrapPlugin : OptionHandler() {
    ...
    override fun handle(player: Player, node: Node, option: String): Boolean {
        ...
        when (option) {
            ...
            "open" -> when (id) {
                ...
                2686, 2685 -> sendNPCDialogue(player,
                    4999,
                ...
                2677 -> {
                    ...
                    if (quest.getStage(player) <= 54 && quest.getStage(player) != 53) {
                        player.packetDispatch.sendMessage("The captain spots you before you manage to open the chest...")
                        player.lock(3)
                ...
                2676, 2675 -> {
                    if (!TouristTrap.hasSlaveClothes(player)) {
                        sendNPCDialogue(player,
                            4997,
                            "Watch it! Only slaves can travel into the mine."
                        )
                    }
                    setAttribute(player, "ana-delay", ticks + 2)
                ...
                2674, 2673 -> {
                    if (quest.getStage(player) > 60 && quest.getStage(player) < 98 && player.inventory.containsItem(
                        ...
                        player.lock()
                        sendNPCDialogue(player,4999,
                            "Would you like me to take that heavy barrel for you?"
                        )
                        Pulser.submit(object : Pulse(4, player) {
                            ...
                            override fun pulse(): Boolean {
                                when (++counter) {
                                    ...
                                    3 -> player.packetDispatch.sendMessage("You are roughed up by the guards and manhandled into a cell.")
                                    4 -> {
                                        player.unlock()
                    ...
                    if (node.location.withinDistance(Location(3273, 3028, 0)) && player.location.x < 3274) {
                        ...
                        DoorActionHandler.handleAutowalkDoor(player, node as Scenery)
                        player.packetDispatch.sendMessage("The guards search you thoroughly as you go through the gates.")
                        return true
                    }
                    DoorActionHandler.handleAutowalkDoor(player, node as Scenery)
                }
            ...
            "search" -> when (id) {
                ...
                18958, 18959 -> {
                    ...
                    player.packetDispatch.sendMessage("This looks like a mine cart which takes barrels out of the encampment to Al Kharid.")
                }
            ...
            "bend" -> {
                ...
                Pulser.submit(object : Pulse(5, player) {
                    override fun pulse(): Boolean {
                        player.packetDispatch.sendMessage("You bend the bars back.")
                        setVarbit(player, 2801, 1)
                        return true
                    }
                })
            }
            ...
            "inspect" -> if (id == 18875) {
                player.dialogueInterpreter.sendDialogue("You remember that Irena mentioned something about Ana wearing a red scarf before she left for the desert.")
            }
    ...
    private fun hasItem(player: Player, item: Item): Boolean {
    ...
    /**
     * Instantiates a new Anna cart handler.
     */
        : UseWithHandler(TouristTrap.ANNA_BARREL.id) {
        override fun newInstance(arg: Any?): Plugin<Any> {
        ...
        class AnnaCartCutscene : CutscenePlugin {
            /**
             * Instantiates a new Anna cart cutscene.
             */
                    ...
                    override fun pulse(): Boolean {
                        when (++counter) {
                            ...
                            22 -> {
                                ...
                                for (l in PATHS[0]) {
                                    val loc = base.transform(l.localX, l.localY, 0)
                                    cart!!.getWalkingQueue().addPath(loc.x, loc.y)
                                }
                            }
    ...
    class MineCartDialogue(player: Player? = null) : Dialogue(player) {
        ...
        class MiningCartCutscene : CutscenePlugin {
            ...
            /**
             * Instantiates a new Mining cart cutscene.
             */
            ...
            override fun open() {
                ...
                SceneryBuilder.remove(
                    getObject(
                        base.location.transform(
                            path[path.size - 1].localX, path[path.size - 1].localY, 0
                        )
                    )
                )
                ...
                Pulser.submit(object : Pulse(22, player) {
                    override fun pulse(): Boolean {
                        setAttribute(
                            player,
                            "real-end",
    ...
    class WinchCutscene : CutscenePlugin {
        /**
         * Instantiates a new Winch cutscene.
         */
        ...
        override fun open() {
            ...
            Pulser.submit(object : Pulse(AnimationDefinition.forId(5054)!!.getDurationTicks(), player) {
                override fun pulse(): Boolean {
    ...
    /**
     * Instantiates a new Bedabin key handler.
     */
        : UseWithHandler(TouristTrap.BEDABIN_KEY.id) {
        override fun newInstance(arg: Any?): Plugin<Any> {
    ...
    /**
     * Instantiates a new Bedabin anvil handler.
     */
        : UseWithHandler(2349) {
        override fun newInstance(arg: Any?): Plugin<Any> {
        ...
        /**
         * Instantiates a new Anna winch handler.
         */
            : UseWithHandler(TouristTrap.ANNA_BARREL.id) {
            override fun newInstance(arg: Any?): Plugin<Any> {
        ...
        /**
         * Instantiates a new Prototype dart handler.
         */
            : UseWithHandler(314) {
            override fun newInstance(arg: Any?): Plugin<Any> {
        ...
        class BedabinAnvilDialogue(player: Player? = null) : Dialogue(player) {
            ...
            /**
             * Instantiates a new Proto type pulse.
             *
             * @param player the player
             */
                (player: Player?) : SkillPulse<Scenery?>(player, null) {
                private var ticks = 0
