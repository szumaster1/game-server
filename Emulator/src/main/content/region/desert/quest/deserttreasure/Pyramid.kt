 ...
 * @author <a href="https://gitlab.com/ovenbreado">Ovenbreado</a>
 */
class Pyramid {
    companion object {
        /*
        ...
        fun nearSarcophagus(loc: Location): Location? {
            for (sarcoph in sarcophagusList) {
                if(loc.withinDistance(sarcoph, 2)) {
                    return sarcoph
                }
            }
            return null
...
class PyramidAreaFirstThree: MapArea {
    ...
    override fun entityStep(entity: Entity, location: Location, lastLocation: Location) {
        if (entity is Player) {
            ...
            if ((1..30).random() == 1) {
                Pyramid.spawnScarabs(entity)
            }

            if ((1..15).random() == 1) {
                // A mummy would jump out if you walk near a sarcophagus of 2 radius.
                val sarcoph = Pyramid.nearSarcophagus(entity.location)
                if (sarcoph != null) {
                    Pyramid.spawnMummy(entity, sarcoph)
                }
            }

            if (randomValue > averageLevel) {
                Pyramid.trapdoorTrap(entity)
            }
        }
    }
}
