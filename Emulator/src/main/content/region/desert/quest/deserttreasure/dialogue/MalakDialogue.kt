...
/**
 * Represents the Malak dialogue.
 * @author Ovenbreado
 */
@Initializable
class MalakDialogue(player: Player? = null) : Dialogue(player) {
...
/**
 * Represents the Malak dialogue file.
 * @author Ovenbreado
 */
    ...
    override fun create(b: DialogueBuilder) {
        ...
        b.onQuestStages(DesertTreasure.questName, 9)
            .branch { player ->
                ...
                branch.onValue(2)
                    .branch { player ->
                        return@branch if (inInventory(player, Items.BLESSED_POT_4659) || inInventory(
                                player,
                        ...
                        branch2.onValue(1)
                            .player(
                                "I found Ruantun in Draynors sewers.",
                                "He made me this pot, now where can I get some fresh",
                                "blood to fill it with?"
                            )
                            .betweenStage { df, player, _, _ ->
                                sendMessage(player, "Malak cuts you and pours some of your blood into the pot.")
                                if (inInventory(player, Items.SILVER_POT_4658) && removeItem(
                                        player,
                                        Items.SILVER_POT_4658
                                    )
                                ) {
                                    addItemOrDrop(player, Items.SILVER_POT_4660)
                                } else if (inInventory(player, Items.BLESSED_POT_4659) && removeItem(
                                        player,
                                        Items.BLESSED_POT_4659
                                    )
                                ) {
                ...
                branch.onValue(100)
                    .branch { player ->
                        return@branch if (!inInventory(player, Items.BLOOD_DIAMOND_4670)) {
                            1
                        } else {
                            0
                        }
