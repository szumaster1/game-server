...
class DesertTreasureListener : InteractionListener {
    ...
    override fun defineListeners() {
        ...
        // THE MIRRORS
        on(Scenery.MYSTICAL_MIRROR_6423, SCENERY, "look-into") { _, _ ->
            println("SOUTH")
            // MirrorLookCutscene(player).start()
            return@on true
        }
        on(Scenery.MYSTICAL_MIRROR_6425, SCENERY, "look-into") { _, _ ->
            println("SOUTH-WEST")
            return@on true
        }
        on(Scenery.MYSTICAL_MIRROR_6427, SCENERY, "look-into") { _, _ ->
            println("NORTH-WEST")
            return@on true
        }
        on(Scenery.MYSTICAL_MIRROR_6429, SCENERY, "look-into") { _, _ ->
            println("NORTH")
            return@on true
        }
        on(Scenery.MYSTICAL_MIRROR_6431, SCENERY, "look-into") { _, _ ->
            println("NORTH-EAST")
            return@on true
        }
        on(Scenery.MYSTICAL_MIRROR_6433, SCENERY, "look-into") { _, _ ->
            println("SOUTH-EAST")
            return@on true
        }
        ...
        onUseWith(IntType.SCENERY, Items.BLOOD_DIAMOND_4670, Scenery.OBELISK_6483) { player, _, _ ->
        ...
        onUseWith(IntType.SCENERY, Items.SMOKE_DIAMOND_4672, Scenery.OBELISK_6486) { player, _, _ ->
            if (getDynLevel(player, Skills.MAGIC) > 50) {
                sendMessage(player, "The diamond is absorbed into the pillar.")
        ...
        onUseWith(IntType.SCENERY, Items.ICE_DIAMOND_4671, Scenery.OBELISK_6489) { player, _, _ ->
            if (getDynLevel(player, Skills.MAGIC) > 50) {
                sendMessage(player, "The diamond is absorbed into the pillar.")
        ...
        onUseWith(IntType.SCENERY, Items.SHADOW_DIAMOND_4673, Scenery.OBELISK_6492) { player, _, _ ->
            if (getDynLevel(player, Skills.MAGIC) > 50) {
                sendMessage(player, "The diamond is absorbed into the pillar.")
        ...
        // THE LADDERS
        on(Scenery.LADDER_6497, SCENERY, "climb-down") { player, _ ->
            teleport(player, Location(2913, 4954, 3))
            return@on true
        }
        on(Scenery.LADDER_6504, SCENERY, "climb-up") { player, _ ->
            teleport(player, Location(3233, 2898, 0))
            return@on true
        }

        on(Scenery.LADDER_6498, SCENERY, "climb-down") { player, _ ->
            teleport(player, Location(2846, 4964, 2))
            return@on true
        }
        on(Scenery.LADDER_6503, SCENERY, "climb-up") { player, _ ->
            teleport(player, Location(2909, 4963, 3))
            return@on true
        }

        on(Scenery.LADDER_6499, SCENERY, "climb-down") { player, _ ->
            teleport(player, Location(2782, 4972, 1))
            return@on true
        }
        on(Scenery.LADDER_6502, SCENERY, "climb-up") { player, _ ->
            teleport(player, Location(2845, 4973, 2))
            return@on true
        }

        on(Scenery.LADDER_6500, SCENERY, "climb-down") { player, _ ->
            teleport(player, Location(3233, 9293, 0))
            return@on true
        }
        on(Scenery.LADDER_6501, SCENERY, "climb-up") { player, _ ->
