...
class RasoloDialogueFile : DialogueBuilderFile() {
    override fun create(b: DialogueBuilder) {
                        ...
                        optionBuilder2.option("Yes")
                            ...
                            .endWith { _, player ->
                                if (DesertTreasure.getSubStage(
                                        player,
                                        DesertTreasure.attributeShadowStage
                                    ) == 0
                                ) {
        ...
        b.onPredicate { player ->
            ...
            .branch { player ->
                if (DesertTreasure.getSubStage(player, DesertTreasure.attributeShadowStage) == 2 || inInventory(
                        player,
        ...
        b.onPredicate { player ->
            DesertTreasure.getSubStage(player, DesertTreasure.attributeShadowStage) >= 3 &&
                    getQuestStage(player, DesertTreasure.questName) >= 9 || getQuestStage(
                player,
                DesertTreasure.questName
            ) >= 10
        }
