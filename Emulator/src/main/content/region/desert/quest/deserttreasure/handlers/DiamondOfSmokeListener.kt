...
class DiamondOfSmokeListener : InteractionListener {
    ...
    override fun defineListeners() {
        ...
        on(intArrayOf(Scenery.GATE_6451, Scenery.GATE_6452), SCENERY, "open") { player, node ->
            if (inInventory(player, Items.WARM_KEY_4656)) {
                if (DesertTreasure.getSubStage(player, DesertTreasure.attributeSmokeStage) == 1 &&
...
/**
 * Represents the Standing torch timer
 *
    ...
    override fun run(entity: Entity): Boolean {
        if (entity is Player) {
            when(timerIdentifier) {
                DiamondOfSmokeListener.timerIdentifierTorchNE -> sendMessage(entity, "The North-east torch burns out...")
                DiamondOfSmokeListener.timerIdentifierTorchSE -> sendMessage(entity, "The South-east torch burns out...")
                DiamondOfSmokeListener.timerIdentifierTorchSW -> sendMessage(entity, "The South-west torch burns out...")
                DiamondOfSmokeListener.timerIdentifierTorchNW -> sendMessage(entity, "The North-west torch burns out...")
