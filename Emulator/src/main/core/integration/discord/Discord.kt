...
class Discord : TickListener {
    ...
    companion object {
        ...
        fun postNewOffer(isSale: Boolean, itemId: Int, value: Int, qty: Int, user: String) {
            if (Configuration.DISCORD_GE_WEBHOOK.isEmpty()) return
            GlobalScope.launch {
                val offer = encodeOfferJson(isSale, itemId, value, qty, user)
                try {
                    sendJsonPost(Configuration.DISCORD_GE_WEBHOOK, offer)
                } catch (e: Exception) {
                    e.printStackTrace()
                }
            }
        ...
        fun postPlayerAlert(player: String, type: String) {
            if (Configuration.DISCORD_MOD_WEBHOOK.isEmpty()) return
            GlobalScope.launch {
                val alert = encodeUserAlert(type, player)
                try {
                    sendJsonPost(Configuration.DISCORD_MOD_WEBHOOK, alert)
                } catch (e: Exception) {
                    e.printStackTrace()
                }
            }
        ...
        @JvmStatic fun sendToOpenRSC(player: String, type: String) {
            if (Configuration.DISCORD_OPENRSC_HOOK.isEmpty()) return
            GlobalScope.launch {
                val alert = encodeUserAlert(type, player)
                try {
                    sendJsonPost(Configuration.DISCORD_OPENRSC_HOOK, alert)
                } catch (e: Exception) {
                    e.printStackTrace()
                }
            }
        ...
        private fun sendJsonPost(url: String = Configuration.DISCORD_GE_WEBHOOK, data: String) {
            try {
