...
import core.network.packet.outgoing.AudioPacket
 ...
 */
fun replaceSlot(
    player: Player, slot: Int, item: Item, currentItem: Item? = null, container: Container = Container.INVENTORY
): Item? {
    val cont = when (container) {
        Container.INVENTORY -> player.inventory
        Container.EQUIPMENT -> player.equipment
    ...
    PlayerMonitor.log(
        player, LogType.DUPE_ALERT, "Potential slot-replacement-based dupe attempt, slot: $slot, item: $item"
    )
...
@JvmOverloads
fun playGlobalAudio(
    location: Location, id: Int, delay: Int = 0, loops: Int = 1, radius: Int = Audio.defaultAudioRadius
) {
    val nearbyPlayers = RegionManager.getLocalPlayers(location, radius)
    for (player in nearbyPlayers) {
        PacketRepository.send(
            AudioPacket::class.java, DefaultContext(player, Audio(id, delay, loops, radius), location)
        )
    }
}
...
fun truncateLoc(mover: Entity, destination: Location): Pair<Boolean, Location> {
    ...
    var multiplier = if (mover is NPC) 14.0 else Configuration.MAX_PATHFIND_DISTANCE.toDouble()
...
fun getCharge(node: Node): Int {
    when (node) {
        ...
        is Scenery -> return node.charge
            ...
            ContentAPI::class.java, Log.ERR, "Attempt to get charge of invalid type: ${node.javaClass.simpleName}"
        ).also { return -1 }
    }
}
...
fun setCharge(node: Node, charge: Int) {
    when (node) {
        ...
        is Scenery -> node.charge = charge
            ...
            ContentAPI::class.java, Log.ERR, "Attempt to set the charge of invalid type: ${node.javaClass.simpleName}"
        )
    }
}
 ...
 */
fun teleport(
    entity: Entity, loc: Location, type: TeleportManager.TeleportType = TeleportManager.TeleportType.INSTANT
): Boolean {
 ...
 * @param msg    The message to send.
 */
fun sendNPCDialogueLines(player: Player, npc: Int, expr: FacialExpression, hideContinue: Boolean, vararg msgs: String) {
...
/**
 * Removes an items from a player inventory.
 */
...
/**
 * Removes an items from a player equipment.
 */
...
fun getPathableRandomLocalCoordinate(target: Entity, radius: Int, center: Location, maxAttempts: Int = 3): Location {
    var maxRadius = Vector.deriveWithEqualComponents(Configuration.MAX_PATHFIND_DISTANCE.toDouble()).x - 1
...
enum class SecondaryBankAccountActivationResult {
    SUCCESS, ALREADY_ACTIVE, NOT_ENOUGH_MONEY, INTERNAL_FAILURE
}
...
fun calculateDragonfireMaxHit(
    entity: Entity, maxDamage: Int, wyvern: Boolean = false, unprotectableDamage: Int = 0, sendMessage: Boolean = false
): Int {
