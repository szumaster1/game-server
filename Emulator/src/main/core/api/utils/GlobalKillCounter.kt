...
class GlobalKillCounter : StartupListener, ShutdownListener {
    override fun startup() {
        ...
        val file = File(Configuration.DATA_PATH + File.separator + "global_kill_stats.json")
        if (!file.exists()) {
            return
        }
        ...
        try {
            ...
            val tmp_kills = data["kills"]
            populate(kills, tmp_kills)
            val tmp_rare_drops = data["rare_drops"]
                ...
                for ((player, tmp_kc) in obj.asIterable()) {
                    if (player is String) {
                        ...
                        for ((npc_id, count) in (tmp_kc as JSONObject).asIterable()) {
                            kc[java.lang.Long.parseLong(npc_id as String)] = count as Long
                        }
                        field[player] = kc
                    }
                }
