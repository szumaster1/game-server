    ...
    private fun handleEvent(event: Message) {
        when (event) {
            ...
            is SendContactInfo -> {
                ...
                PacketRepository.send(ContactPackets::class.java, ContactContext(p, ContactContext.UPDATE_STATE_TYPE))
                ...
                PacketRepository.send(ContactPackets::class.java, ContactContext(p, ContactContext.IGNORE_LIST_TYPE))
            ...
            is SendClanInfo -> {
                if (event.hasInfo) {
                    ...
                    if (info.clanName.isNotEmpty()) {
                        ...
                        if (GameWorld.settings!!.enable_default_clan && event.clanOwner == Configuration.SERVER_NAME.lowercase()) {
                            /*
                            ...
                            if (info == UserAccountInfo.createDefault()) {
                                info.username = Configuration.SERVER_NAME.lowercase()
                                info.password = Configuration.MS_SECRET_KEY
                                info.rights = 2
