...
import core.net.packet.incoming.Packet
import core.net.packet.incoming.RunScript
...
object PacketProcessor {
        ...
        when (pkt) {
            ...
            is Packet.InputPromptResponse -> {
                ...
                try {
                    ...
                    removeAttribute(pkt.player,"parseamount")
                    removeAttribute(pkt.player,"runscript")
                }
            }
            is Packet.ReportAbuse -> {
                ...
                if (pkt.target.equals(pkt.player.username, true)){
                    sendMessage(pkt.player,"You can't report yourself!")
                }
            ...
            is Packet.ChatMessage -> {
                if (pkt.player.details.isMuted)
                    ...
                    if (Configuration.ENABLE_GLOBALCHAT && pkt.message.startsWith("//")) {
    ...
    private fun processComponentUseWith(pkt: Packet) {
        ...
        if (targetId != target.id)
            return
        if (getAttribute(player, "magic:delay", -1) > GameWorld.ticks)
            return
    ...
    private fun processWalkPacket(pkt: Packet) {
        ...
        if (vec.magnitude() > Configuration.MAX_PATHFIND_DISTANCE) {
            val newVec = vec.normalized() * (Configuration.MAX_PATHFIND_DISTANCE - 1)
    ...
    private fun processCloseIface(pkt: Packet.CloseIface) {
        ...
        if (player.getAttribute<Boolean>("logging_in") != null) {
            GameWorld.Pulser.submit(object : Pulse() {
                override fun pulse(): Boolean {
                    removeAttribute(player,"logging_in")
    ...
    private fun processPlayerAction(pkt: Packet.PlayerAction) {
        val player = pkt.player
        if (pkt.otherIndex !in 1 until Configuration.MAX_PLAYERS) {
            return sendClearMinimap(player)
        }
        player.scripts.reset()
    ...
    private fun processSceneryAction(pkt: Packet.SceneryAction) {
        ...
        if (hasWrapper) {
            player.debug("WrapperID: ${scenery.id}, ${scenery.definition.configFile?.let { "VarbitPacket: ${it.id}"} ?: "Varp: ${scenery.definition.configId}"}")
        }
        player.debug("------------------------------------------------")
