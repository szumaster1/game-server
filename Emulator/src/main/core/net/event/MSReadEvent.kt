 ...
 * @author Emperor
 */
class MSReadEvent(session: IoSession, buffer: ByteBuffer) : IoReadEvent(session, buffer) {

    private val PACKET_SIZE = intArrayOf(-1, -1, -1, -2, -1, -1, -2, -1, -2, -1, 4, -1, -1, -1, -1, -1, -1, -1, -1, -1)

    override fun read(session: IoSession, buffer: ByteBuffer) {
        var last = -1
        while (buffer.hasRemaining()) {
            val opcode = buffer.get().toInt() and 0xFF
            if (opcode >= PACKET_SIZE.size) {
                break
            }
