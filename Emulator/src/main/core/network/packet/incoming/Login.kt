...
object Login {
    ...
    // Decode the login packet from the buffer
    fun decodeFromBuffer(buffer: ByteBuffer): Pair<AuthResponse, LoginInfo?> {
        try {
            ...
            val decryptedBuffer = decryptRSABuffer(buffer, Configuration.EXPONENT, Configuration.MODULUS);
    ...
    // Produce an ISAACPair from the buffer
    ...
    // Skip a certain amount of bytes in the buffer
    ...
    fun proceedWith(session: IoSession, details: PlayerDetails, opcode: Int) {
        ...
        if (flaggedIps.contains(details.ipAddress)) {
            PlayerMonitor.log(
                player,
                LogType.IP_LOG,
                "${details.username} Login from flagged IP ${details.ipAddress}."
            );
        }
    ...
    // Check if the player can bypass the account limit check
    ...
    // Proceed with the login process if it is acceptable
    ...
    // Check if the account limit has been exceeded
    private fun checkAccountLimit(ipAddress: String, username: String): Boolean {
        var accountLimit = Configuration.DAILY_ACCOUNT_LIMIT;

        if (File(Configuration.CONFIG_PATH + "account_limit_exceptions.conf").exists()) {
            try {
                val f = File(Configuration.CONFIG_PATH + "account_limit_exceptions.conf");
