...
class GrimDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            ).also { player.setAttribute("/save:hween2:grim_spoken", true); stage++ }
    ...
    fun handleRewardShop(player: Player, buttonId: Int) {
        ...
        if (!hasUnlocked && !hasEmote) {
            ...
            when (stage) {
                104 -> player.dialogueInterpreter.sendOptions(title, "Staff Orange Recolor (10c)", "Trick Emote (10c)")
                    .also { stage++ }
            ...
            npcl(FacialExpression.NEUTRAL, "You've already bought everything.").also { stage = END_DIALOGUE }
        }
    }
