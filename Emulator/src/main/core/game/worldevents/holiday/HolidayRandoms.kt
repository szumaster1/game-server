...
class HolidayRandoms : PersistTimer(0, "holiday", isAuto = true), Commands {
    ...
    override fun onRegister(entity: Entity) {
        if (entity !is Player || entity.isArtificial || !Configuration.HOLIDAY_EVENT_RANDOMS) {
    ...
    fun checkIfHoliday(): String {
        ...
        if ((!currentDate.isBefore(halloweenStartDate) && !currentDate.isAfter(halloweenEndDate)) || Configuration.FORCE_HALLOWEEN_EVENTS)
            return "halloween"

        if ((!currentDate.isBefore(christmasStartDate) && !currentDate.isAfter(christmasEndDate)) || Configuration.FORCE_CHRISTMAS_EVENTS)
            return "christmas"
    ...
    override fun defineCommands() {
        ...
        define("forcehrevents", Privilege.ADMIN, "::forcehrevents [eventname]", "Force enable holiday random events.") { player, args ->
            ...
            Configuration.HOLIDAY_EVENT_RANDOMS = true
            when (event) {
                "halloween" -> {
                    Configuration.FORCE_HALLOWEEN_EVENTS = true
                ...
                "christmas" -> {
                    Configuration.FORCE_CHRISTMAS_EVENTS = true
                    for (p in Repository.players) {
                        if (getTimer<HolidayRandoms>(p) != null || p.isArtificial) {
                            continue
                        }
        ...
        define("stophrevents", Privilege.ADMIN, "::stophrevents", "Stops all holiday random events.") { player, _ ->
            if (checkIfHoliday() == "none" || !Configuration.HOLIDAY_EVENT_RANDOMS)
                reject(player, "No holiday random events are currently active.")
            Configuration.HOLIDAY_EVENT_RANDOMS = false
            Configuration.FORCE_HALLOWEEN_EVENTS = false
            Configuration.FORCE_CHRISTMAS_EVENTS = false
            for (p in Repository.players) {
