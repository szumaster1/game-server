...
class EasterEvent : WorldEvent("easter"), TickListener, InteractionListener, LoginListener, Commands {
    ...
    override fun tick() {
        ...
        for (loc in locData) {
            val gi = GroundItemManager.create(GroundItem(Item(eggs.random()), loc, EGG_SPAWN_TIME, null))
    ...
    override fun checkActive(cal: Calendar): Boolean {
        return cal.get(Calendar.MONTH) == Calendar.APRIL || Configuration.FORCE_EASTER_EVENTS
    }
    ...
    companion object {
        ...
        val EGGS_BROKEN = "/save:easter${Configuration.STARTUP_MOMENT.get(Calendar.YEAR)}:eggs"
