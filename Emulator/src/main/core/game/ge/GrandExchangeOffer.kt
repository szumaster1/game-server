...
class GrandExchangeOffer {
    ...
    var amount: Int = 0
        get() = if (isBot) min(field, Configuration.BOTSTOCK_LIMIT) else field
     ...
     */
    val amountLeft: Int
        get() = if (isBot) min(Configuration.BOTSTOCK_LIMIT, amount - completedAmount) else amount - completedAmount
    ...
    fun visualize(player: Player?) {
        ...
        PacketRepository.send(
            ...
            GrandExchangeContext(
                player, index.toByte(), offerState.ordinal.toByte(), itemID.toShort(),
    ...
    fun writeNew() {
        GEDB.run { conn ->
            if (isBot) {
                ...
                GrandExchangeRecords.getInstance(player).offerRecords[index] =
                    GrandExchangeRecords.OfferRecord(uid, index)
                visualize(player)
                stmt.close()
