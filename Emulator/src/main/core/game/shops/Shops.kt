...
class Shops : StartupListener, TickListener, InteractionListener, InterfaceListener, Commands {
    ...
    override fun startup() {
        val path = Configuration.CONFIG_PATH + "shops.json"
        var shopCount = 0
    ...
    override fun tick() {
        ...
        val playerStockClearInterval = secondsToTicks(Configuration.PLAYER_STOCK_CLEAR_INTERVAL * 60)
        if (getWorldTicks() % playerStockClearInterval == 0) {
            val clearToGe = Configuration.PLAYER_STOCK_RECIRCULATE
            if (clearToGe) {
    ...
    override fun defineInterfaceListeners() {
        ...
        onClose(Components.SHOP_TEMPLATE_620) { player, _ ->
            ...
            else shop.stockInstances[Configuration.SERVER_NAME.hashCode()]!!.listeners.remove(listener)
