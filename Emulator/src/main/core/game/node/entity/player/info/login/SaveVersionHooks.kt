...
class SaveVersionHooks : LoginListener {
    ...
    override fun login(player: Player) {
        ...
        if (player.version < Configuration.CURRENT_SAVEFILE_VERSION) {
            // Send a message to the player about migrating their save file version
            sendMessage(
                player,
                "<col=CC6600>Migrating save file version ${player.version} to current save file version ${Configuration.CURRENT_SAVEFILE_VERSION}.</col>"
            )
            ...
            player.version = Configuration.CURRENT_SAVEFILE_VERSION
        }
    }
}