...
class PlayerSaveParser(val player: Player) {
    ...
    fun parse() {
        val JSON = File(Configuration.PLAYER_SAVE_PATH + player.name + ".json")
        if (JSON.exists()) {
            reader = FileReader(JSON)
        }
        reader ?: log(
            this::class.java, Log.WARN, "Couldn't find save file for ${player.name}, or save is corrupted."
        ).also { read = false }
        if (read) {
    ...
    fun parseFarming() {/*val farmingData = saveFile!!["farming"] as JSONObject
