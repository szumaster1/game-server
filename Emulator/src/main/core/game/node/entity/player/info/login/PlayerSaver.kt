...
class PlayerSaver(val player: Player) {
    ...
    fun save() = runBlocking {
        ...
        val json: String
        if (Configuration.JAVA_VERSION < 11) {
        ...
        try {
            if (!File("${Configuration.PLAYER_SAVE_PATH}${player.name}.json").exists()) {
                File("${Configuration.PLAYER_SAVE_PATH}").mkdirs()
                withContext(Dispatchers.IO) {
                    File("${Configuration.PLAYER_SAVE_PATH}${player.name}.json").createNewFile()
                }
            }
            withContext(Dispatchers.IO) {
                FileWriter("${Configuration.PLAYER_SAVE_PATH}${player.name}.json").use { file ->
                    file.write(json)
                    file.flush()
                }
            }
    ...
    fun saveGlobalData(root: JSONObject) {
        ...
        globalData["playerTestStage"] = player.savedData.globalData.getPlayerTestStage().toString()
